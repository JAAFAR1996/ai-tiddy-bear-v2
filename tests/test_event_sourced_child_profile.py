...
from infrastructure.repositories.event_sourced_child_repository import EventSourcedChildRepository
...
class TestChildProfileEventSourcing:
    def test_child_profile_update_dispatches_event(self):
        async def _test():
            ...
            initial_event_count = len(child.get_uncommitted_events()) # This will be 1 (ChildRegistered) and clear the list
    ...
    def test_child_profile_reconstructs_from_events(self):
        async def _test():
            ...
            events = [
                ...
                ChildProfileUpdated.create(child_id=child_id, preferences={"color": "blue"}),
            ]
...
class TestEventSourcedChildRepository:
    def test_save_and_load_child_profile(self, event_sourced_child_repository, sample_child_profile):
        async def _test():
            ...
            loaded_child = await event_sourced_child_repository.get_by_id(sample_child_profile.id)
    ...
    def test_load_non_existent_child(self, event_sourced_child_repository):
        async def _test():
            ...
            loaded_child = await event_sourced_child_repository.get_by_id(non_existent_id)
