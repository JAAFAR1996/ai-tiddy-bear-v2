...
class TestParentalDashboardEndpoints:
    ...
    async def test_get_children_list(self, client, auth_headers, child_data):
        ...
        with patch('application.use_cases.manage_child_profile.ManageChildProfileUseCase') as mock_use_case:
            ...
            response = await client.get("/api/v1/dashboard/children", headers=auth_headers)
    ...
    async def test_get_conversation_history(self, client, auth_headers, child_data):
        ...
        mock_conversations = [
            ...
            },
            {
                ...
                "ended_at": (datetime.utcnow() - timedelta(hours=2) + timedelta(minutes=10)).isoformat(),
                "message_count": 15,
        ...
        with patch('application.services.conversation_service.ConversationService') as mock_service:
            mock_service_instance = AsyncMock()
            mock_service_instance.get_conversation_history.return_value = mock_conversations
            ...
            assert data["conversations"][0]["summary"] == "Conversation about favorite animals"
    ...
    async def test_get_safety_report(self, client, auth_headers, child_data):
        ...
        with patch('infrastructure.security.safety_monitor_service.SafetyMonitorService') as mock_service:
            mock_service_instance = AsyncMock()
            mock_service_instance.generate_safety_report.return_value = mock_safety_report
            mock_service.return_value = mock_service_instance
    ...
    async def test_get_usage_analytics(self, client, auth_headers, child_data):
        ...
        with patch('application.services.analytics_service.AnalyticsService') as mock_service:
            mock_service_instance = AsyncMock()
    ...
    async def test_get_device_status(self, client, auth_headers):
        ...
        with patch('application.services.esp32_device_service.ESP32DeviceService') as mock_service:
            mock_service_instance = AsyncMock()
            mock_service_instance.get_device_status.return_value = mock_device_status
            mock_service.return_value = mock_service_instance

            response = await client.get("/api/v1/dashboard/devices", headers=auth_headers)
    ...
    async def test_unauthorized_access(self, client):
        ...
        response = await client.get("/api/v1/dashboard/children", headers=invalid_headers)
        assert response.status_code == 401
