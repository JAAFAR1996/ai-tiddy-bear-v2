...
class TestESP32Endpoints:
    ...
    @pytest.mark.asyncio
    async def test_process_audio_success(self, client, audio_request_data, mock_ai_response):
        """Test successful audio processing."""
        with patch('application.use_cases.process_esp32_audio.ProcessESP32AudioUseCase') as mock_use_case:
            ...
            data = response.json()
            assert data["response_text"] == "Hello! That's a great question about animals."
    ...
    async def test_process_audio_child_not_found(self, client, audio_request_data):
        ...
        try:
            ...
            from common.mock_fastapi import FastAPI, HTTPException
            pass
        pass
    ...
    async def test_process_audio_safety_violation(self, client, audio_request_data):
        ...
        with patch('application.use_cases.process_esp32_audio.ProcessESP32AudioUseCase') as mock_use_case:
            ...
            assert response.status_code == 200  # Still successful, but content is filtered
            data = response.json()
    ...
    async def test_concurrent_requests(self, client, audio_request_data):
        ...
        for i in range(5):
            ...
            request_data["child_id"] = str(uuid4())  # Different child for each request
    ...
    async def test_error_handling(self, client, audio_request_data):
        ...
        with patch('application.use_cases.process_esp32_audio.ProcessESP32AudioUseCase') as mock_use_case:
            mock_use_case_instance = AsyncMock()
            mock_use_case_instance.execute.side_effect = Exception("Internal service error")
            mock_use_case.return_value = mock_use_case_instance
