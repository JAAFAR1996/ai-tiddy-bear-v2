...
from src.infrastructure.persistence.database_service_orchestrator import DatabaseServiceOrchestrator
...
class TestChildOperationsIntegration:
    ...
    async def test_child_profile_lifecycle(self, db_service, test_user):
        ...
        # Update child
        success = await db_service.update_child(child_id, {
            "age": 9,
            "preferences": {
...
class TestDataRetentionIntegration:
    ...
    async def test_old_data_cleanup(self, db_service, test_user):
        ...
        # Note: In real test, we'd need to mock time or insert with old timestamps
