...
from src.infrastructure.security.hardening.coppa_compliance import ProductionCOPPACompliance
...
class TestCompleteUserFlow:
    ...
    @pytest.mark.asyncio
    async def test_login_and_child_creation(self, async_client: AsyncClient, child_data):
    ...
    @pytest.mark.asyncio
    async def test_conversation_flow(self, async_client: AsyncClient, auth_headers, child_data):
...
class TestSafetyAndCompliance:
    ...
    async def test_coppa_consent_flow(self, async_client: AsyncClient, auth_headers):
        ...
        assert consent_record["data_types_consented"] == ["data_collection", "safety_monitoring"]
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_database_error_handling(self, async_client: AsyncClient, auth_headers):
        """Test database error handling."""
        with patch("src.infrastructure.persistence.real_database_service.DatabaseService.create_child") as mock_create:
    ...
    @pytest.mark.asyncio
    async def test_external_service_failure(self, async_client: AsyncClient, auth_headers):
        """Test handling of external service failures."""
        # Mock OpenAI failure
        with patch("src.infrastructure.ai.real_ai_service.ProductionAIService.generate_response") as mock_ai:
...
class TestPerformanceAndCaching:
    ...
    async def test_response_caching(self, async_client: AsyncClient, auth_headers):
        ...
        assert second_duration < first_duration * 0.1  # Cached response should be much faster
