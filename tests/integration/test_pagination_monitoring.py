...
from src.infrastructure.pagination import (
    PaginationService, PaginationRequest, SortOrder
)
from src.infrastructure.monitoring import (
    monitoring_service, AlertSeverity, MetricType
)
...
class TestPaginationIntegration:
    ...
    def sample_data(self):
        ...
        return [
            {"id": i, "name": f"Item {i}", "created_at": datetime.utcnow() - timedelta(days=i)}
    ...
    def test_pagination_with_search(self, pagination_service, sample_data):
        ...
        # Should find items containing "Item 1" (Item 1, Item 10, Item 11, etc.)
    ...
    def test_pagination_with_sorting(self, pagination_service, sample_data):
        ...
        request = PaginationRequest(page=1, size=5, sort_by="id", sort_order=SortOrder.DESC)
        result = pagination_service.paginate_list(sample_data, request)
    ...
    async def test_conversation_search_integration(self):
        ...
        result = await service.search_conversations("child_123", "hello", 
                                                  PaginationRequest(page=1, size=5))
        
        assert result.metadata.page == 1
        # Search results should contain the search term
        for item in result.items:
            message_text = item.get("child_message", "").lower() + item.get("ai_response", "").lower()
...
class TestMonitoringIntegration:
    ...
    def test_child_safety_event_recording(self):
        ...
        assert len(monitoring_service.child_safety_monitor.safety_events) == initial_events + 1
    ...
    def test_emergency_alert_creation(self):
        ...
        assert len(monitoring_service.child_safety_monitor.safety_alerts) > initial_alerts
        
        # Find the emergency alert
        emergency_alerts = [
            alert for alert in monitoring_service.child_safety_monitor.safety_alerts.values()
...
class TestIntegratedWorkflow:
    ...
    async def test_safety_incident_workflow(self):
        ...
        pattern_alerts = [
            alert for alert in safety_alerts.values()
