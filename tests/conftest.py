...
from src.infrastructure.di.simplified_container import SimplifiedContainer, reset_container
from src.infrastructure.security.unified_security_service import UnifiedSecurityService, SecurityConfig
...
def test_settings():
    ...
    return Settings(
        ...
        OPENAI_API_KEY=os.getenv("TEST_OPENAI_API_KEY", "sk-test-key-not-real-for-testing-only"),
...
def mock_openai_client():
    ...
    client.chat.completions.create = AsyncMock(
        return_value=MagicMock( # Using MagicMock for more flexible attribute access
            ...
            usage=MagicMock(prompt_tokens=10, completion_tokens=20, total_tokens=30) # Usage stats
        )
    )
...
def reset_singletons():
    ...
    from src.infrastructure.persistence.real_database_service import reset_database_service
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "child_safety: Child safety tests"
    )
