...
class TestComprehensiveSecurityService:
    ...
    async def test_child_content_validation_safe(self, security_service):
        ...
        result = await security_service.validate_child_content(
            "Let's play with toys and read a fun story!",
            child_age=7
        )
    ...
    async def test_child_content_validation_unsafe_violence(self, security_service):
        ...
        result = await security_service.validate_child_content(
            "The character uses a gun to fight the monster",
            child_age=7
        )
        ...
        violence_issue = next((issue for issue in result["issues"] 
                             if issue["category"] == "violence"), None)
    ...
    async def test_child_content_validation_age_appropriate(self, security_service):
        ...
        assert len(result["issues"]) > 0
        age_issue = next((issue for issue in result["issues"]
                         if issue["category"] == "age_inappropriate"), None)
    ...
    async def test_incident_response_trigger(self, security_service):
        ...
        with patch.object(security_service, '_trigger_incident_response') as mock_incident:
    ...
    async def test_data_cleanup(self, security_service):
        ...
        remaining_threats = [t for t in security_service.threats 
                           if t.threat_id == "old123"]
...
class TestSecurityIntegration:
    ...
    async def test_audit_trail_completeness(self):
        ...
        # Check audit trail
        assert len(service.audit_entries) >= 0  # May have background audit entries
