...
class SecurityValidationTest:
    ...
    def test_hardcoded_credentials_removed(self):
        ...
        try:
            ...
            assert 'parent_id = "' not in source or 'parent_id = ""' in source, "Hardcoded parent ID pattern detected"
    ...
    def test_production_host_binding(self):
        ...
        try:
            ...
            with patch.dict(os.environ, {'ENVIRONMENT': 'production', 'HOST': '0.0.0.0'}):
                from src.main import get_host_config
                
                host = get_host_config()
                assert host != "127.0.0.1", "Production still bound to localhost"
                assert host in ["0.0.0.0", None], f"Unexpected host binding: {host}"
    ...
    def test_ssl_enforcement(self):
        ...
        try:
            ...
            with patch.dict(os.environ, {'ENVIRONMENT': 'production', 'SSL_CERT_PATH': '/fake/cert.pem'}):
    ...
    def test_sql_injection_prevention(self):
        ...
        try:
            ...
            from src.infrastructure.persistence.database.validators import DatabaseConnectionValidator
            ...
            assert 'SELECT version()' not in source or 'text("SELECT version()")' in source, "Raw SQL still present"
    ...
    def test_error_message_sanitization(self):
        ...
        try:
            ...
            test_error = Exception("Database connection failed: postgresql://user:password@host/db")
            safe_error = error_handler.create_safe_error_response(test_error)
            
            assert "password" not in str(safe_error.detail), "Password leaked in error message"
            assert "postgresql://user" not in str(safe_error.detail), "Connection string leaked"
            assert "Error ID:" in str(safe_error.detail), "Error ID not present for tracking"
        ...
        try:
            ...
            for key, value in test_vars.items():
                ...
                if "MALICIOUS" in key or "INJECTION" in key:
                    assert not result.is_valid, f"Failed to detect malicious env var: {key}"
                else:
    ...
    def test_deprecated_imports_removed(self):
        ...
        try:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                try:
                    from src.infrastructure.validation.comprehensive_input_validator import create_comprehensive_validator
                    # This should trigger a deprecation warning
                    assert len(w) > 0, "No deprecation warning for deprecated import"
                    assert "deprecated" in str(w[0].message).lower(), "Warning doesn't mention deprecation"
                except ImportError:
                    # If module doesn't exist, that's also fine (completely removed)
                    pass
    ...
    def test_type_annotations_present(self):
        ...
        try:
            ...
            from src.infrastructure.security.database_input_validator import SafeDatabaseOperations
