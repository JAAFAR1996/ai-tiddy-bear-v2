...
class TestResponseGenerator:
    ...
    def mock_session(self):
        ...
        return SessionData(
            ...
            data={"child_preferences": {"favorite_color": "blue", "interests": ["animals", "space"]}}
        )
    ...
    @pytest.mark.asyncio
    async def test_determine_activity_type_with_ai_success(self, generator_with_ai, mock_ai_service, mock_session):
        ...
        # Act
        result = await generator_with_ai.determine_activity_type(text, emotion, mock_session)
    ...
    @pytest.mark.asyncio
    async def test_determine_activity_type_with_ai_error(self, generator_with_ai, mock_ai_service, mock_session):
        """Test activity type determination with AI service error."""
        # Arrange
        text = "Let's play a game"
        emotion = {"sentiment": "excited", "score": 0.9}
        mock_ai_service.determine_activity_type.side_effect = Exception("AI service error")
    ...
    @pytest.mark.asyncio
    async def test_determine_activity_type_without_ai(self, generator_without_ai, mock_session):
        ...
        # Act
        result = await generator_without_ai.determine_activity_type(text, emotion, mock_session)
        
        # Assert
        assert result == ActivityType.CONVERSATION  # Default fallback
    
    @pytest.mark.asyncio
    async def test_generate_contextual_response_with_ai_success(self, generator_with_ai, mock_ai_service, mock_session):
        ...
        # Act
        result = await generator_with_ai.generate_contextual_response(text, emotion, mock_session)
    ...
    @pytest.mark.asyncio
    async def test_generate_contextual_response_with_ai_error(self, generator_with_ai, mock_ai_service, mock_session):
        ...
        with patch.object(generator_with_ai, 'logger') as mock_logger:
            ...
            # Assert
            assert isinstance(result, ResponseContext)
            assert "I'm sorry, I couldn't generate a personalized response" in result.text
    ...
    @pytest.mark.asyncio
    async def test_generate_contextual_response_without_ai(self, generator_without_ai, mock_session):
        ...
        with patch.object(generator_without_ai, 'logger') as mock_logger:
            # Act
            result = await generator_without_ai.generate_contextual_response(text, emotion, mock_session)
    ...
    @pytest.mark.asyncio
    async def test_activity_type_all_values(self, generator_with_ai, mock_ai_service, mock_session):
        """Test that all ActivityType values can be determined."""
        text = "test input"
        emotion = {"sentiment": "neutral", "score": 0.5}
        
        activity_types = [
            "STORY", "GAME", "LEARNING", "SLEEP_ROUTINE", "CONVERSATION", "COMFORT"
        ]
    ...
    @pytest.mark.asyncio
    async def test_emotion_extraction_from_context(self, generator_with_ai, mock_ai_service, mock_session):
    ...
    async def test_session_data_handling(self, generator_with_ai, mock_ai_service):
        ...
        session_variations = [
            ...
            SessionData(child_id="child_2", session_id="session_2", data={"child_preferences": {"theme": "animals"}}),
            SessionData(child_id="child_3", session_id="session_3", data={"custom_field": "value"}),
        ]
        ...
        for session in session_variations:
            result = await generator_with_ai.generate_contextual_response(text, emotion, session)
            ...
            assert call_args[1]["child_preferences"] == session.data.get("child_preferences", None)
    
    @pytest.mark.asyncio
    async def test_concurrent_response_generation(self, generator_with_ai, mock_ai_service, mock_session):
        ...
        mock_ai_service.determine_activity_type.side_effect = ["STORY", "GAME", "LEARNING"]
        ...
        expected_activities = [ActivityType.STORY, ActivityType.GAME, ActivityType.LEARNING]
        for i, result in enumerate(results):
            assert result.activity_type == expected_activities[i]
    
    @pytest.mark.asyncio
    async def test_logging_behavior(self, generator_with_ai, mock_ai_service, mock_session):
        """Test logging behavior in different scenarios."""
        text = "test message"
        emotion = {"sentiment": "neutral", "score": 0.5}
        mock_ai_service.generate_response.return_value = "AI generated response text here"
        mock_ai_service.determine_activity_type.return_value = "CONVERSATION"
        
        with patch.object(generator_with_ai, 'logger') as mock_logger:
            await generator_with_ai.generate_contextual_response(text, emotion, mock_session)
    ...
    @pytest.mark.asyncio
    async def test_edge_cases_invalid_ai_responses(self, generator_with_ai, mock_ai_service, mock_session):
    ...
    @pytest.mark.asyncio
    async def test_different_emotion_sentiments(self, generator_with_ai, mock_ai_service, mock_session):
        """Test handling of different emotion sentiments."""
        text = "test input"
        mock_ai_service.generate_response.return_value = "Response"
        mock_ai_service.determine_activity_type.return_value = "CONVERSATION"
        
        sentiments = ["joy", "sadness", "anger", "fear", "surprise", "disgust", "neutral"]
    ...
    @pytest.mark.asyncio
    async def test_conversation_history_handling(self, generator_with_ai, mock_ai_service, mock_session):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_sequences(self, generator_with_ai, mock_ai_service, mock_session):
        ...
        with patch.object(generator_with_ai, 'logger'):
            ...
            result1 = await generator_with_ai.generate_contextual_response(text, emotion, mock_session)
            ...
            result2 = await generator_with_ai.generate_contextual_response(text, emotion, mock_session)
