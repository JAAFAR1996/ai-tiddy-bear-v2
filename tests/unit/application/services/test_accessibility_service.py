...
class MockAccessibilityProfileRepository:
    ...
    async def get_profile_by_child_id(self, child_id: UUID) -> AccessibilityProfile | None:
...
class MockAccessibilityConfig:
    ...
    def __init__(self):
        self.adaptation_rules = {
            ...
            "speech_impairment": ["alternative_input", "extended_timeout", "simplified_responses"],
            "cognitive_delay": ["simplified_language", "repetition", "visual_cues"],
            "motor_impairment": ["voice_control", "simplified_interface", "large_buttons"],
            "sensory_sensitivity": ["reduced_stimuli", "calming_colors", "gentle_sounds"]
        }
...
def sample_special_needs():
    ...
    return [SpecialNeedType.VISUAL_IMPAIRMENT, SpecialNeedType.HEARING_IMPAIRMENT]
...
class TestAccessibilityService:
    ...
    def test_init_with_default_logger(self, mock_repository, mock_config):
        ...
        with patch('src.application.services.accessibility_service.logger') as mock_default_logger:
            service = AccessibilityService(mock_repository, mock_config)
            assert service.logger is mock_default_logger

    @pytest.mark.asyncio
    async def test_create_accessibility_profile_success(self, accessibility_service, sample_child_id, sample_special_needs):
        """Test successful creation of accessibility profile."""
        # Act
        result = await accessibility_service.create_accessibility_profile(sample_child_id, sample_special_needs)
    ...
    @pytest.mark.asyncio
    async def test_create_accessibility_profile_logging(self, accessibility_service, sample_child_id, sample_special_needs):
        """Test that profile creation is logged correctly."""
        # Act
        await accessibility_service.create_accessibility_profile(sample_child_id, sample_special_needs)
        ...
        assert f"Creating accessibility profile for child: {sample_child_id}" in first_call[0][0]
        
        # Check second log call (profile created)
        second_call = accessibility_service.logger.info.call_args_list[1]
        assert f"Accessibility profile created and saved for child: {sample_child_id}" in second_call[0][0]

    @pytest.mark.asyncio
    async def test_create_accessibility_profile_with_empty_needs(self, accessibility_service, sample_child_id):
        """Test creating accessibility profile with empty special needs."""
        # Act
        result = await accessibility_service.create_accessibility_profile(sample_child_id, [])
    ...
    @pytest.mark.asyncio
    async def test_create_accessibility_profile_with_all_needs(self, accessibility_service, sample_child_id):
        """Test creating accessibility profile with all special needs types."""
        all_needs = list(SpecialNeedType)
        
        # Act
        result = await accessibility_service.create_accessibility_profile(sample_child_id, all_needs)
    ...
    @pytest.mark.asyncio
    async def test_create_accessibility_profile_repository_error(self, accessibility_service, sample_child_id, sample_special_needs):
    ...
    @pytest.mark.asyncio
    async def test_get_accessibility_profile_found(self, accessibility_service, sample_child_id, sample_special_needs):
        """Test successful retrieval of existing accessibility profile."""
        # Arrange - Create profile first
        created_profile = await accessibility_service.create_accessibility_profile(sample_child_id, sample_special_needs)
    ...
    @pytest.mark.asyncio
    async def test_get_accessibility_profile_not_found(self, accessibility_service, sample_child_id):
    ...
    @pytest.mark.asyncio
    async def test_get_accessibility_profile_logging_found(self, accessibility_service, sample_child_id, sample_special_needs):
        ...
        assert f"Attempting to retrieve accessibility profile for child: {sample_child_id}" in debug_call
    ...
    @pytest.mark.asyncio
    async def test_get_accessibility_profile_logging_not_found(self, accessibility_service, sample_child_id):
        ...
        accessibility_service.logger.info.assert_called_once()
        info_call = accessibility_service.logger.info.call_args[0][0]
        assert f"Accessibility profile not found for child: {sample_child_id}" in info_call

    @pytest.mark.asyncio
    async def test_get_accessibility_profile_repository_error(self, accessibility_service, sample_child_id):
    ...
    def test_get_adaptations_multiple_needs(self, accessibility_service):
        ...
        # Arrange
        needs = [SpecialNeedType.VISUAL_IMPAIRMENT, SpecialNeedType.HEARING_IMPAIRMENT]
        
        # Act
        result = accessibility_service._get_adaptations(needs)
        
        # Assert
        expected = ["audio_descriptions", "high_contrast", "large_text", "visual_indicators", "subtitles", "sign_language"]
        assert set(result) == set(expected)
    ...
    def test_get_adaptations_removes_duplicates(self, accessibility_service):
        ...
        accessibility_service.config.adaptation_rules["visual_impairment"] = ["audio_descriptions", "high_contrast"]
        accessibility_service.config.adaptation_rules["hearing_impairment"] = ["high_contrast", "subtitles"]
        needs = [SpecialNeedType.VISUAL_IMPAIRMENT, SpecialNeedType.HEARING_IMPAIRMENT]
    ...
    def test_get_accessibility_settings_motor_and_cognitive(self, accessibility_service):
        """Test accessibility settings for motor and cognitive impairments."""
        # Arrange
        needs = [SpecialNeedType.MOTOR_IMPAIRMENT, SpecialNeedType.COGNITIVE_DELAY]
    ...
    def test_get_accessibility_settings_config_override(self, accessibility_service):
        ...
        # Arrange
        needs = [SpecialNeedType.VISUAL_IMPAIRMENT]
        accessibility_service.config.accessibility_settings_rules["visual_impairment"]["custom_setting"] = "custom_value"
    ...
    def test_get_accessibility_settings_sensory_sensitivity(self, accessibility_service):
    ...
    @pytest.mark.asyncio
    async def test_full_workflow_create_and_retrieve(self, accessibility_service, sample_child_id, sample_special_needs):
        """Test complete workflow of creating and retrieving a profile."""
        # Create profile
        created_profile = await accessibility_service.create_accessibility_profile(sample_child_id, sample_special_needs)
        
        # Retrieve profile
        retrieved_profile = await accessibility_service.get_accessibility_profile(sample_child_id)
        ...
        assert retrieved_profile.preferred_interaction_mode == created_profile.preferred_interaction_mode
    ...
    async def test_multiple_profiles_isolation(self, accessibility_service):
        ...
        # Act
        profile_1 = await accessibility_service.create_accessibility_profile(child_id_1, needs_1)
        profile_2 = await accessibility_service.create_accessibility_profile(child_id_2, needs_2)
    ...
    async def test_concurrent_profile_operations(self, accessibility_service):
        ...
        create_tasks = [
            ...
            for child_id, needs in zip(child_ids, needs_list)
        ]
    ...
    @pytest.mark.asyncio
    async def test_error_handling_preserves_state(self, accessibility_service, sample_child_id):
        ...
        with pytest.raises(Exception):
            await accessibility_service.create_accessibility_profile(sample_child_id, [])
        
        # Reset error condition
        accessibility_service.repository.should_raise_exception = False
        
        # Should still work after error
        profile = await accessibility_service.create_accessibility_profile(sample_child_id, [])
