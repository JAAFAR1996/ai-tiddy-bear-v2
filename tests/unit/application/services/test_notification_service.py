...
class TestNotificationService:
    ...
    async def test_send_notification_basic(self, service):
        ...
        with patch('src.application.services.notification_service.datetime') as mock_datetime:
    ...
    async def test_send_notification_urgent(self, service):
        ...
        assert result["max_attempts"] == 3  # Urgent notifications get more attempts
        assert result["status"] == "sent"
    ...
    async def test_send_by_channel_email(self, service):
        ...
        with patch('src.application.services.notification_service.logger') as mock_logger:
    ...
    async def test_notification_error_handling(self, service):
        ...
        with patch.object(service, '_send_by_channel', side_effect=Exception("Channel error")):
            with patch('src.application.services.notification_service.logger') as mock_logger:
                result = await service.send_notification(recipient, message)
