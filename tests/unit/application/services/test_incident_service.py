...
class TestIncidentService:
    ...
    async def test_report_incident_basic(self, service):
        ...
        with patch('src.application.services.incident_service.datetime') as mock_datetime:
    ...
    async def test_get_incident_existing(self, service):
        ...
        incident_details = {"title": "Test Incident", "description": "Test description"}
    ...
    async def test_resolve_incident_existing(self, service):
        ...
        # First, report an incident
        incident_details = {"title": "Bug Report", "description": "UI not responsive"}
        reported = await service.report_incident(incident_details)
        incident_id = reported["id"]
    ...
    async def test_concurrent_incident_operations(self, service):
        ...
        incident_tasks = [
            service.report_incident({"title": f"Incident {i}", "description": f"Description {i}"})
            for i in range(5)
        ]
    ...
    async def test_resolution_timestamp_format(self, service):
        ...
        with patch('src.application.services.incident_service.datetime') as mock_datetime:
            ...
            mock_datetime.utcnow.return_value = datetime(2024, 3, 15, 11, 30, 45, 789012)
            
            resolved = await service.resolve_incident(incident["id"], {"solution": "Fixed"})
