...
class TestResponseGenerator:
    ...
    def generator_with_openai(self, mock_openai_client):
        ...
        with patch('src.application.services.ai.modules.response_generator.AsyncOpenAI') as mock_openai:
            mock_openai.return_value = mock_openai_client
            with patch('src.application.services.ai.modules.response_generator.OPENAI_AVAILABLE', True):
    ...
    def generator_without_openai(self):
        ...
        with patch('src.application.services.ai.modules.response_generator.OPENAI_AVAILABLE', False):
    ...
    @pytest.mark.asyncio
    async def test_generate_response_basic_success(self, generator_with_openai, mock_openai_client):
        """Test basic successful response generation."""
        # Mock OpenAI response
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message = Mock()
        mock_response.choices[0].message.content = "مرحبا! أنا دبدوب لطيف وأحب اللعب معك!"
    ...
    async def test_filter_input_safe_content(self, generator_with_openai):
        ...
        result = await generator_with_openai._filter_input(safe_text, SafetyLevel.STRICT)
    ...
    async def test_filter_input_unsafe_content(self, generator_with_openai):
        ...
        result = await generator_with_openai._filter_input(unsafe_text, SafetyLevel.STRICT)
        
        assert result["safe"] is False
        assert "[محذوف]" in result["text"]
        assert len(result["warnings"]) > 0
        assert any("Unsafe content detected" in warning for warning in result["warnings"])
    ...
    async def test_filter_input_length_truncation(self, generator_with_openai):
        ...
        result = await generator_with_openai._filter_input(long_text, SafetyLevel.MODERATE)
        
        assert len(result["text"]) == generator_with_openai.max_input_length
        assert "Input too long, truncating" in result["warnings"]
    
    @pytest.mark.asyncio
    async def test_generate_openai_response_educational(self, generator_with_openai, mock_openai_client):
        """Test OpenAI response generation for educational content."""
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message = Mock()
        mock_response.choices[0].message.content = "دعنا نتعلم عن الألوان! الأحمر والأزرق والأصفر."
        mock_response.usage = Mock()
    ...
    @pytest.mark.asyncio
    async def test_generate_openai_response_story(self, generator_with_openai, mock_openai_client):
        """Test OpenAI response generation for story content."""
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message = Mock()
        mock_response.choices[0].message.content = "كان يا ما كان، أرنب صغير يحب الجزر..."
        mock_response.usage = Mock()
    ...
    @pytest.mark.asyncio
    async def test_generate_openai_response_api_error(self, generator_with_openai, mock_openai_client):
    ...
    @pytest.mark.asyncio
    async def test_generate_fallback_response_keyword_based(self, generator_with_openai):
    ...
    async def test_filter_response_safe_content(self, generator_with_openai):
        ...
        result = await generator_with_openai._filter_response(safe_response, SafetyLevel.STRICT)
    ...
    async def test_filter_response_unsafe_content(self, generator_with_openai):
        ...
        result = await generator_with_openai._filter_response(unsafe_response, SafetyLevel.STRICT)
    ...
    async def test_filter_response_length_truncation(self, generator_with_openai):
        ...
        result = await generator_with_openai._filter_response(long_response, SafetyLevel.MODERATE)
    ...
    async def test_filter_response_complexity_check(self, generator_with_openai):
        ...
        result = await generator_with_openai._filter_response(complex_response, SafetyLevel.STRICT)
    ...
    @pytest.mark.asyncio
    async def test_response_generation_different_types(self, generator_with_openai, mock_openai_client):
    ...
    @pytest.mark.asyncio
    async def test_response_generation_different_safety_levels(self, generator_with_openai, mock_openai_client):
        ...
        for age, expected_safety_level in age_safety_pairs:
            result = await generator_with_openai.generate_response(
                text="test input",
                child_context={"age": age, "name": "test"}
            )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_response_generation(self, generator_with_openai, mock_openai_client):
    ...
    @pytest.mark.asyncio
    async def test_processing_time_tracking(self, generator_with_openai, mock_openai_client):
    ...
    async def test_unsafe_patterns_detection(self, generator_with_openai):
        ...
        for unsafe_input in unsafe_inputs:
            result = await generator_with_openai._filter_input(unsafe_input, SafetyLevel.STRICT)
            assert result["safe"] is False
            assert "[محذوف]" in result["text"]
    ...
    def test_safety_system_prompts_configuration(self, generator_with_openai):
        ...
        moderate_prompt = generator_with_openai.safety_system_prompts[SafetyLevel.MODERATE]
        assert "7-10" in moderate_prompt
        
        relaxed_prompt = generator_with_openai.safety_system_prompts[SafetyLevel.RELAXED]
    ...
    def test_fallback_responses_configuration(self, generator_with_openai):
        ...
        for safety_level in [SafetyLevel.STRICT, SafetyLevel.MODERATE, SafetyLevel.RELAXED]:
