...
from src.application.services.ai.modules.transcription_service import TranscriptionService
...
class TestTranscriptionService:
    ...
    def service_with_engines(self):
        ...
        with patch('src.application.services.ai.modules.transcription_service.SPEECH_RECOGNITION_AVAILABLE', True):
            with patch('src.application.services.ai.modules.transcription_service.AUDIO_PROCESSING_AVAILABLE', True):
                with patch('src.application.services.ai.modules.transcription_service.whisper') as mock_whisper:
                    with patch('src.application.services.ai.modules.transcription_service.sr') as mock_sr:
                        mock_whisper.load_model.return_value = Mock()
                        mock_sr.Recognizer.return_value = Mock()
                        
                        service = TranscriptionService(model_size="base", language_default="ar")
                        return service
    
    @pytest.fixture
    def service_without_engines(self):
        """Create transcription service without speech recognition engines."""
        with patch('src.application.services.ai.modules.transcription_service.SPEECH_RECOGNITION_AVAILABLE', False):
            with patch('src.application.services.ai.modules.transcription_service.AUDIO_PROCESSING_AVAILABLE', False):
    ...
    def test_initialization_without_engines(self, service_without_engines):
        ...
        assert service_without_engines.supported_languages == ["ar", "en", "fr", "es"]
    ...
    @pytest.mark.asyncio
    async def test_transcribe_audio_unsupported_language(self, service_with_engines, sample_audio_data):
        """Test transcription with unsupported language."""
        with patch.object(service_with_engines, '_validate_audio_file') as mock_validate:
            mock_validate.return_value = {"valid": True, "duration": 5.0, "error": None}
            
            with patch.object(service_with_engines, '_perform_transcription') as mock_transcribe:
                mock_transcribe.return_value = {"text": "transcribed text", "confidence": 0.8, "engine": "whisper"}
                
                with patch.object(service_with_engines, '_apply_safety_filters') as mock_filter:
                    mock_filter.return_value = {"text": "transcribed text", "safe": True, "warnings": []}
                    ...
                    assert result["language"] == "ar"  # Should default to Arabic
    
    @pytest.mark.asyncio
    async def test_transcribe_audio_success_with_whisper(self, service_with_engines, sample_audio_data):
    ...
    @pytest.mark.asyncio
    async def test_transcribe_audio_with_safety_filtering(self, service_with_engines, sample_audio_data):
        """Test transcription with safety filtering applied."""
        with patch.object(service_with_engines, '_validate_audio_file') as mock_validate:
            mock_validate.return_value = {"valid": True, "duration": 3.0, "error": None}
    ...
    async def test_validate_audio_file_valid(self, service_with_engines):
        ...
        with patch('src.application.services.ai.modules.transcription_service.AUDIO_PROCESSING_AVAILABLE', True):
            with patch('src.application.services.ai.modules.transcription_service.wave') as mock_wave:
                mock_wav_file = Mock()
    ...
    async def test_validate_audio_file_too_long(self, service_with_engines):
        ...
        with patch('src.application.services.ai.modules.transcription_service.AUDIO_PROCESSING_AVAILABLE', True):
            with patch('src.application.services.ai.modules.transcription_service.wave') as mock_wave:
                mock_wav_file = Mock()
                mock_wav_file.getnframes.return_value = 44100 * 400  # 400 seconds (too long)
                mock_wav_file.getframerate.return_value = 44100
                mock_wav_file.getnchannels.return_value = 1
                mock_wave.open.return_value.__enter__.return_value = mock_wav_file
    ...
    @pytest.mark.asyncio
    async def test_validate_audio_file_processing_unavailable(self, service_with_engines):
    ...
    @pytest.mark.asyncio
    async def test_perform_transcription_whisper_failure_google_fallback(self, service_with_engines):
        """Test transcription with Whisper failure and Google fallback."""
        # Mock Whisper failure
        service_with_engines.whisper_model.transcribe.side_effect = Exception("Whisper error")
    ...
    async def test_apply_safety_filters_unsafe_content(self, service_with_engines):
        ...
        assert len(result["warnings"]) > 0
        assert any("unsafe content detected" in warning.lower() for warning in result["warnings"])
    ...
    @pytest.mark.asyncio
    async def test_transcribe_audio_file_cleanup(self, service_with_engines, sample_audio_data):
    ...
    async def test_concurrent_transcription_processing(self, service_with_engines):
        ...
        with patch.object(service_with_engines, '_validate_audio_file') as mock_validate:
            mock_validate.return_value = {"valid": True, "duration": 2.0, "error": None}
            
            with patch.object(service_with_engines, '_perform_transcription') as mock_transcribe:
                mock_transcribe.side_effect = [
                    {"text": "first transcription", "confidence": 0.9, "engine": "whisper"},
                    {"text": "second transcription", "confidence": 0.8, "engine": "whisper"},
                    {"text": "third transcription", "confidence": 0.85, "engine": "whisper"}
                ]
                
                with patch.object(service_with_engines, '_apply_safety_filters') as mock_filter:
                    mock_filter.side_effect = [
                        {"text": "first transcription", "safe": True, "warnings": []},
                        {"text": "second transcription", "safe": True, "warnings": []},
                        {"text": "third transcription", "safe": True, "warnings": []}
                    ]
                    
                    # Process multiple transcriptions concurrently
                    tasks = [
                        service_with_engines.transcribe_audio(audio, child_id=f"child_{i}")
    ...
    @pytest.mark.asyncio
    async def test_transcription_error_handling(self, service_with_engines, sample_audio_data):
    ...
    @pytest.mark.asyncio
    async def test_transcription_without_child_id(self, service_with_engines, sample_audio_data):
        """Test transcription without child ID (should still work)."""
        with patch.object(service_with_engines, '_validate_audio_file') as mock_validate:
            mock_validate.return_value = {"valid": True, "duration": 3.0, "error": None}
            
            with patch.object(service_with_engines, '_perform_transcription') as mock_transcribe:
                mock_transcribe.return_value = {"text": "test transcription", "confidence": 0.8, "engine": "fallback"}
                
                with patch.object(service_with_engines, '_apply_safety_filters') as mock_filter:
                    mock_filter.return_value = {"text": "test transcription", "safe": True, "warnings": []}
                    
                    result = await service_with_engines.transcribe_audio(sample_audio_data)
