...
class TestConsentService:
    ...
    async def test_request_consent_custom_expiry(self, service):
        ...
        result = await service.request_consent(parent_id, child_id, feature, expiry_days=custom_days)
    ...
    async def test_grant_consent_not_found(self, service):
        ...
        result = await service.grant_consent("nonexistent_consent", "email_verification")
    ...
    async def test_check_consent_status_expired(self, service):
        ...
        result = await service.request_consent(parent_id, child_id, feature, expiry_days=-1)
        consent_id = result["consent_id"]
    ...
    async def test_verify_parental_consent_valid(self, service):
        ...
        # Verify consent
        result = await service.verify_parental_consent(parent_id, child_id, consent_type)
    ...
    async def test_verify_parental_consent_not_found(self, service):
        ...
        result = await service.verify_parental_consent("nonexistent_parent", "nonexistent_child", "feature")
    ...
    async def test_initiate_email_verification(self, service):
        ...
        with patch.object(service.verification_service, 'send_email_verification') as mock_send:
            mock_send.return_value = {"status": "success", "attempt_id": "test_attempt"}
    ...
    async def test_initiate_sms_verification(self, service):
        ...
        with patch.object(service.verification_service, 'send_sms_verification') as mock_send:
            mock_send.return_value = {"status": "success", "attempt_id": "sms_attempt"}
    ...
    async def test_complete_verification(self, service):
        ...
        with patch.object(service.verification_service, 'verify_code') as mock_verify:
            mock_verify.return_value = {"status": "success", "message": "Verification successful"}
    ...
    async def test_consent_lifecycle_complete_workflow(self, service):
        ...
        grant_result = await service.grant_consent(consent_id, "video_call_verification")
        assert grant_result["status"] == "granted"
        
        # Step 4: Verify consent is active
        verify_result = await service.verify_parental_consent(parent_id, child_id, feature)
    ...
    async def test_consent_revocation_workflow(self, service):
        ...
        verify_before = await service.verify_parental_consent(parent_id, child_id, feature)
        ...
        verify_after = await service.verify_parental_consent(parent_id, child_id, feature)
    ...
    async def test_multiple_consents_same_parent_child(self, service):
        ...
        for feature in features:
            ...
            await service.grant_consent(result["consent_id"], "government_id_verification")
    ...
    async def test_verification_integration_flow(self, service):
        ...
        with patch.object(service.verification_service, 'send_email_verification') as mock_email:
            with patch.object(service.verification_service, 'verify_code') as mock_verify:
                mock_email.return_value = {"status": "success", "attempt_id": "integration_attempt"}
                mock_verify.return_value = {"status": "success", "message": "Verification successful"}
                ...
                email_result = await service.initiate_email_verification(consent_id, email)
                assert email_result["status"] == "success"
                
                # Complete verification
                verify_result = await service.complete_verification("integration_attempt", "123456")
                assert verify_result["status"] == "success"
                
                # Grant consent
                grant_result = await service.grant_consent(consent_id, "email_verification")
                assert grant_result["status"] == "granted"
    
    @pytest.mark.asyncio
    async def test_logging_behavior(self, service):
        """Test that consent service logs appropriately."""
        with patch('src.application.services.consent.consent_service.logger') as mock_logger:
    ...
    async def test_coppa_compliance_requirements(self, service):
        ...
        audit_trail = service.get_consent_audit_trail(consent_id)
        assert all(key in audit_trail for key in [
            "consent_id", "parent_id", "child_id", "feature", "status",
            "requested_at", "granted_at", "verification_method", "expiry_date"
        ])
    ...
    async def test_expiry_date_calculation(self, service):
        ...
        result = await service.request_consent(parent_id, child_id, feature, expiry_days)
