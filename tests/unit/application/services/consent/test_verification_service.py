...
class TestVerificationService:
    ...
    async def test_send_email_verification_success(self, service):
        ...
        with patch.object(service, '_validate_email', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="123456"):
                with patch('src.application.services.consent.verification_service.secrets.token_urlsafe', return_value="random_token"):
    ...
    async def test_send_sms_verification_success(self, service):
        ...
        with patch.object(service, '_validate_phone', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="654321"):
                with patch('src.application.services.consent.verification_service.secrets.token_urlsafe', return_value="sms_token"):
    ...
    async def test_verify_code_success(self, service):
        ...
        with patch.object(service, '_validate_email', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="999888"):
                with patch('src.application.services.consent.verification_service.secrets.token_urlsafe', return_value="verify_token"):
                    email_result = await service.send_email_verification(email, consent_id)
    ...
    async def test_verify_code_wrong_code(self, service):
        ...
        with patch.object(service, '_validate_email', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="correct_code"):
                with patch('src.application.services.consent.verification_service.secrets.token_urlsafe', return_value="wrong_token"):
                    email_result = await service.send_email_verification(email, consent_id)
    ...
    def test_generate_verification_code_format(self, service):
        ...
        with patch('src.application.services.consent.verification_service.secrets.randbelow', return_value=123456):
    ...
    def test_generate_verification_code_zero_padding(self, service):
        ...
        with patch('src.application.services.consent.verification_service.secrets.randbelow', return_value=123):
            code = service._generate_verification_code()
            assert code == "000123"
            assert len(code) == 6
    ...
    async def test_multiple_verification_attempts_same_consent(self, service):
        ...
        with patch.object(service, '_validate_email', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="111111"):
                email_result = await service.send_email_verification("email@test.com", consent_id)
        
        # Send SMS verification for same consent
        with patch.object(service, '_validate_phone', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="222222"):
                sms_result = await service.send_sms_verification("+1234567890", consent_id)
    ...
    async def test_verification_attempt_lifecycle(self, service):
        ...
        with patch.object(service, '_validate_email', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="555666"):
                result = await service.send_email_verification(email, consent_id)
    ...
    async def test_verification_attempt_failure_lifecycle(self, service):
        ...
        # Step 1: Initiate verification
        with patch.object(service, '_validate_email', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="correct123"):
                result = await service.send_email_verification(email, consent_id)
    ...
    async def test_concurrent_verification_operations(self, service):
        ...
        with patch.object(service, '_validate_email', return_value=True):
            with patch.object(service, '_generate_verification_code', side_effect=["111111", "222222", "333333"]):
    ...
    async def test_attempt_id_generation_pattern(self, service):
        ...
        with patch.object(service, '_validate_email', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="123456"):
                with patch('src.application.services.consent.verification_service.secrets.token_urlsafe', return_value="test_token"):
                    result = await service.send_email_verification("test@example.com", "consent_pattern")
    ...
    async def test_sms_attempt_id_generation_pattern(self, service):
        ...
        with patch.object(service, '_validate_phone', return_value=True):
            with patch.object(service, '_generate_verification_code', return_value="123456"):
                with patch('src.application.services.consent.verification_service.secrets.token_urlsafe', return_value="sms_token"):
                    result = await service.send_sms_verification("+1234567890", "consent_sms_pattern")
    ...
    async def test_logging_behavior(self, service):
        ...
        with patch('src.application.services.consent.verification_service.logger') as mock_logger:
            # Test email verification logging
            with patch.object(service, '_validate_email', return_value=True):
                with patch.object(service, '_generate_verification_code', return_value="123456"):
                    await service.send_email_verification("test@example.com", "consent_log")
    ...
    async def test_logging_successful_verification(self, service):
        ...
        with patch('src.application.services.consent.verification_service.logger') as mock_logger:
            ...
            with patch.object(service, '_validate_email', return_value=True):
                with patch.object(service, '_generate_verification_code', return_value="success123"):
                    result = await service.send_email_verification("success@test.com", "consent_success")
    ...
    async def test_logging_failed_verification(self, service):
        ...
        with patch('src.application.services.consent.verification_service.logger') as mock_logger:
            # Create verification attempt
            with patch.object(service, '_validate_email', return_value=True):
                with patch.object(service, '_generate_verification_code', return_value="fail123"):
                    result = await service.send_email_verification("fail@test.com", "consent_fail")
            ...
            with patch('src.application.services.consent.verification_service.logger') as mock_logger:
                with patch.object(service, '_validate_phone', return_value=True):
                    with patch.object(service, '_generate_verification_code', return_value="123456"):
                        ...
                        asyncio.run(service.send_sms_verification(phone, "consent_mask"))
    ...
    def test_email_masking_in_logs(self, service):
        ...
        for email, expected_mask in test_cases:
            with patch('src.application.services.consent.verification_service.logger') as mock_logger:
                with patch.object(service, '_validate_email', return_value=True):
                    with patch.object(service, '_generate_verification_code', return_value="123456"):
                        asyncio.run(service.send_email_verification(email, "consent_mask"))
