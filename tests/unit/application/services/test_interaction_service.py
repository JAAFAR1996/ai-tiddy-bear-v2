...
class TestInteractionService:
    ...
    async def test_process_valid_interaction(self, service):
        ...
        with patch.object(service, '_check_content_safety') as mock_safety:
            ...
            with patch('src.application.services.interaction_service.datetime') as mock_datetime:
    ...
    async def test_process_unsafe_content(self, service):
        ...
        with patch.object(service, '_check_content_safety') as mock_safety:
            mock_safety.return_value = {"safe": False, "reason": "Contains inappropriate language"}
            
            with patch('src.application.services.interaction_service.logger') as mock_logger:
                with patch('src.application.services.interaction_service.datetime') as mock_datetime:
                    mock_datetime.utcnow.return_value = datetime(2024, 1, 15, 10, 30, 0)
    ...
    async def test_concurrent_interaction_processing(self, service):
        ...
        with patch.object(service, '_check_content_safety') as mock_safety:
            ...
            tasks = [service.process(child_id, message) for child_id, message in interactions]
    ...
    async def test_timestamp_consistency(self, service):
        ...
        with patch.object(service, '_check_content_safety') as mock_safety:
            ...
            with patch('src.application.services.interaction_service.datetime') as mock_datetime:
                mock_datetime.utcnow.return_value = datetime(2024, 3, 15, 14, 30, 45, 123456)
