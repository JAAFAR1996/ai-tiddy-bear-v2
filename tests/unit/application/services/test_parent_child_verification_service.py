...
class TestParentChildVerificationService:
    ...
    async def test_verification_service_basic_functionality(self):
        ...
        assert request.status in [RelationshipStatus.PENDING, RelationshipStatus.VERIFIED]
    ...
    async def test_error_handling_verification(self):
        ...
        status = await service.get_verification_status("nonexistent_parent", "nonexistent_child")
    ...
    async def test_service_integration(self):
        ...
        child_relationship = next(
            (rel for rel in relationships if rel.child_id == child_id), 
            None
        )
