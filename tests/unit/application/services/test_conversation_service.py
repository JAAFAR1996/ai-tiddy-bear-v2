...
class TestConversationService:
    ...
    @pytest.mark.asyncio
    async def test_start_new_conversation_success(self, service, mock_conversation_repo, child_id):
        """Test successfully starting a new conversation."""
        # Arrange
        initial_text = "Hello, how are you today?"
        
        with patch('src.domain.entities.conversation.Conversation.create_new') as mock_create:
            ...
            mock_conversation.update_summary.assert_called_once_with(initial_text)
            mock_conversation_repo.save.assert_called_once_with(mock_conversation)
    
    @pytest.mark.asyncio
    async def test_start_new_conversation_empty_text(self, service, mock_conversation_repo, child_id):
    ...
    @pytest.mark.asyncio
    async def test_start_new_conversation_repository_error(self, service, mock_conversation_repo, child_id):
    ...
    @pytest.mark.asyncio
    async def test_get_conversation_history_success(self, service, mock_conversation_repo, child_id):
        ...
        assert len(result) == 3
        mock_conversation_repo.find_by_child_id.assert_called_once_with(child_id)
    
    @pytest.mark.asyncio
    async def test_get_conversation_history_empty(self, service, mock_conversation_repo, child_id):
    ...
    @pytest.mark.asyncio
    async def test_get_conversation_by_id_success(self, service, mock_conversation_repo, conversation_id):
        ...
        # Assert
        assert result == mock_conversation
        mock_conversation_repo.get_by_id.assert_called_once_with(conversation_id)
    
    @pytest.mark.asyncio
    async def test_get_conversation_by_id_not_found(self, service, mock_conversation_repo, conversation_id):
    ...
    @pytest.mark.asyncio
    async def test_update_conversation_analysis_success(self, service, mock_conversation_repo, conversation_id):
        ...
        # Assert
        assert result == mock_conversation
        mock_conversation.update_analysis.assert_called_once_with(emotion_analysis, sentiment_score)
        mock_conversation_repo.save.assert_called_once_with(mock_conversation)
    
    @pytest.mark.asyncio
    async def test_update_conversation_analysis_invalid_id(self, service, mock_conversation_repo, conversation_id):
    ...
    @pytest.mark.asyncio
    async def test_update_conversation_analysis_edge_scores(self, service, mock_conversation_repo, conversation_id):
        ...
        for emotion_analysis, sentiment_score in test_cases:
            ...
            # Assert
            mock_conversation.update_analysis.assert_called_with(emotion_analysis, sentiment_score)
    
    @pytest.mark.asyncio
    async def test_update_conversation_summary_success(self, service, mock_conversation_repo, conversation_id):
    ...
    @pytest.mark.asyncio
    async def test_update_conversation_summary_empty(self, service, mock_conversation_repo, conversation_id):
    ...
    @pytest.mark.asyncio
    async def test_add_interaction_new_conversation(self, service, mock_conversation_repo, child_id):
        """Test adding interaction when no conversation exists."""
        # Arrange
        user_input = "Tell me a story"
        ai_response = "Once upon a time..."
        
        mock_conversation_repo.find_by_child_id.return_value = []  # No existing conversations
        ...
        with patch.object(service, 'start_new_conversation') as mock_start_new:
            ...
            mock_conversation.add_interaction.assert_called_once_with(user_input, ai_response)
            mock_conversation_repo.save.assert_called_once_with(mock_conversation)
    
    @pytest.mark.asyncio
    async def test_add_interaction_existing_conversation(self, service, mock_conversation_repo, child_id):
    ...
    @pytest.mark.asyncio
    async def test_add_interaction_multiple_conversations(self, service, mock_conversation_repo, child_id):
        ...
        # Assert
        assert result == mock_conversations[0]  # Should use the first conversation
        mock_conversations[0].add_interaction.assert_called_once_with(user_input, ai_response)
        mock_conversation_repo.save.assert_called_once_with(mock_conversations[0])
    
    @pytest.mark.asyncio
    async def test_add_interaction_special_characters(self, service, mock_conversation_repo, child_id):
    ...
    @pytest.mark.asyncio
    async def test_add_interaction_long_text(self, service, mock_conversation_repo, child_id):
    ...
    @pytest.mark.asyncio
    async def test_repository_error_handling(self, service, mock_conversation_repo, child_id):
        """Test handling of repository errors in various methods."""
        # Test save error in add_interaction
        mock_conversation = Mock(spec=Conversation)
        mock_conversation.add_interaction = Mock()
        mock_conversation_repo.find_by_child_id.return_value = [mock_conversation]
        mock_conversation_repo.save.side_effect = Exception("Database connection lost")
        
        with pytest.raises(Exception, match="Database connection lost"):
            await service.add_interaction(child_id, "test", "response")
    
    @pytest.mark.asyncio
    async def test_concurrent_operations(self, service, mock_conversation_repo, child_id):
    ...
    @pytest.mark.parametrize("child_id_value", [
        uuid4(),
    ...
    @pytest.mark.asyncio
    async def test_various_uuid_formats(self, service, mock_conversation_repo, child_id_value):
        ...
        assert result == []
        mock_conversation_repo.find_by_child_id.assert_called_once_with(child_id_value)