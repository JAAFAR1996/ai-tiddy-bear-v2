...
from src.application.services.advanced_personalization_service import AdvancedPersonalizationService
from src.domain.value_objects.personality import ChildPersonality, PersonalityType
from src.domain.interfaces.personality_profile_repository import IPersonalityProfileRepository
...
class MockAIProvider:
    ...
    async def analyze_personality(self, interactions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def generate_personalized_content(self, child_id: UUID, profile_dict: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
...
def personalization_service(mock_repository, mock_ai_provider, mock_logger):
    ...
    return AdvancedPersonalizationService(mock_repository, mock_ai_provider, mock_logger)
...
def sample_interactions():
    ...
    return [
        {"text": "I love animals!", "sentiment": "positive", "timestamp": "2024-01-01T10:00:00Z"},
        {"text": "Can you tell me about dinosaurs?", "sentiment": "curious", "timestamp": "2024-01-01T10:05:00Z"},
        {"text": "That's so cool!", "sentiment": "excited", "timestamp": "2024-01-01T10:10:00Z"}
    ]
...
class TestAdvancedPersonalizationService:
    ...
    def test_init_sets_dependencies(self, mock_repository, mock_ai_provider, mock_logger):
        """Test that constructor properly sets dependencies."""
        service = AdvancedPersonalizationService(mock_repository, mock_ai_provider, mock_logger)
    ...
    def test_init_with_default_logger(self, mock_repository, mock_ai_provider):
        ...
        with patch('src.application.services.advanced_personalization_service.logger') as mock_default_logger:
            service = AdvancedPersonalizationService(mock_repository, mock_ai_provider)
            assert service.logger is mock_default_logger

    @pytest.mark.asyncio
    async def test_create_personality_profile_success(self, personalization_service, sample_child_id, sample_interactions):
        """Test successful creation of personality profile."""
        # Act
        result = await personalization_service.create_personality_profile(sample_child_id, sample_interactions)
    ...
    @pytest.mark.asyncio
    async def test_create_personality_profile_logging(self, personalization_service, sample_child_id, sample_interactions):
        """Test that profile creation is logged correctly."""
        # Act
        await personalization_service.create_personality_profile(sample_child_id, sample_interactions)
        
        # Assert
        assert personalization_service.logger.info.call_count == 3  # 2 from create + 1 from analyze
        
        # Check first log call (creating profile)
        first_call = personalization_service.logger.info.call_args_list[0]
        assert f"Creating personality profile for child: {sample_child_id}" in first_call[0][0]
        
        # Check last log call (profile created)
        last_call = personalization_service.logger.info.call_args_list[-1]
        assert f"Personality profile created and saved for child: {sample_child_id}" in last_call[0][0]

    @pytest.mark.asyncio
    async def test_create_personality_profile_with_empty_interactions(self, personalization_service, sample_child_id):
        """Test creating personality profile with empty interactions."""
        # Act
        result = await personalization_service.create_personality_profile(sample_child_id, [])
    ...
    @pytest.mark.asyncio
    async def test_create_personality_profile_ai_error_fallback(self, personalization_service, sample_child_id, sample_interactions):
    ...
    @pytest.mark.asyncio
    async def test_create_personality_profile_repository_error(self, personalization_service, sample_child_id, sample_interactions):
    ...
    @pytest.mark.asyncio
    async def test_get_personality_profile_found(self, personalization_service, sample_child_id, sample_interactions):
        """Test successful retrieval of existing personality profile."""
        # Arrange - Create profile first
        created_profile = await personalization_service.create_personality_profile(sample_child_id, sample_interactions)
    ...
    @pytest.mark.asyncio
    async def test_get_personality_profile_not_found(self, personalization_service, sample_child_id):
    ...
    @pytest.mark.asyncio
    async def test_get_personality_profile_logging_found(self, personalization_service, sample_child_id, sample_interactions):
        ...
        assert f"Attempting to retrieve personality profile for child: {sample_child_id}" in debug_call
    ...
    @pytest.mark.asyncio
    async def test_get_personality_profile_logging_not_found(self, personalization_service, sample_child_id):
        ...
        personalization_service.logger.info.assert_called_once()
        info_call = personalization_service.logger.info.call_args[0][0]
        assert f"Personality profile not found for child: {sample_child_id}" in info_call

    @pytest.mark.asyncio
    async def test_get_personalized_content_success(self, personalization_service, sample_child_id, sample_interactions):
    ...
    @pytest.mark.asyncio
    async def test_get_personalized_content_no_profile(self, personalization_service, sample_child_id):
    ...
    @pytest.mark.asyncio
    async def test_get_personalized_content_ai_error_fallback(self, personalization_service, sample_child_id, sample_interactions):
        """Test fallback behavior when AI content generation fails."""
        # Arrange
        await personalization_service.create_personality_profile(sample_child_id, sample_interactions)
        personalization_service.ai_provider.should_raise_exception = True
        personalization_service.ai_provider.exception_message = "AI content generation failed"
        ...
        personalization_service.logger.error.assert_called()
        personalization_service.logger.warning.assert_called_with("Falling back to generic content recommendations.")

    @pytest.mark.asyncio
    async def test_analyze_interactions_success(self, personalization_service, sample_interactions):
        """Test successful interaction analysis."""
        # Act
        result = await personalization_service._analyze_interactions(sample_interactions)
    ...
    @pytest.mark.asyncio
    async def test_analyze_interactions_invalid_personality_type(self, personalization_service, sample_interactions):
        """Test handling of invalid personality type from AI."""
        # Arrange
        personalization_service.ai_provider.personality_analysis_result["personality_type"] = "INVALID_TYPE"
    ...
    @pytest.mark.asyncio
    async def test_analyze_interactions_missing_fields(self, personalization_service, sample_interactions):
    ...
    @pytest.mark.asyncio
    async def test_analyze_interactions_ai_error_fallback(self, personalization_service, sample_interactions):
        """Test fallback behavior when AI analysis fails."""
        # Arrange
        personalization_service.ai_provider.should_raise_exception = True
    ...
    @pytest.mark.asyncio
    async def test_personality_type_mapping(self, personalization_service, sample_interactions):
        ...
        for ai_type, expected_type in personality_types:
            # Arrange
            personalization_service.ai_provider.personality_analysis_result["personality_type"] = ai_type
    ...
    @pytest.mark.asyncio
    async def test_full_workflow_create_and_get_content(self, personalization_service, sample_child_id, sample_interactions):
        """Test complete workflow of creating profile and getting content."""
        # Create profile
        created_profile = await personalization_service.create_personality_profile(sample_child_id, sample_interactions)
        
        # Get personalized content
        content = await personalization_service.get_personalized_content(sample_child_id)
    ...
    async def test_multiple_profiles_isolation(self, personalization_service):
        ...
        # Act
        profile_1 = await personalization_service.create_personality_profile(child_id_1, interactions_1)
        ...
        profile_2 = await personalization_service.create_personality_profile(child_id_2, interactions_2)
    ...
    @pytest.mark.asyncio
    async def test_child_id_consistency(self, personalization_service, sample_child_id, sample_interactions):
        """Test that child_id is consistently set throughout the process."""
        # Act
        profile = await personalization_service.create_personality_profile(sample_child_id, sample_interactions)
        
        # Assert
        assert profile.child_id == sample_child_id
        
        # Verify it's stored with correct ID
        stored_profile = await personalization_service.get_personality_profile(sample_child_id)
        assert stored_profile.child_id == sample_child_id

    @pytest.mark.asyncio
    async def test_datetime_handling(self, personalization_service, sample_child_id, sample_interactions):
    ...
    @pytest.mark.asyncio
    async def test_metadata_preservation(self, personalization_service, sample_child_id, sample_interactions):
    ...
    @pytest.mark.asyncio
    async def test_content_generation_context(self, personalization_service, sample_child_id, sample_interactions):
        """Test that context is properly passed to content generation."""
        # Arrange
        await personalization_service.create_personality_profile(sample_child_id, sample_interactions)
        
        # Act
        with patch('src.application.services.advanced_personalization_service.datetime') as mock_datetime:
    ...
    @pytest.mark.asyncio
    async def test_error_handling_preserves_state(self, personalization_service, sample_child_id, sample_interactions):
        """Test that errors don't corrupt service state."""
        # Arrange
        personalization_service.ai_provider.should_raise_exception = True
        
        # Act - First call should use fallback
        profile1 = await personalization_service.create_personality_profile(sample_child_id, sample_interactions)
        ...
        profile2 = await personalization_service.create_personality_profile(another_child_id, sample_interactions)
    ...
    def test_personality_type_enum_coverage(self):
        ...
        expected_values = [
            "curious", "creative", "analytical", "empathetic", 
            "energetic", "calm", "other"
        ]
    ...
    def test_service_dependencies_injection(self, mock_repository, mock_ai_provider):
        ...
        # Act
        service = AdvancedPersonalizationService(mock_repository, mock_ai_provider, custom_logger)
