...
class TestFederatedLearningService:
    ...
    def service_no_event_bus(self):
        ...
        with patch('src.application.services.federated_learning_service.get_event_bus') as mock_get_bus:
    ...
    def test_initialization_with_event_bus(self, mock_event_bus):
        ...
        with patch.object(FederatedLearningService, '_initialize_global_model') as mock_init:
    ...
    def test_initialize_global_model(self, service):
        ...
        with patch('src.application.services.federated_learning_service.logger') as mock_logger:
    ...
    async def test_process_multiple_local_updates(self, service):
        ...
        # Verify model has been updated (exact values depend on aggregation order)
    ...
    def test_aggregate_model_update_simple_averaging(self, service):
        ...
        expected_weights = [0.2, 0.3, 0.4]  # [(0.1+0.3)/2, (0.2+0.4)/2, (0.3+0.5)/2]
    ...
    async def test_model_update_logging(self, service):
        ...
        with patch('src.application.services.federated_learning_service.logger') as mock_logger:
            ...
            receive_log = next((msg for msg in log_messages if "Received local model update" in msg), None)
            assert receive_log is not None
            assert device_id in receive_log
            
            # Should log aggregating the update
            aggregate_log = next((msg for msg in log_messages if "Aggregating model update" in msg), None)
    ...
    async def test_different_model_structures(self, service):
        ...
        for update in updates_different_lengths:
            # Should handle gracefully (might raise IndexError or handle differently)
            try:
