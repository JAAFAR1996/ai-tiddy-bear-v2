...
class MockSpeechProcessor:
    ...
    async def text_to_speech(self, text: str, voice_id: str) -> bytes:
        ...
        key = (text, voice_id)
        return self.text_to_speech_results.get(key, f"audio_data_for_{text}_{voice_id}".encode())
...
class TestSpeechProcessor:
    ...
    def test_mock_implementation_conforms_to_protocol(self):
        ...
        # This should not raise a type error if the mock conforms to the protocol
    ...
    async def test_speech_to_text_with_different_languages(self):
        ...
        for language in languages:
            processor.speech_to_text_results[(len(audio_data), language)] = f"Text in {language}"
    ...
    async def test_text_to_speech_with_different_voices(self):
        ...
        voice_ids = ["child_voice_1", "child_voice_2", "adult_voice", "narrator_voice"]
        
        for voice_id in voice_ids:
            processor.text_to_speech_results[(text, voice_id)] = f"audio_for_{voice_id}".encode()
    ...
    async def test_speech_to_text_large_audio_data(self):
        ...
        processor.speech_to_text_results[(len(audio_data), language)] = "Long transcription"
    ...
    async def test_text_to_speech_long_text(self):
        ...
        processor = MockSpeechProcessor()
        text = "This is a very long text message that simulates a story or long conversation. " * 100
    ...
    async def test_speech_to_text_unicode_language_codes(self):
        ...
        for language in unicode_languages:
            processor.speech_to_text_results[(len(audio_data), language)] = f"Unicode text in {language}"
    ...
    async def test_child_safety_voice_filtering(self):
        ...
        for voice_id in child_safe_voices:
            processor.text_to_speech_results[(text, voice_id)] = f"safe_audio_{voice_id}".encode()
    ...
    async def test_language_code_validation(self):
        ...
        for language in valid_languages:
            processor.speech_to_text_results[(len(audio_data), language)] = f"Valid result for {language}"
    ...
    async def test_audio_data_format_handling(self):
        ...
        for audio_data in audio_formats:
            processor.speech_to_text_results[(len(audio_data), language)] = f"Processed {len(audio_data)} bytes"
    ...
    async def test_text_content_safety(self):
        ...
        for text in safe_texts:
            processor.text_to_speech_results[(text, voice_id)] = f"safe_audio_for_{hash(text)}".encode()
    ...
    def test_protocol_structure(self):
        ...
        protocol_methods = [name for name in dir(SpeechProcessor) if not name.startswith('_')]
