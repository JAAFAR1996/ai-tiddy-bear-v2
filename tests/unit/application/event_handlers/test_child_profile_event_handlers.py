...
from src.application.event_handlers.child_profile_event_handlers import ChildProfileEventHandlers
from src.application.interfaces.read_model_interfaces import IChildProfileReadModel, IChildProfileReadModelStore
...
class MockChildProfileReadModel(IChildProfileReadModel):
    ...
    def __init__(self, id: str, name: str, age: int, preferences: Dict[str, Any]):
        self._id = id
        self._name = name
        self._age = age
...
class TestChildProfileEventHandlers:
    ...
    @pytest.mark.asyncio
    @patch('src.application.event_handlers.child_profile_event_handlers.create_child_profile_read_model')
    async def test_handle_child_registered_success(self, mock_create_model, child_profile_event_handlers, sample_child_registered_event):
        ...
        # Act
        await child_profile_event_handlers.handle_child_registered(sample_child_registered_event)
    ...
    async def test_handle_child_registered_logs_success(self, mock_logger, mock_create_model, child_profile_event_handlers, sample_child_registered_event):
    ...
    @patch('src.application.event_handlers.child_profile_event_handlers.create_child_profile_read_model')
    @patch('src.application.event_handlers.child_profile_event_handlers.logger')
    async def test_handle_child_registered_error_handling(self, mock_logger, mock_create_model, child_profile_event_handlers, sample_child_registered_event):
    ...
    @pytest.mark.asyncio
    async def test_handle_child_profile_updated_success(self, child_profile_event_handlers, sample_child_profile_updated_event):
        ...
        child_profile_event_handlers.read_model_store.models[str(sample_child_profile_updated_event.child_id)] = existing_model
        
        # Act
        await child_profile_event_handlers.handle_child_profile_updated(sample_child_profile_updated_event)
    ...
    @pytest.mark.asyncio
    @patch('src.application.event_handlers.child_profile_event_handlers.logger')
    async def test_handle_child_profile_updated_not_found(self, mock_logger, child_profile_event_handlers, sample_child_profile_updated_event):
        """Test handling of profile update when child not found."""
        # Act
        await child_profile_event_handlers.handle_child_profile_updated(sample_child_profile_updated_event)
        
        # Assert
        mock_logger.warning.assert_called_once_with("Child profile not found for update")
        assert not child_profile_event_handlers.read_model_store.save_called

    @pytest.mark.asyncio
    @patch('src.application.event_handlers.child_profile_event_handlers.logger')
    async def test_handle_child_profile_updated_no_changes(self, mock_logger, child_profile_event_handlers):
        ...
        child_profile_event_handlers.read_model_store.models[str(child_id)] = existing_model
        ...
        # Assert
        mock_logger.debug.assert_called_with("No changes detected, skipping database update")
        assert not child_profile_event_handlers.read_model_store.save_called

    @pytest.mark.asyncio
    @patch('src.application.event_handlers.child_profile_event_handlers.logger')
    async def test_handle_child_profile_updated_partial_changes(self, mock_logger, child_profile_event_handlers):
        ...
        assert existing_model.preferences["favorite_color"] == "green"  # Unchanged
        assert child_profile_event_handlers.read_model_store.save_called

    @pytest.mark.asyncio
    @patch('src.application.event_handlers.child_profile_event_handlers.logger')
    async def test_handle_child_profile_updated_preference_changes(self, mock_logger, child_profile_event_handlers):
        ...
        assert existing_model.preferences["favorite_animal"] == "cat"  # Unchanged
        assert child_profile_event_handlers.read_model_store.save_called

    @pytest.mark.asyncio
    @patch('src.application.event_handlers.child_profile_event_handlers.logger')
    async def test_handle_child_profile_updated_logs_changes(self, mock_logger, child_profile_event_handlers):
    ...
    @pytest.mark.asyncio
    @patch('src.application.event_handlers.child_profile_event_handlers.logger')
    async def test_handle_child_profile_updated_error_handling(self, mock_logger, child_profile_event_handlers, sample_child_profile_updated_event):
        """Test error handling in child profile update."""
        # Arrange
        child_profile_event_handlers.read_model_store.get_by_id = AsyncMock(side_effect=Exception("Database error"))
    ...
    @pytest.mark.asyncio
    async def test_async_save_with_async_store(self, child_profile_event_handlers):
        ...
        # Assert
        child_profile_event_handlers.read_model_store.async_save.assert_called_once_with(mock_model)

    @pytest.mark.asyncio
    @patch('asyncio.get_event_loop')
    async def test_async_save_fallback_to_sync(self, mock_get_loop, child_profile_event_handlers):
        ...
        if hasattr(child_profile_event_handlers.read_model_store, 'async_save'):
            delattr(child_profile_event_handlers.read_model_store, 'async_save')
    ...
    @pytest.mark.asyncio
    async def test_async_get_by_id_with_async_store(self, child_profile_event_handlers):
        """Test _async_get_by_id method with async-capable store."""
        # Arrange
        child_id = "test_id"
        mock_model = MockChildProfileReadModel(child_id, "Test", 8, {})
        child_profile_event_handlers.read_model_store.async_get_by_id = AsyncMock(return_value=mock_model)
        
        # Act
        result = await child_profile_event_handlers._async_get_by_id(child_id)
        
        # Assert
        child_profile_event_handlers.read_model_store.async_get_by_id.assert_called_once_with(child_id)
        assert result is mock_model

    @pytest.mark.asyncio
    @patch('asyncio.get_event_loop')
    async def test_async_get_by_id_fallback_to_sync(self, mock_get_loop, child_profile_event_handlers):
        ...
        if hasattr(child_profile_event_handlers.read_model_store, 'async_get_by_id'):
            delattr(child_profile_event_handlers.read_model_store, 'async_get_by_id')
    ...
    @pytest.mark.asyncio
    async def test_async_get_by_id_returns_none_when_not_found(self, child_profile_event_handlers):
        """Test _async_get_by_id returns None when child not found."""
        # Arrange
        child_id = "nonexistent_id"
        child_profile_event_handlers.read_model_store.async_get_by_id = AsyncMock(return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_concurrent_event_handling(self, child_profile_event_handlers):
        ...
        # Act
        with patch('src.application.event_handlers.child_profile_event_handlers.create_child_profile_read_model') as mock_create:
            # Mock the create function to return different models
            mock_create.side_effect = [
                MockChildProfileReadModel(str(event.child_id), event.name, event.age, event.preferences)
                for event in events
            ]
    ...
    @pytest.mark.asyncio
    async def test_event_handling_with_empty_preferences(self, child_profile_event_handlers):
    ...
    @pytest.mark.asyncio
    async def test_event_handling_with_none_preferences(self, child_profile_event_handlers):
        ...
        assert existing_model.preferences["favorite_color"] == "blue"  # Unchanged

    @pytest.mark.asyncio
    async def test_event_handling_performance(self, child_profile_event_handlers):
    ...
    @pytest.mark.asyncio
    async def test_error_propagation(self, child_profile_event_handlers, sample_child_registered_event):
        """Test that errors are properly propagated from async operations."""
        # Arrange
        child_profile_event_handlers.read_model_store.save = AsyncMock(side_effect=Exception("Save failed"))
    ...
    @pytest.mark.asyncio
    async def test_logging_does_not_expose_pii(self, child_profile_event_handlers, sample_child_registered_event):
        """Test that logging does not expose personally identifiable information."""
        # Act
        with patch('src.application.event_handlers.child_profile_event_handlers.create_child_profile_read_model') as mock_create:
            with patch('src.application.event_handlers.child_profile_event_handlers.logger') as mock_logger:
                mock_create.return_value = MockChildProfileReadModel(
                    str(sample_child_registered_event.child_id),
                    sample_child_registered_event.name,
                    sample_child_registered_event.age,
        ...
        assert str(sample_child_registered_event.child_id) not in debug_call_args
