...
class TestEmotionAnalyzer:
    ...
    def test_analyze_text_calm_emotions(self, analyzer):
        ...
        calm_texts = ["I feel calm", "Everything is peaceful", "I'm relaxed"]
    ...
    def test_analyze_voice_calm(self, analyzer):
        ...
        audio_features = {"pitch": 0.5, "energy": 0.5, "tempo": 0.5}
        result = analyzer.analyze_voice(audio_features)
        assert result.primary_emotion == "calm"
        assert result.confidence == 0.7
        assert result.sentiment_score == 0.2
    ...
    def test_analyze_voice_default_values(self, analyzer):
        ...
        audio_features = {"pitch": 0.8}  # Missing energy and tempo
        result = analyzer.analyze_voice(audio_features)
        # Default values are 0.5, so won't trigger excited (needs high energy
        # too)
        assert result.primary_emotion == "calm"
    ...
    def test_emotion_categories_consistency(self, analyzer):
        ...
        positive_negative_overlap = (
            analyzer._positive_emotions & analyzer._negative_emotions
        )
        positive_neutral_overlap = (
            analyzer._positive_emotions & analyzer._neutral_emotions
        )
        negative_neutral_overlap = (
            analyzer._negative_emotions & analyzer._neutral_emotions
        )
    ...
    def test_voice_analysis_edge_values(self, analyzer):
        ...
        audio_features = {"pitch": 1.0, "energy": 1.0, "tempo": 1.0}
        result = analyzer.analyze_voice(audio_features)
        assert result.primary_emotion == "excited"

        # All features at minimum
        audio_features = {"pitch": 0.0, "energy": 0.0, "tempo": 0.0}
        result = analyzer.analyze_voice(audio_features)
        assert result.primary_emotion == "sad"
