...
class TestConsentVerificationRoute:
    ...
    def mock_consent_manager(self):
        ...
        manager = AsyncMock(spec=ConsentManager)
        return manager
    ...
    def mock_request(self):
        ...
        request.state.user = {"user_id": "parent_123", "email": "parent@test.com"}
        return request
    ...
    async def test_extract_child_id_from_request_body(self):
        ...
        request.query_params = {}
        request.method = "POST"
        request.body = AsyncMock(return_value=b'{"child_id": "child_456", "audio_data": "base64data"}')
    ...
    async def test_verify_consent_success(self, mock_consent_manager):
        ...
        with patch('src.presentation.api.middleware.consent_verification.get_consent_manager', 
    ...
    async def test_verify_consent_failure(self, mock_consent_manager):
        ...
        mock_consent_manager.verify_parental_consent = AsyncMock(side_effect=mock_verify)
...
class TestConsentVerificationMiddleware:
    ...
    async def test_middleware_path_matching(self):
        ...
        with patch('src.presentation.api.middleware.consent_verification.Request') as mock_request_class:
            mock_request = MagicMock()
            mock_request.url.path = "/api/v1/process-audio"
            mock_request_class.return_value = mock_request
            
            # Mock route extraction
            with patch.object(ConsentVerificationRoute, '_extract_child_id', 
                            return_value="child_123") as mock_extract:
                with patch.object(ConsentVerificationRoute, '_verify_consent') as mock_verify:
...
class TestRequireConsentDecorator:
    ...
    def test_decorator_adds_consent_types(self):
        ...
        assert test_function._consent_types == ["data_collection", "voice_recording"]
