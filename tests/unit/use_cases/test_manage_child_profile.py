...
class TestManageChildProfileUseCase:
    ...
    async def test_create_child_profile_success(self, use_case):
        ...
        with pytest.mock.patch('domain.entities.child_profile.ChildProfile.create_new', return_value=mock_profile):
            # Execute
    ...
    async def test_get_child_profile_exists(self, use_case, child_read_model):
        ...
        use_case.child_profile_read_model_store.get_by_id.return_value = child_read_model
    ...
    @pytest.mark.asyncio
    async def test_update_child_profile_success(self, use_case, child_profile, child_read_model):
        ...
        # Verify
        assert isinstance(result, ChildData)
        child_profile.update_profile.assert_called_once_with(new_name, new_age, new_preferences)
        use_case.child_repository.save.assert_called_once_with(child_profile)

    @pytest.mark.asyncio
    async def test_update_child_profile_partial_update(self, use_case, child_profile, child_read_model):
    ...
    @pytest.mark.asyncio
    async def test_update_child_profile_publishes_events(self, use_case, child_profile, child_read_model):
