...
class TestAISafety:
    ...
    async def test_age_appropriate_content_filtering(self, ai_service):
        ...
        result = ai_service._is_age_appropriate("This is a scary story about death", ChildAge.TODDLER)
        assert result is False
        
        # Test appropriate content for toddlers
        result = ai_service._is_age_appropriate("Let's play with colorful toys!", ChildAge.TODDLER)
        assert result is True
    
    def test_safety_score_calculation(self, ai_service):
        """Test safety score calculation"""
        # Safe content
        safe_moderation = {"safe": True, "category_scores": {"violence": 0.1, "hate": 0.05}}
        score = ai_service._calculate_safety_score(safe_moderation)
        assert score >= 0.8
        
        # Unsafe content
        unsafe_moderation = {"safe": False, "category_scores": {"violence": 0.9, "hate": 0.8}}
    ...
    def test_inappropriate_terms_detection(self, ai_service):
        ...
        result = ai_service._is_age_appropriate("This is scary and violent", ChildAge.TODDLER)
        assert result is False
        
        # Test school age appropriate content
        result = ai_service._is_age_appropriate("Let's learn about friendship", ChildAge.SCHOOL_AGE)
        assert result is True
...
class TestChildDataEncryption:
    ...
    def test_child_model_encryption(self):
        ...
        # Mock encryption key
        with patch.dict('os.environ', {'COPPA_ENCRYPTION_KEY': 'test_key_32_characters_long_12345'}):
...
class TestRateLimiting:
    ...
    async def test_login_rate_limiting(self):
        ...
        with patch('infrastructure.security.rate_limiter.get_rate_limiter') as mock_limiter:
    ...
    async def test_rate_limiter_mandatory(self):
        ...
        with patch('infrastructure.security.rate_limiter.get_rate_limiter', side_effect=ImportError()):
            request = LoginRequest(email="test@example.com", password="test123")
            
            with pytest.raises(Exception):  # Should raise HTTPException for unavailable service
                await login(request, client_ip="192.168.1.1")
