...
def mock_content_filter_service():
    mock = Mock()
    mock.is_safe_for_children.side_effect = lambda phrase, age: {"safe": "unsafe" not in phrase and "inappropriate" not in phrase}
    return mock
...
def mock_parental_control_service():
    ...
    mock.check_content_access.side_effect = lambda content_type, allowed_types: {"allowed": content_type in allowed_types}
    mock.get_activity_summary.return_value = {"total_time": 120, "interaction_count": 50}
    return mock
...
def mock_security_service():
    ...
    mock.decrypt_sensitive_data.side_effect = lambda data: data.replace("encrypted(", "").replace(")", "")
    return mock
...
def mock_input_validator():
    ...
    mock.validate.side_effect = lambda value, type_: {
        "valid": True if (type_ == "email" and "@" in value) or \
                        (type_ == "username" and "DROP TABLE" not in value) else False
    }
    return mock
...
def test_child_safety_protection(mock_coppa_service, mock_content_filter_service):
    ...
    assert mock_coppa_service.validate_child_age(13)["compliant"] is False # COPPA is under 13
    assert mock_coppa_service.validate_child_age(18)["compliant"] is False

    # Test content filtering
    assert mock_content_filter_service.is_safe_for_children("safe content", 5)["safe"] is True
    assert mock_content_filter_service.is_safe_for_children("unsafe content", 5)["safe"] is False
    assert mock_content_filter_service.is_safe_for_children("inappropriate language", 5)["safe"] is False
...
def test_parental_controls(mock_parental_control_service):
    ...
    assert mock_parental_control_service.check_content_access("educational", ["educational"])["allowed"] is True
    assert mock_parental_control_service.check_content_access("gaming", ["educational"])["allowed"] is False
...
def test_security_features(mock_security_service, mock_auth_service, mock_enhanced_security_service, mock_input_validator):
    ...
    assert mock_input_validator.validate("admin'; DROP TABLE--", "username")["valid"] is False
...
def test_api_endpoints_importability():
    ...
    children_module = importlib.import_module("src.presentation.api.endpoints.children.routes")
    ...
    chat_schemas_module = importlib.import_module("src.presentation.api.schemas.chat_schemas")
    auth_schemas_module = importlib.import_module("src.presentation.api.schemas.auth_schemas")
