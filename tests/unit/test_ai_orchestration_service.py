...
@pytest.fixture
def ai_orchestration_service(mock_ai_provider, mock_safety_monitor, mock_conversation_service, mock_tts_service):
...
def test_child_preferences():
    return ChildPreferences(age=6, interests=["adventure", "fantasy"], language="en", personality_traits=["imaginative"])
...
async def test_get_ai_response_safe_content(
    ...
    response = await ai_orchestration_service.get_ai_response(
        test_child_id, test_conversation_history, test_current_input, test_voice_id, test_child_preferences
    )

    mock_ai_provider.generate_response.assert_called_once_with(
        test_child_id, test_conversation_history, test_current_input, test_child_preferences
    )
