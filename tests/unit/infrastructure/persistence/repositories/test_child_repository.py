...
def child_repository(mock_database, mock_consent_manager):
    ...
    with patch(
        "src.infrastructure.persistence.repositories.child_repository.get_consent_manager",
...
class TestChildRepositoryCreate:
    ...
    async def test_create_child_success(
        self, child_repository, mock_database, mock_consent_manager, sample_child_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_child_no_consent(
        self, child_repository, mock_consent_manager, sample_child_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_child_invalid_age(
        self, child_repository, mock_database, sample_child_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_child_security_violation(
        self, child_repository, mock_database, sample_child_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_child_database_error(
        self, child_repository, mock_database, sample_child_data
    ):
...
class TestChildRepositoryRead:
    ...
    @pytest.mark.asyncio
    async def test_get_children_by_parent_success(
...
class TestChildRepositoryUpdate:
    ...
    @pytest.mark.asyncio
    async def test_update_child_parent_id_ignored(
        self, child_repository, mock_database
    ):
        """Test that parent_id cannot be updated (security feature)."""
        # Arrange
        child_id = str(uuid4())
        updates = {"parent_id": str(uuid4()), "age": 9}  # Should be ignored
    ...
    @pytest.mark.asyncio
    async def test_update_child_security_violation(
        self, child_repository, mock_database
    ):
...
class TestChildRepositoryEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_create_child_age_boundary_cases(
        self, child_repository, mock_database, sample_child_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_child_maximum_preferences(
        self, child_repository, mock_database
    ):
        """Test child creation with large preferences object."""
        # Arrange
        parent_id = str(uuid4())
        large_preferences = {f"pref_{i}": f"value_{i}" for i in range(100)}
        child_data = {
            "name": "TestChild",
            "age": 8,
    ...
    @pytest.mark.asyncio
    async def test_get_children_by_parent_database_error(
        self, child_repository, mock_database
    ):
