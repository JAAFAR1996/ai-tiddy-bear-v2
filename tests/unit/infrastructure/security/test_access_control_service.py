...
class TestAccessControlService:
    ...
    async def test_verify_access_expired_relationship(self, service):
        ...
        await service.register_parent_child_relationship(
            parent_id, child_id, AccessLevel.FULL_PARENT, "sms_verification",
    ...
    async def test_access_token_validation_expired(self, service):
        ...
        result = await service.validate_access_token(token_id, parent_id, child_id, action)
    ...
    async def test_access_token_validation_mismatch(self, service):
        ...
        result = await service.validate_access_token(token_id, parent_id, child_id, wrong_action)
    ...
    async def test_one_time_token_usage(self, service):
        ...
        result1 = await service.validate_access_token(token_id, parent_id, child_id, action)
        assert result1["valid"] is True
        
        # Second validation should fail (token used)
        result2 = await service.validate_access_token(token_id, parent_id, child_id, action)
    ...
    async def test_audit_logging(self, service):
        ...
        await service.register_parent_child_relationship(
            parent_id, child_id, AccessLevel.SHARED_PARENT, "email_verification"
        )
    ...
    async def test_suspicious_activity_detection(self, service):
        ...
        with patch('src.infrastructure.security.access_control_service.logger') as mock_logger:
    ...
    async def test_get_parent_children_excludes_expired(self, service):
        ...
        expired_date = datetime.utcnow() - timedelta(days=1)
        await service.register_parent_child_relationship(
            parent_id, "expired_child", AccessLevel.FULL_PARENT, "government_id",
            expiry_date=expired_date
        )
    ...
    async def test_different_access_levels_permissions(self, service):
        ...
        test_cases = [
            ...
            (AccessLevel.TEMPORARY_GUARDIAN, AccessAction.GRANT_CONSENT, False),
            (AccessLevel.TEMPORARY_GUARDIAN, AccessAction.READ_CONVERSATIONS, True),
            (AccessLevel.READ_ONLY, AccessAction.UPDATE_PROFILE, False),
            (AccessLevel.READ_ONLY, AccessAction.READ_ANALYTICS, True),
            (AccessLevel.EMERGENCY_CONTACT, AccessAction.READ_ANALYTICS, False),
    ...
    async def test_multiple_parents_same_child(self, service):
        ...
        result1 = await service.verify_access(parent1_id, child_id, AccessAction.READ_PROFILE)
        result2 = await service.verify_access(parent2_id, child_id, AccessAction.READ_PROFILE)
        
        assert result1["access_granted"] is True
        assert result2["access_granted"] is True
        
        # Only primary parent should be able to delete
        delete_result1 = await service.verify_access(parent1_id, child_id, AccessAction.DELETE_PROFILE)
        delete_result2 = await service.verify_access(parent2_id, child_id, AccessAction.DELETE_PROFILE)
    ...
    async def test_coppa_compliance_audit_trail(self, service):
        ...
        reg_result = await service.register_parent_child_relationship(
            parent_id, child_id, AccessLevel.FULL_PARENT, "government_id_verification",
        ...
        relevant_audits = [
            log for log in service.audit_logs
        ...
        for audit in relevant_audits:
            assert all(key in audit for key in [
                "event_id", "timestamp", "parent_id", "child_id", 
                "action", "access_level", "success", "details"
            ])
    ...
    async def test_relationship_status_management(self, service):
        ...
        result = await service.verify_access(parent_id, child_id, AccessAction.READ_PROFILE)
    ...
    async def test_error_handling_system_errors(self, service):
        ...
        with patch.object(service, '_find_relationship', side_effect=Exception("System error")):
            result = await service.verify_access(parent_id, child_id, action)
    ...
    def test_edge_cases_invalid_inputs(self, service):
        ...
        with pytest.raises((TypeError, AttributeError)):
            asyncio.run(service.register_parent_child_relationship(
