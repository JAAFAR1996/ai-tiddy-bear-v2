...
from src.infrastructure.security.comprehensive_input_validation_middleware import (
...
class TestSecurityThreat:
    ...
    def test_security_threat_detection_time(self):
        ...
        with patch('src.infrastructure.security.input_validation.core.datetime') as mock_dt:
...
class TestInputValidationResult:
    ...
    def test_input_validation_result_critical_threat_detection(self):
        ...
        critical_threat = SecurityThreat("command_injection", "critical", "cmd", "rm -rf /", "Critical threat")
        high_threat = SecurityThreat("path_traversal", "high", "path", "../../../etc/passwd", "High threat")
        
        result = InputValidationResult(
            is_valid=False,
            threats=[critical_threat, high_threat]
        )
    ...
    def test_input_validation_result_no_critical_threats(self):
        ...
        medium_threat = SecurityThreat("suspicious_input", "medium", "field", "value", "Medium threat")
        low_threat = SecurityThreat("minor_issue", "low", "field", "value", "Low threat")
        
        result = InputValidationResult(
            is_valid=True,
            threats=[medium_threat, low_threat]
        )
...
class TestComprehensiveInputValidator:
    ...
    def mock_audit_integration(self):
        ...
        with patch('src.infrastructure.security.input_validation.validator.get_audit_integration', return_value=mock_audit):
            yield mock_audit
    ...
    async def test_validate_input_oversized_input(self, validator):
        ...
        oversized_threats = [t for t in result.threats if t.threat_type == "oversized_input"]
    ...
    async def test_validate_input_exception_handling(self, validator):
        ...
        with patch.object(validator, 'detect_sql_injection', side_effect=Exception("Detection error")):
    ...
    async def test_validate_input_mock_threat_detection(self, validator):
        ...
        mock_threat = SecurityThreat("sql_injection", "high", "field", "value", "SQL injection detected")
        
        with patch.object(validator, 'detect_sql_injection', return_value=[mock_threat]):
            with patch.object(validator, 'detect_xss', return_value=[]):
                with patch.object(validator, 'detect_path_traversal', return_value=[]):
                    with patch.object(validator, 'detect_command_injection', return_value=[]):
                        with patch.object(validator, 'detect_ldap_injection', return_value=[]):
                            with patch.object(validator, 'detect_template_injection', return_value=[]):
                                with patch.object(validator, 'detect_inappropriate_content', return_value=[]):
                                    with patch.object(validator, 'detect_pii', return_value=[]):
                                        with patch.object(validator, 'detect_encoding_attacks', return_value=[]):
                                            result = await validator.validate_input("'; DROP TABLE users; --", "query")
                                            
                                            assert result.is_valid is False
                                            assert len(result.threats) == 1
                                            assert result.threats[0].threat_type == "sql_injection"
            ...
            with patch.object(validator, 'detect_xss', return_value=[]):
                with patch.object(validator, 'detect_path_traversal', return_value=[]):
                    with patch.object(validator, 'detect_command_injection', return_value=[]):
                        with patch.object(validator, 'detect_ldap_injection', return_value=[]):
                            with patch.object(validator, 'detect_template_injection', return_value=[]):
                                with patch.object(validator, 'detect_inappropriate_content', return_value=["profanity"]):
                                    with patch.object(validator, 'detect_pii', return_value=["personal_info"]):
                                        with patch.object(validator, 'detect_encoding_attacks', return_value=[]):
                                            result = await validator.validate_input("inappropriate content", "message")
                                            
                                            assert result.is_valid is False
                                            assert len(result.child_safety_violations) == 2
                                            assert "profanity" in result.child_safety_violations
                                            assert "personal_info" in result.child_safety_violations
    ...
    async def test_validate_input_critical_vs_high_threats(self, validator):
        ...
        critical_threat = SecurityThreat("command_injection", "critical", "field", "value", "Critical")
        high_threat = SecurityThreat("sql_injection", "high", "field", "value", "High")
        
        # Test critical threat
        with patch.object(validator, 'detect_command_injection', return_value=[critical_threat]):
            with patch.object(validator, 'detect_sql_injection', return_value=[]):
                with patch.object(validator, 'detect_xss', return_value=[]):
                    with patch.object(validator, 'detect_path_traversal', return_value=[]):
                        with patch.object(validator, 'detect_ldap_injection', return_value=[]):
                            with patch.object(validator, 'detect_template_injection', return_value=[]):
                                with patch.object(validator, 'detect_inappropriate_content', return_value=[]):
                                    with patch.object(validator, 'detect_pii', return_value=[]):
                                        with patch.object(validator, 'detect_encoding_attacks', return_value=[]):
                                            result = await validator.validate_input("rm -rf /", "command")
        ...
        # Test high threat
        with patch.object(validator, 'detect_command_injection', return_value=[]):
            with patch.object(validator, 'detect_sql_injection', return_value=[high_threat]):
                with patch.object(validator, 'detect_xss', return_value=[]):
                    with patch.object(validator, 'detect_path_traversal', return_value=[]):
                        with patch.object(validator, 'detect_ldap_injection', return_value=[]):
                            with patch.object(validator, 'detect_template_injection', return_value=[]):
...
class TestInputValidationMiddleware:
    ...
    def mock_validator(self):
        ...
        with patch('src.infrastructure.security.input_validation.middleware.get_input_validator', return_value=validator):
            yield validator
    ...
    def mock_audit_integration(self):
        ...
        with patch('src.infrastructure.security.input_validation.middleware.get_audit_integration', return_value=mock_audit):
            yield mock_audit
    
    @pytest.fixture
    def middleware(self, mock_app, mock_validator, mock_audit_integration):
        """Create middleware instance."""
        return InputValidationMiddleware(mock_app, enable_child_safety=True, strict_mode=False)
    ...
    def test_middleware_initialization(self, mock_app):
        ...
        middleware = InputValidationMiddleware(mock_app, enable_child_safety=True, strict_mode=True)
    ...
    @pytest.mark.asyncio
    async def test_dispatch_get_request_validation(self, middleware, mock_request, mock_validator):
        ...
        mock_validator.validate_input.return_value = InputValidationResult(is_valid=True)
    ...
    @pytest.mark.asyncio
    async def test_dispatch_post_request_validation(self, middleware, mock_request, mock_validator):
    ...
    @pytest.mark.asyncio
    async def test_dispatch_security_threat_blocking(self, middleware, mock_request, mock_validator):
        ...
        critical_threat = SecurityThreat("sql_injection", "critical", "param", "malicious_value", "SQL injection")
    ...
    @pytest.mark.asyncio
    async def test_dispatch_child_safety_blocking(self, middleware, mock_request, mock_validator):
    ...
    @pytest.mark.asyncio
    async def test_dispatch_strict_mode_blocking(self, middleware, mock_request, mock_validator):
        ...
        high_threat = SecurityThreat("xss", "high", "param", "suspicious_value", "XSS attempt")
        mock_validator.validate_input.return_value = InputValidationResult(
            is_valid=False,
            threats=[high_threat]
        )
    ...
    @pytest.mark.asyncio
    async def test_dispatch_multiple_medium_threats_blocking(self, middleware, mock_request, mock_validator):
        ...
        threats = [
            SecurityThreat("suspicious_pattern", "medium", "param", "value", "Suspicious pattern 1"),
            SecurityThreat("suspicious_pattern", "medium", "param", "value", "Suspicious pattern 2"),
            SecurityThreat("suspicious_pattern", "medium", "param", "value", "Suspicious pattern 3")
        ]
        mock_validator.validate_input.return_value = InputValidationResult(
            is_valid=False,
            threats=threats
    ...
    @pytest.mark.asyncio
    async def test_dispatch_low_threats_allowed(self, middleware, mock_request, mock_validator):
        ...
        low_threat = SecurityThreat("minor_issue", "low", "param", "slightly_suspicious", "Minor issue")
        mock_validator.validate_input.return_value = InputValidationResult(
            is_valid=True,
            threats=[low_threat]
        )
    ...
    async def test_should_block_request_critical_threats(self, middleware):
        ...
        threats = [SecurityThreat("command_injection", "critical", "field", "value", "Critical")]
    ...
    @pytest.mark.asyncio
    async def test_should_block_request_child_safety_on_child_endpoint(self, middleware):
        """Test that child safety violations cause blocking on child endpoints."""
        threats = []
        child_safety_violations = ["inappropriate_content"]
        context = {"is_child_endpoint": True}
        
        should_block = await middleware._should_block_request(threats, child_safety_violations, context)
        
        assert should_block is True
    
    @pytest.mark.asyncio
    async def test_should_block_request_child_safety_on_regular_endpoint(self, middleware):
    ...
    @pytest.mark.asyncio
    async def test_log_security_incident(self, middleware, mock_request, mock_audit_integration):
        """Test security incident logging."""
        threats = [SecurityThreat("sql_injection", "critical", "field", "value", "SQL injection")]
        child_safety_violations = ["inappropriate_content"]
        
        await middleware._log_security_incident(mock_request, threats, child_safety_violations)
        ...
        assert "Request blocked due to input validation failures" in call_args["description"]
    
    @pytest.mark.asyncio
    async def test_log_security_warning(self, middleware, mock_request, mock_audit_integration):
        """Test security warning logging."""
        threats = [SecurityThreat("suspicious_pattern", "medium", "field", "value", "Suspicious")]
        child_safety_violations = []
        
        await middleware._log_security_warning(mock_request, threats, child_safety_violations)
    ...
    async def test_create_security_error_response_child_safety(self, middleware):
        ...
        response = await middleware._create_security_error_response(threats, child_safety_violations, context)
    ...
    async def test_create_security_error_response_generic(self, middleware):
        ...
        threats = [SecurityThreat("suspicious_pattern", "high", "field", "value", "Suspicious")]
        child_safety_violations = []
        context = {"is_child_endpoint": False}
...
class TestGlobalValidator:
    ...
    async def test_validate_user_input_function(self):
        ...
        with patch('src.infrastructure.security.input_validation.validator.get_input_validator') as mock_get_validator:
            mock_validator = Mock()
            mock_validator.validate_input = AsyncMock(return_value=InputValidationResult(True))
    ...
    async def test_validate_user_input_require_child_safe(self):
        ...
        with patch('src.infrastructure.security.input_validation.validator.get_input_validator') as mock_get_validator:
            mock_validator = Mock()
            mock_validator.validate_input = AsyncMock(return_value=InputValidationResult(
                True, 
                child_safety_violations=["inappropriate_content"]
            ))
            mock_get_validator.return_value = mock_validator
    ...
    async def test_validate_child_message_function(self):
        ...
        with patch('src.infrastructure.security.input_validation.validator.validate_user_input') as mock_validate:
            mock_validate.return_value = InputValidationResult(True)
            
            result = await validate_child_message("Hello, world!")
            
            assert result.is_valid is True
            mock_validate.assert_called_once_with("Hello, world!", "message", require_child_safe=True)
...
class TestBackwardsCompatibilityImports:
    ...
    def test_create_input_validation_middleware_import(self):
        ...
        assert imported_create_input_validation_middleware is create_input_validation_middleware
    
    def test_all_exports_available(self):
        """Test that all exports are available."""
        from src.infrastructure.security.comprehensive_input_validation_middleware import __all__
...
class TestInputValidationIntegration:
    ...
    async def test_full_middleware_workflow_clean_request(self):
        ...
        with patch('src.infrastructure.security.input_validation.middleware.get_input_validator') as mock_get_validator:
            mock_validator = Mock()
            mock_validator.validate_input = AsyncMock(return_value=InputValidationResult(True))
            mock_get_validator.return_value = mock_validator
            
            with patch('src.infrastructure.security.input_validation.middleware.get_audit_integration') as mock_get_audit:
                mock_audit = Mock()
                mock_audit.log_security_event = AsyncMock()
    ...
    async def test_full_middleware_workflow_malicious_request(self):
        ...
        with patch('src.infrastructure.security.input_validation.middleware.get_input_validator') as mock_get_validator:
            ...
            # Mock validator to return critical threat
            critical_threat = SecurityThreat("sql_injection", "critical", "param", "'; DROP TABLE users; --", "SQL injection")
            mock_validator.validate_input = AsyncMock(return_value=InputValidationResult(
                False, 
                threats=[critical_threat]
            ))
    ...
    async def test_child_safety_workflow(self):
        ...
        with patch('src.infrastructure.security.input_validation.middleware.get_input_validator') as mock_get_validator:
            ...
            with patch('src.infrastructure.security.input_validation.middleware.get_audit_integration') as mock_get_audit:
                ...
                middleware = InputValidationMiddleware(mock_app, enable_child_safety=True)
