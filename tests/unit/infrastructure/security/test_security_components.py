...
class TestSecurityManager:
    ...
    async def test_authenticate_user_invalid_token(self, security_manager):
        ...
        security_manager.jwt_auth.verify_token.side_effect = jwt.InvalidTokenError("Invalid token")
        security_manager.rate_limiter.is_allowed.return_value = True
    ...
    async def test_authorize_action_success(self, security_manager):
        ...
        authorized = await security_manager.authorize_action(token_data, required_permission)
...
class TestSafetyMonitorService:
    ...
    async def test_analyze_content_toxic(self, safety_monitor):
        ...
        toxic_content = "This content contains harmful and inappropriate language for children."
    ...
    async def test_analyze_content_bias_detection(self, safety_monitor):
        ...
        biased_content = "Content that might contain subtle bias against certain groups."
    ...
    async def test_age_appropriate_content_check(self, safety_monitor):
        ...
        result = await safety_monitor.analyze_content(content_for_older_kids, target_age=target_age)
    ...
    async def test_real_time_monitoring(self, safety_monitor):
        ...
        conversation_analysis = await safety_monitor.analyze_conversation(conversation_messages)
