...
from src.infrastructure.security.security_manager import SecurityManager, security_manager
...
class TestSecurityManager:
    ...
    def test_hash_password_bcrypt_parameters(self):
        ...
        with patch('bcrypt.gensalt') as mock_gensalt:
            with patch('bcrypt.hashpw') as mock_hashpw:
                ...
                mock_hashpw.assert_called_once_with(password.encode('utf-8'), b'fake_salt')
    ...
    def test_generate_file_signature_verification(self):
        ...
        original_signature = SecurityManager.generate_file_signature(file_content, secret_key)
        verification_signature = SecurityManager.generate_file_signature(file_content, secret_key)
        modified_signature = SecurityManager.generate_file_signature(modified_content, secret_key)
        ...
        assert SecurityManager.secure_compare(original_signature, verification_signature)
        assert not SecurityManager.secure_compare(original_signature, modified_signature)
...
class TestSecurityManagerIntegration:
    ...
    def test_token_generation_security_properties(self):
        ...
        assert all(length == 64 for length in lengths)  # All should be 64 chars
    ...
    def test_file_signature_integration(self):
        ...
        original_signature = SecurityManager.generate_file_signature(original_content, secret_key)
        
        # Verify signature matches
        verification_signature = SecurityManager.generate_file_signature(original_content, secret_key)
        assert SecurityManager.secure_compare(original_signature, verification_signature)
        
        # Detect tampering
        tampered_content = b"Important file content that HAS BEEN tampered with"
        tampered_signature = SecurityManager.generate_file_signature(tampered_content, secret_key)
        assert not SecurityManager.secure_compare(original_signature, tampered_signature)
    ...
    def test_cross_function_compatibility(self):
        ...
        new_signature = SecurityManager.generate_file_signature(file_content, secret_key)
...
class TestSecurityManagerChildSafety:
    ...
    def test_child_data_protection(self):
        ...
        # Verify signature
        verification_signature = SecurityManager.generate_file_signature(child_data_bytes, child_key)
    ...
    def test_child_content_signing(self):
        ...
        for content in child_contents:
            ...
            # Verify signature
            assert len(signature) == 64
            verification_signature = SecurityManager.generate_file_signature(content, content_key)
            assert SecurityManager.secure_compare(signature, verification_signature)
    ...
    def test_coppa_compliance_features(self):
        ...
        consent_token = SecurityManager.generate_secure_token(64)  # Extra long for consent
        ...
        consent_signature = SecurityManager.generate_file_signature(consent_data, consent_key)
        
        # Verify consent signature
        assert len(consent_signature) == 64
        verification = SecurityManager.generate_file_signature(consent_data, consent_key)
