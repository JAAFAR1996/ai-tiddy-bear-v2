...
class TestComprehensiveAuditIntegration:
    ...
    def mock_audit_logger(self):
        ...
        with patch('src.infrastructure.security.comprehensive_audit_integration.get_audit_logger', return_value=mock_logger):
            yield mock_logger
    ...
    def mock_datetime(self):
        ...
        with patch('src.infrastructure.security.comprehensive_audit_integration.datetime') as mock_dt:
    ...
    @pytest.mark.asyncio
    async def test_log_authentication_event_success(self, audit_integration, mock_audit_logger, mock_datetime):
        ...
        assert call_kwargs["context"].user_id == "us***@example.com"  # Sanitized email
    ...
    @pytest.mark.asyncio
    async def test_log_authentication_event_failure(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_authentication_event_account_locked(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_authentication_event_different_types(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_authorization_event_granted(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_authorization_event_denied(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_child_data_operation_create(self, audit_integration, mock_audit_logger, mock_datetime):
        ...
        assert call_kwargs["context"].child_id == "chil***123"  # Sanitized child ID
        assert call_kwargs["details"]["operation"] == "create"
        assert call_kwargs["details"]["data_type"] == "profile"
    
    @pytest.mark.asyncio
    async def test_log_child_data_operation_delete_sensitive(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_child_data_operation_different_operations(self, audit_integration, mock_audit_logger, mock_datetime):
        """Test logging different child data operations."""
        operations = ["create", "read", "update", "delete", "unknown_operation"]
    ...
    @pytest.mark.asyncio
    async def test_log_coppa_compliance_event(self, audit_integration, mock_audit_logger, mock_datetime):
        """Test logging COPPA compliance event."""
        with patch('src.infrastructure.security.comprehensive_audit_integration.requires_coppa_audit_logging', return_value=True):
        ...
        assert call_kwargs["context"].child_id == "copp***789"  # Sanitized child ID
        assert call_kwargs["details"]["verification_method"] == "government_id"
    
    @pytest.mark.asyncio
    async def test_log_coppa_compliance_event_disabled(self, audit_integration, mock_audit_logger, mock_datetime):
        """Test logging COPPA compliance event when COPPA is disabled."""
        with patch('src.infrastructure.security.comprehensive_audit_integration.requires_coppa_audit_logging', return_value=False):
            result = await audit_integration.log_coppa_compliance_event(
                event_type="consent_granted",
    ...
    @pytest.mark.asyncio
    async def test_log_coppa_compliance_different_event_types(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_security_event(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_security_event_different_severities(self, audit_integration, mock_audit_logger, mock_datetime):
        """Test logging security events with different severity levels."""
        severities = ["debug", "info", "warning", "error", "critical", "unknown_severity"]
    ...
    @pytest.mark.asyncio
    async def test_log_system_event(self, audit_integration, mock_audit_logger, mock_datetime):
    ...
    @pytest.mark.asyncio
    async def test_log_system_event_different_types(self, audit_integration, mock_audit_logger, mock_datetime):
        """Test logging different system event types."""
        system_events = ["startup", "shutdown", "config_change", "unknown_system_event"]
    ...
    @pytest.mark.asyncio
    async def test_audit_child_data_operation_convenience_function(self, mock_audit_logger):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_audit_operations(self, audit_integration, mock_audit_logger, mock_datetime):
        """Test concurrent audit operations."""
        import asyncio
        
        # Create multiple concurrent audit operations
        tasks = [
            audit_integration.log_authentication_event("login", "user1@example.com", True),
            audit_integration.log_authorization_event("user2", "resource", "action", True),
            audit_integration.log_child_data_operation("read", "child1", "parent1", "profile"),
            audit_integration.log_security_event("test_event", "info", "Test description"),
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_audit_workflow(self, audit_integration, mock_audit_logger, mock_datetime):
        """Test comprehensive audit workflow covering all event types."""
        # Simulate a complete user workflow
        workflow_events = [
            # 1. User logs in
            ("auth", audit_integration.log_authentication_event("login", "workflow@example.com", True)),
            
            # 2. User accesses child profile
            ("authz", audit_integration.log_authorization_event("workflow_user", "child_profile", "read", True, "workflow_child")),
            
            # 3. User reads child data
            ("data", audit_integration.log_child_data_operation("read", "workflow_child", "workflow_user", "profile")),
            
            # 4. COPPA event for data access
            ("coppa", audit_integration.log_coppa_compliance_event("data_accessed", "workflow_child", "workflow_user", "Child data accessed")),
            
            # 5. Security event for activity monitoring
            ("security", audit_integration.log_security_event("user_activity", "info", "User activity monitored")),
            
            # 6. User logs out
            ("logout", audit_integration.log_authentication_event("logout", "workflow@example.com", True))
        ]
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_audit_operations(self, audit_integration, mock_datetime):
        """Test error handling in audit operations."""
        # Mock audit logger to raise exception
        mock_failing_logger = Mock()
        mock_failing_logger.log_event = AsyncMock(side_effect=Exception("Audit logging error"))
        
        with patch('src.infrastructure.security.comprehensive_audit_integration.get_audit_logger', return_value=mock_failing_logger):
            ...
            with pytest.raises(Exception, match="Audit logging error"):
                await audit_integration_failing.log_authentication_event("login", "error@example.com", True)
    
    @pytest.mark.asyncio
    async def test_edge_cases_invalid_inputs(self, audit_integration, mock_audit_logger, mock_datetime):
        ...
        edge_cases = [
            ...
            (None, None, True),
            # Very long strings
            ("very_long_event_type_" + "x" * 100, "very_long_email_" + "x" * 100 + "@example.com", True),
            # Special characters
        ...
        for event_type, email, success in edge_cases:
            try:
                result = await audit_integration.log_authentication_event(event_type, email, success)
    ...
    @pytest.mark.asyncio
    async def test_audit_context_propagation(self, audit_integration, mock_audit_logger, mock_datetime):
