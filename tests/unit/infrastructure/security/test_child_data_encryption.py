...
def mock_cryptography(mocker):
    ...
    mock_fernet_class = mocker.patch('src.infrastructure.security.child_data_encryption.Fernet')
    mock_fernet_class.return_value = mock_fernet
    
    mock_pbkdf2 = mocker.patch('src.infrastructure.security.child_data_encryption.PBKDF2HMAC')
    mock_hashes = mocker.patch('src.infrastructure.security.child_data_encryption.hashes')
...
class TestChildDataEncryption:
    ...
    def test_sensitive_fields_definition(self):
        ...
        expected_sensitive = {
            "name", "date_of_birth", "medical_notes", "emergency_contacts",
            "special_needs", "cultural_background", "custom_settings"
        }
        
        assert ChildDataEncryption.SENSITIVE_FIELDS == expected_sensitive
    
    def test_pii_fields_definition(self):
        """Test that PII fields are properly defined."""
        expected_pii = {
            "name", "date_of_birth", "gender", "emergency_contacts",
            "medical_notes", "cultural_background"
        }
    ...
    def test_encrypt_child_data_with_none_values(self, encryption_service, mock_cryptography):
    ...
    def test_decrypt_child_data_basic(self, encryption_service, mock_cryptography):
        ...
        with patch.object(encryption_service, '_calculate_checksum', return_value="test_checksum"):
    ...
    def test_decrypt_child_data_integrity_check_failure(self, encryption_service, mock_cryptography):
    ...
    def test_decrypt_child_data_checksum_mismatch(self, encryption_service, mock_cryptography):
        ...
        with patch.object(encryption_service, '_calculate_checksum', return_value="correct_checksum"):
            with patch('src.infrastructure.security.child_data_encryption.logger') as mock_logger:
                result = encryption_service.decrypt_child_data(encrypted_data)
    ...
    def test_validate_coppa_compliance_valid(self, encryption_service):
        ...
        result = encryption_service.validate_coppa_compliance(child_data, consent_record)
    ...
    def test_validate_coppa_compliance_pii_without_consent(self, encryption_service):
        ...
        pii_violation = [v for v in violations if "PII data present without consent" in v]
    ...
    def test_encryption_error_handling(self, encryption_service):
        ...
        with patch.object(encryption_service.cipher, 'encrypt', side_effect=Exception("Encryption error")):
    ...
    def test_comprehensive_encryption_decryption_cycle(self, encryption_service, mock_cryptography):
        ...
        sensitive_data = {"name": "Test Child", "medical_notes": "No allergies"}
