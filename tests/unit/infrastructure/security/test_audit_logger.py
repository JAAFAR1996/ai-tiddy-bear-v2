...
class TestAuditLogger:
    ...
    def test_audit_event_type_enum_values(self):
        ...
        assert AuditEventType.PARENTAL_CONSENT_REQUEST.value == "parental_consent_request"
        assert AuditEventType.PARENTAL_CONSENT_GRANTED.value == "parental_consent_granted"
        assert AuditEventType.PARENTAL_CONSENT_REVOKED.value == "parental_consent_revoked"
    ...
    async def test_log_event_error_handling(self, audit_logger):
        ...
        with patch('src.infrastructure.security.audit_logger.AuditEvent', side_effect=Exception("Event creation error")):
            event_id = await audit_logger.log_event(
                event_type=AuditEventType.SYSTEM_STARTUP,
                severity=AuditSeverity.INFO,
                category=AuditCategory.SYSTEM_SECURITY,
    ...
    async def test_log_child_interaction_content_hashing(self, audit_logger):
        ...
        assert len(details["content_hash"]) == 16  # SHA256 truncated to 16 chars
    ...
    async def test_write_events_to_file(self, audit_logger):
        ...
        # Mock aiofiles.open
        with patch('src.infrastructure.security.audit_logger.aiofiles.open', mock_open()) as mock_file:
    ...
    async def test_write_events_empty_list(self, audit_logger):
        ...
        with patch('src.infrastructure.security.audit_logger.aiofiles.open') as mock_file:
    ...
    async def test_write_events_error_handling(self, audit_logger):
        ...
        with patch('src.infrastructure.security.audit_logger.aiofiles.open', side_effect=Exception("File error")):
        ...
        with patch.object(audit_logger, '_write_events_to_file') as mock_write:
            async with audit_logger.buffer_lock:
                if len(audit_logger.audit_entries) >= audit_logger.config.batch_size:
                    events_to_write = audit_logger.audit_entries[:audit_logger.config.batch_size]
                    audit_logger.audit_entries = audit_logger.audit_entries[audit_logger.config.batch_size:]
    ...
    async def test_comprehensive_coppa_audit_trail(self, audit_logger):
        ...
        events = [
            ...
            (AuditEventType.DATA_RETENTION_TRIGGERED, "Data retention policy applied"),
        ]
        
        event_ids = []
        for event_type, description in events:
            if event_type in [AuditEventType.PARENTAL_CONSENT_REQUEST, 
        ...
        coppa_events = [e for e in audit_logger.audit_entries 
