...
class TestEncryptionService:
    ...
    def test_initialization_missing_key_error(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(EncryptionKeyError, match="COPPA_ENCRYPTION_KEY environment variable is required"):
                EncryptionService()
    ...
    def test_initialization_weak_key_error(self):
        ...
        with patch.dict(os.environ, {'COPPA_ENCRYPTION_KEY': weak_key}):
            with pytest.raises(EncryptionKeyError, match="Detected weak or insufficiently strong encryption key"):
                EncryptionService()
    ...
    def test_validate_encryption_key_test_encryption_failure(self, encryption_service):
        ...
        with patch('cryptography.fernet.Fernet') as mock_fernet:
            ...
            mock_instance.decrypt.return_value = b'wrong_data'  # Different from test data
    ...
    def test_derive_key_pbkdf2_parameters(self, encryption_service, valid_fernet_key):
        ...
        with patch('cryptography.hazmat.primitives.kdf.pbkdf2.PBKDF2HMAC') as mock_pbkdf2:
            mock_kdf = Mock()
    ...
    def test_get_key_version_generated(self, encryption_service):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch('src.infrastructure.security.encryption_service.datetime') as mock_dt:
    ...
    def test_encrypt_uninitialized_service(self):
        ...
        with pytest.raises(EncryptionKeyError, match="Encryption service not initialized"):
    ...
    def test_encrypt_fernet_exception(self, encryption_service):
        ...
        with patch.object(encryption_service._fernet, 'encrypt', side_effect=Exception("Fernet error")):
            with pytest.raises(EncryptionKeyError, match="Encryption initialization failed"):
    ...
    def test_decrypt_dict_data(self, encryption_service):
        ...
        test_data = {"key": "value", "number": 42, "nested": {"inner": "value"}}
    ...
    def test_decrypt_invalid_token(self, encryption_service):
        ...
        with pytest.raises(EncryptionKeyError, match="Decryption failed - data may be corrupted"):
    ...
    def test_decrypt_invalid_json_payload(self, encryption_service):
        ...
        with patch.object(encryption_service._fernet, 'decrypt', return_value=b'invalid_json'):
            with pytest.raises(EncryptionKeyError, match="Decryption failed"):
    ...
    def test_decrypt_missing_metadata(self, encryption_service):
        ...
        encrypted_payload = encryption_service._fernet.encrypt(json.dumps(payload).encode())
    ...
    def test_rotate_key_rollback_on_failure(self, encryption_service):
        ...
        with patch.object(encryption_service, '_derive_key', side_effect=Exception("Derive error")):
    ...
    def test_entropy_calculation_comprehensive(self, encryption_service):
        ...
        entropy_tests = [
            ...
            ("abcdefghijklmnopqrstuvwxyz" + "0123456789" + "ABCDEF", False),  # Medium entropy
...
class TestGetEncryptionService:
    ...
    def test_get_encryption_service_caching(self, mock_environment):
        ...
        with patch('src.infrastructure.security.encryption_service.EncryptionService') as mock_service:
            mock_instance = Mock()
...
class TestEncryptionServiceIntegration:
    ...
    def test_full_coppa_compliance_workflow(self, mock_environment):
        ...
        child_data = {
            ...
            "preferences": {"favorite_color": "blue", "games": ["puzzle", "story"]}
        }
    ...
    def test_logging_and_monitoring(self, mock_environment):
        ...
        with patch('src.infrastructure.security.encryption_service.logger') as mock_logger:
            service = EncryptionService()
    ...
    def test_child_safety_data_protection(self, mock_environment):
        ...
        child_data_types = [
            ...
            {"personal_info": {"name": "Child", "age": 7, "school": "Elementary"}},
