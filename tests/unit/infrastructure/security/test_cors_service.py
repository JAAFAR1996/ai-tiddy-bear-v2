...
class TestCORSConfiguration:
    ...
    def test_cors_configuration_validation_invalid_wildcard_with_credentials(self):
...
class TestCORSSecurityService:
    ...
    def test_validate_origin_allowed_strict(self, cors_service):
        ...
        assert "headers" in result
        assert result["headers"]["Access-Control-Allow-Origin"] == "https://ai-teddy.com"
    ...
    def test_validate_origin_with_custom_policy(self, cors_service):
        ...
        result = cors_service.validate_origin("https://localhost:3000", policy=CORSPolicy.MODERATE)
    ...
    def test_handle_preflight_request_no_method(self, cors_service):
        ...
        assert result["allowed"] is True  # Method validation should pass if None
    ...
    def test_handle_preflight_request_no_headers(self, cors_service):
        ...
        assert result["allowed"] is True  # Headers validation should pass if None
    ...
    def test_generate_preflight_headers(self, cors_service):
        ...
        config = cors_service.configurations[CORSPolicy.STRICT]
        headers = cors_service._generate_preflight_headers("https://ai-teddy.com", config)
    ...
    def test_log_cors_violation(self, cors_service):
        ...
        with patch('src.infrastructure.security.cors_service.logger') as mock_logger:
            cors_service._log_cors_violation("https://malicious.com", "Origin not allowed")
    ...
    def test_add_allowed_origin_valid(self, cors_service):
        ...
        result = cors_service.add_allowed_origin("https://newdomain.com", CORSPolicy.STRICT)
        
        assert result is True
        assert "https://newdomain.com" in cors_service.configurations[CORSPolicy.STRICT].allowed_origins
    
    def test_add_allowed_origin_invalid_security(self, cors_service):
        """Test adding invalid origin fails."""
        result = cors_service.add_allowed_origin("http://malicious.com", CORSPolicy.STRICT)
        
        assert result is False
        assert "http://malicious.com" not in cors_service.configurations[CORSPolicy.STRICT].allowed_origins
    ...
    def test_add_allowed_origin_exception_handling(self, cors_service):
        ...
        with patch.object(cors_service, '_validate_origin_security', side_effect=Exception("Test error")):
            result = cors_service.add_allowed_origin("https://example.com", CORSPolicy.STRICT)
    ...
    def test_remove_allowed_origin(self, cors_service):
        ...
        assert result is True
        assert origin not in cors_service.configurations[CORSPolicy.STRICT].allowed_origins
    ...
    def test_remove_allowed_origin_exception_handling(self, cors_service):
        ...
        with patch.object(cors_service.configurations[CORSPolicy.STRICT], 'allowed_origins', side_effect=Exception("Test error")):
            result = cors_service.remove_allowed_origin("https://example.com", CORSPolicy.STRICT)
...
class TestCORSServiceIntegration:
    ...
    def test_cors_policy_escalation(self, cors_service):
        ...
        moderate_result = cors_service.validate_origin("http://localhost:3000", policy=CORSPolicy.MODERATE)
        assert moderate_result["allowed"] is False
        
        # Try with permissive policy
        permissive_result = cors_service.validate_origin("http://localhost:3000", policy=CORSPolicy.PERMISSIVE)
    ...
    def test_cors_violation_tracking(self, cors_service):
        ...
        assert malicious_origin in [origin for origin, count in stats["top_violating_origins"]]
    ...
    def test_cors_dynamic_origin_management(self, cors_service):
        ...
        # Remove origin
        remove_result = cors_service.remove_allowed_origin(new_origin, CORSPolicy.STRICT)
