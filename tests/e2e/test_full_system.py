...
class TestCompleteUserJourney:
    ...
    async def test_full_user_journey(
        ...
        usage_response = await test_client.get(
            f"/api/v1/children/{child_id}/usage?days=1", headers=auth_headers
        )
        ...
        safety_response = await test_client.get(
            f"/api/v1/children/{child_id}/safety-events", headers=auth_headers
        )

        assert safety_response.status_code == 200
        safety_events = safety_response.json()
        # At least one from unsafe message
        assert len(safety_events["events"]) >= 1

        # Step 10: Export Child Data (COPPA Requirement)
        export_response = await test_client.get(
            f"/api/v1/coppa/export/{child_id}", headers=auth_headers
        )
...
class TestMultiChildFamily:
    ...
    @pytest.mark.asyncio
    async def test_multi_child_management(
        self, test_client: AsyncClient, test_database
    ):
...
class TestLongRunningSession:
    ...
    async def test_extended_conversation(
        ...
        for i, message in enumerate(story_messages):
            chat_data = {"child_id": child_id, "message": message}
...
class TestSystemResilience:
    ...
    async def test_concurrent_users(
        ...
        for login_response in login_responses[:5]:  # Limit to 5 for test speed
            ...
            task = test_client.post(
                "/api/v1/conversations/chat",
                json={"child_id": child_id, "message": "Hello AI friend!"},
                headers=auth_headers,
            )
