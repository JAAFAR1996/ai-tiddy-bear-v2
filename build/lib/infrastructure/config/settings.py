    ...
    def model_post_init(self, __context) -> None:
        ...
        if env == "production":
            if self.application.DEBUG:
                raise ValueError("DEBUG must be False in production environment for security.")
            ...
            if not self.security.COPPA_ENCRYPTION_KEY or len(self.security.COPPA_ENCRYPTION_KEY) != 44:
                raise ValueError("COPPA_ENCRYPTION_KEY must be a valid Fernet key (44 characters) in production.")
            ...
            if not self.ai.OPENAI_API_KEY or not self.ai.OPENAI_API_KEY.startswith("sk-"):
                raise ValueError("OPENAI_API_KEY must be a valid OpenAI API key in production.")

            # Sentry DSN for error monitoring
            if not self.sentry.SENTRY_DSN:
                logger.warning("SENTRY_DSN is not set in production. Error monitoring will be limited.")
        ...
        if self.security.ACCESS_TOKEN_EXPIRE_MINUTES >= (
            ...
            raise ValueError("ACCESS_TOKEN_EXPIRE_MINUTES cannot exceed REFRESH_TOKEN_EXPIRE_DAYS.")
