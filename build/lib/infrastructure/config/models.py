...
class ServerSettings(BaseModel):
    ...
    MAX_CONTENT_LENGTH_MB: int = Field(
        50, description="Maximum content length for requests in MB"
    )
    REQUEST_TIMEOUT_SECONDS: int = Field(
        60, description="Default request timeout in seconds"
    )
    ...
    SSL_KEY_PATH: Optional[str] = Field(None, description="Path to SSL key file")
...
class DatabaseSettings(BaseModel):
    ...
    DATABASE_POOL_SIZE: int = Field(
        20, ge=5, le=100, description="Database connection pool size"
    )
    DATABASE_MAX_OVERFLOW: int = Field(
        0, ge=0, le=50, description="Database connection max overflow"
    )
    DATABASE_POOL_RECYCLE: int = Field(
        300, ge=60, le=3600, description="Database connection pool recycle time in seconds"
    )
    DATABASE_ECHO: bool = Field(False, description="Enable SQLAlchemy echo mode")
    DATABASE_SSL_MODE: str = Field("require", description="SSL mode for database connection")
...
class SecuritySettings(BaseModel):
    ...
    REFRESH_TOKEN_EXPIRE_MINUTES: int = Field(
        10080, description="Refresh token expiration time in minutes (7 days)"
    )
    PASSWORD_HASHING_ALGORITHM: str = Field(
        "bcrypt", description="Algorithm for password hashing"
    )
...
class LoggingSettings(BaseModel):
    ...
    LOG_ROTATION: str = Field("10 MB", description="Log file rotation size or frequency")
...
class COPPAComplianceSettings(BaseModel):
    ...
    DATA_RETENTION_DAYS: int = Field(
        90, description="Data retention period in days for child data"
    )
    PARENTAL_CONSENT_REQUIRED: bool = Field(
        True, description="Require parental consent for data collection"
    )
