...
from src.common.constants import CHILD_SPECIFIC_API_ENDPOINTS # Import the constant
...
class ApplicationSettings(BaseApplicationSettings):
    ...
    CHILD_ENDPOINTS: List[str] = Field(
        CHILD_SPECIFIC_API_ENDPOINTS, env="CHILD_ENDPOINTS" # Use the imported constant
    )
    MIN_CHILD_AGE: int = Field(3, env="MIN_CHILD_AGE")
    MAX_CHILD_AGE: int = Field(13, env="MAX_CHILD_AGE")
    PROJECT_ROOT: Path = Field(Path(__file__).parent.parent.parent.parent.resolve(), env="PROJECT_ROOT") # Ensure PROJECT_ROOT is a Path object directly.
