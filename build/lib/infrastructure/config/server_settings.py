...
class ServerSettings(BaseSettings):
    ...
    DEV_PORT_MIN: int = Field(
        3000, description="Minimum port for development environment."
    )
    DEV_PORT_MAX: int = Field(
        9999, description="Maximum port for development environment."
    )
    PROD_PORT_MIN: int = Field(
        8000, description="Minimum port for production environment."
    )
    PROD_PORT_MAX: int = Field(
        65535, description="Maximum port for production environment."
    )
    UVICORN_BACKLOG: int = Field(2048, description="Uvicorn connection backlog.")
    UVICORN_KEEPALIVE_TIMEOUT: int = Field(
        60, description="Uvicorn keep-alive timeout in seconds."
    )
    UVICORN_MAX_REQUESTS: int = Field(
        1000, description="Uvicorn worker recycling max requests."
    )
    UVICORN_MAX_REQUESTS_JITTER: int = Field(
        50, description="Uvicorn worker recycling max requests jitter."
    )
    UVICORN_DEV_WORKERS: int = Field(
        1, description="Number of Uvicorn workers in development mode."
    )
    UVICORN_PROD_WORKERS_DEFAULT: int = Field(
        4, description="Default number of Uvicorn workers in production mode."
    )
