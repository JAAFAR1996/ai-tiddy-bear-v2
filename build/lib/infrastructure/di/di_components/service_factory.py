...
class ServiceFactory:
    ...
    def create_speech_processor(self, settings: Any) -> Any:
        ...
        try:
            ...
            raise ImportError(f"Missing speech processor dependency: whisper") from e
        except AttributeError as e:
            logger.critical(f"CRITICAL ERROR: Missing WHISPER_MODEL in settings: {e}")
            raise ValueError(f"WHISPER_MODEL configuration required") from e
        except Exception as e:
            logger.critical(f"CRITICAL ERROR: Failed to create speech processor: {e}")
            raise RuntimeError(f"Speech processor service creation failed") from e
    def create_openai_client(self, settings: Any) -> Any:
        ...
        try:
            ...
            raise ImportError(f"Missing OpenAI dependency: openai") from e
        except AttributeError as e:
            logger.critical(f"CRITICAL ERROR: Missing OPENAI_API_KEY in settings: {e}")
            raise ValueError(f"OPENAI_API_KEY configuration required") from e
        except Exception as e:
            logger.critical(f"CRITICAL ERROR: Failed to create OpenAI client: {e}")
            raise RuntimeError(f"OpenAI service creation failed") from e
    def create_tts_service(self, settings: Any) -> Any:
        ...
        try:
            ...
            raise ImportError(f"Missing TTS dependency: elevenlabs") from e
        except AttributeError as e:
            logger.critical(
                f"CRITICAL ERROR: Missing ELEVENLABS_API_KEY in settings: {e}"
            )
            raise ValueError(f"ELEVENLABS_API_KEY configuration required") from e
        except Exception as e:
            logger.critical(f"CRITICAL ERROR: Failed to create TTS service: {e}")
            raise RuntimeError(f"TTS service creation failed") from e
    def create_database(self, settings: Any) -> Any:
        ...
        try:
            ...
            # Pass connection pooling settings from main settings to Database constructor
            return Database(
            ...
            logger.critical("Install required dependencies: pip install sqlalchemy asyncpg")
            raise ImportError(f"Missing database dependency") from e
        except AttributeError as e:
            logger.critical(f"CRITICAL ERROR: Missing database settings: {e}")
            raise ValueError(f"Database configuration required") from e
        except Exception as e:
            logger.critical(f"CRITICAL ERROR: Failed to create database: {e}")
            raise RuntimeError(f"Database service creation failed") from e