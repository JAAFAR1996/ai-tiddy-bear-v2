...
class RedisPersonalityProfileRepository(IPersonalityProfileRepository):
    ...
    async def get_profile_by_child_id(self, child_id: UUID) -> Optional[ChildPersonality]:
        ...
        try:
            ...
            if profile_data_json:
                ...
                # Deserialize PersonalityType enum member and datetime correctly
                personality_type = PersonalityType[profile_data_dict.get("personality_type", "OTHER").upper()]
                ...
                self.logger.debug(f"Retrieved personality profile for child {child_id} from Redis.")
                return profile
            self.logger.debug(f"Personality profile for child {child_id} not found in Redis.")
            return None
        except Exception as e:
            self.logger.error(f"Error getting personality profile for child {child_id} from Redis: {e}", exc_info=True)
            return None
    ...
    async def save_profile(self, profile: ChildPersonality) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Saved personality profile for child {profile.child_id} to Redis.")
        except Exception as e:
            self.logger.error(f"Error saving personality profile for child {profile.child_id} to Redis: {e}", exc_info=True)
    ...
    async def delete_profile(self, child_id: UUID) -> bool:
        ...
        try:
            ...
            if result > 0:
                self.logger.debug(f"Deleted personality profile for child {child_id} from Redis.")
                return True
            self.logger.debug(f"Personality profile for child {child_id} not found for deletion in Redis.")
            return False
        except Exception as e:
            self.logger.error(f"Error deleting personality profile for child {child_id} from Redis: {e}", exc_info=True)
            return False
    ...
    async def get_all_profiles(self) -> List[ChildPersonality]:
        ...
        self.logger.warning("Attempted to retrieve all personality profiles. This operation is not optimized for large datasets.")
