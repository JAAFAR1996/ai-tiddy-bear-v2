...
class Database:
    ...
    def __init__(
        ...
        if pool_size is not None: self.config.pool_size = pool_size
        if max_overflow is not None: self.config.max_overflow = max_overflow
        if pool_recycle is not None: self.config.pool_recycle = pool_recycle
        if pool_pre_ping is not None: self.config.pool_pre_ping = pool_pre_ping
        if pool_timeout is not None: self.config.pool_timeout = pool_timeout
