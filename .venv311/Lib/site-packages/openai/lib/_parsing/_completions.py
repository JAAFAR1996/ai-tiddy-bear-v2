...
def get_input_tool_by_name(
    *, input_tools: list[ChatCompletionToolParam], name: str
    ...
    return next(
        (t for t in input_tools if t.get("function", {}).get("name") == name), None
    )
...
def _parse_content(
    response_format: type[ResponseFormatT], content: str
) -> ResponseFormatT:
    ...
    if is_dataclass_like_type(response_format):
        if not PYDANTIC_V2:
            raise TypeError(
                f"Non BaseModel types are only supported with Pydantic v2 - {response_format}"
            )

        return pydantic.TypeAdapter(response_format).validate_json(content)

    raise TypeError(
        f"Unable to automatically parse response format type {response_format}"
    )
