...
def parse_text(
    text: str, text_format: type[TextFormatT] | NotGiven
) -> TextFormatT | None:
    ...
    if is_dataclass_like_type(text_format):
        if not PYDANTIC_V2:
            raise TypeError(
                f"Non BaseModel types are only supported with Pydantic v2 - {text_format}"
            )
...
def get_input_tool_by_name(
    *, input_tools: Iterable[ToolParam], name: str
) -> FunctionToolParam | None:
...
def parse_function_tool_arguments(
    *,
    ...
    input_tool = get_input_tool_by_name(
        input_tools=input_tools, name=function_call.name
    )
    if not input_tool:
        return None
