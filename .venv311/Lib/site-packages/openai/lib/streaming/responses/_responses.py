...
from typing import Any, Awaitable, Generic, Iterable, List, cast

from typing_extensions import AsyncIterator, Callable, Iterator, Self
...
from ....types.responses import ResponseStreamEvent as RawResponseStreamEvent
...
from ..._parsing._responses import TextFormatT, parse_response, parse_text
from ._events import (
...
class ResponseStream(Generic[TextFormatT]):
    def __init__(
        ...
        *,
        ...
        self._state = ResponseStreamState(
            text_format=text_format, input_tools=input_tools
        )
    ...
    def __stream__(self) -> Iterator[ResponseStreamEvent[TextFormatT]]:
        for sse_event in self._raw_stream:
            ...
            for event in events_to_fire:
                if (
                    self._starting_after is None
                    or event.sequence_number > self._starting_after
                ):
                    yield event
...
class ResponseStreamState(Generic[TextFormatT]):
    def __init__(
        ...
        *,
        ...
        self._rich_text_format: type | NotGiven = (
            text_format if inspect.isclass(text_format) else NOT_GIVEN
        )

    def handle_event(
        self, event: RawResponseStreamEvent
    ) -> List[ResponseStreamEvent[TextFormatT]]:
    ...
    def _create_initial_response(
        self, event: RawResponseStreamEvent
    ) -> ParsedResponseSnapshot:
        if event.type != "response.created":
            raise RuntimeError(
                f"Expected to have received `response.created` before `{event.type}`"
            )

        return construct_type_unchecked(
            type_=ParsedResponseSnapshot, value=event.response.to_dict()
        )
