...
from ..cimpl import OFFSET_INVALID, KafkaError
...
from ._listoffsets import ListOffsetsResultInfo, OffsetSpec  # noqa: F401
from ._metadata import BrokerMetadata  # noqa: F401
from ._records import DeletedRecords  # noqa: F401
from ._resource import ResourcePatternType, ResourceType  # noqa: F401
...
class AdminClient(_AdminClientImpl):
    ...
    def _check_delete_records(request):
        if not isinstance(request, list):
            raise TypeError(
                f"Expected Request to be a list, got '{type(request).__name__}' "
            )
        for req in request:
            if not isinstance(req, _TopicPartition):
                raise TypeError(
                    "Element of the request list must be of type 'TopicPartition'"
    ...
    def _check_elect_leaders(election_type, partitions):
        ...
        if partitions is not None:
            if not isinstance(partitions, list):
                raise TypeError(
                    "Expected 'partitions' to be a list, got "
            ...
            for partition in partitions:
                ...
                if partition.partition < 0:
                    raise ValueError(
                        "Elements of the 'partitions' list must not have negative value"
                        + " for 'partition' field"
                    )
            ...
            if types is not None:
                ...
                for type in types:
                    if not isinstance(type, _ConsumerGroupType):
                        raise TypeError(
                            "All elements of types must be of type ConsumerGroupType"
                        )
    ...
    def elect_leaders(self, election_type, partitions=None, **kwargs):
        ...
        super(AdminClient, self).elect_leaders(
            election_type.value, partitions, f, **kwargs
        )
