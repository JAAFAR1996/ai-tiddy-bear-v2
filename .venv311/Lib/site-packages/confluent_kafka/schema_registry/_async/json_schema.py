    ...
    if schema.references is not None:
        for ref in schema.references:
            referenced_schema = await schema_registry_client.get_version(
                ref.subject, ref.version, True
            )
            ref_registry = await _resolve_named_schema(
                referenced_schema.schema, schema_registry_client, ref_registry
            )
...
class AsyncJSONSerializer(AsyncBaseSerializer):
    ...
    async def __init_impl(
        ...
        if to_dict is not None and not callable(to_dict):
            raise ValueError(
                "to_dict must be callable with the signature "
    ...
    async def __serialize(
        ...
        if latest_schema is not None:
            self._schema_id = SchemaId(
                JSON_TYPE, latest_schema.schema_id, latest_schema.guid
            )
            ...
            if self._auto_register:
                ...
                self._schema_id = SchemaId(
                    JSON_TYPE, registered_schema.schema_id, registered_schema.guid
                )
            else:
        ...
        if latest_schema is not None:
            ...
            parsed_schema, ref_registry = await self._get_parsed_schema(
    ...
    async def _get_parsed_schema(
        self, schema: Schema
    ) -> Tuple[Optional[JsonSchema], Optional[Registry]]:
        if schema is None:
            return None, None
    ...
    def _get_validator(
        self, schema: Schema, parsed_schema: JsonSchema, registry: Registry
    ) -> Validator:
...
class AsyncJSONDeserializer(AsyncBaseDeserializer):
    ...
    async def __init_impl(
        ...
        if schema:
            self._reader_schema, self._ref_registry = await self._get_parsed_schema(
                self._schema
            )
        else:
    ...
    async def __serialize(
        ...
        if self._registry is not None:
            ...
            writer_schema, writer_ref_registry = await self._get_parsed_schema(
                writer_schema_raw
            )
            if subject is None:
        ...
        if latest_schema is not None:
            ...
            reader_schema, reader_ref_registry = await self._get_parsed_schema(
                latest_schema.schema
            )
        ...
        reader_ref_resolver = reader_ref_registry.resolver_with_root(
            reader_root_resource
        )
        ...
        if self._validate:
            try:
                validator = self._get_validator(
                    reader_schema_raw, reader_schema, reader_ref_registry
                )
