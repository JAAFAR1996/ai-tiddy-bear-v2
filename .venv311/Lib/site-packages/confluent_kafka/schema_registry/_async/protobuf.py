...
async def _resolve_named_schema(
    ...
    if schema.references is not None:
        for ref in schema.references:
            ...
            referenced_schema = await schema_registry_client.get_version(
                ref.subject, ref.version, True, "serialized"
            )
            await _resolve_named_schema(
                referenced_schema.schema, schema_registry_client, pool, visited
            )
            file_descriptor_proto = _str_to_proto(
                ref.name, referenced_schema.schema.schema_str
            )
...
class AsyncProtobufSerializer(AsyncBaseSerializer):
    ...
    async def __init_impl(
        ...
        if self._use_latest_with_metadata is not None and not isinstance(
            self._use_latest_with_metadata, dict
        ):
        ...
        for rule in self._rule_registry.get_executors():
            rule.configure(
                self._registry.config() if self._registry else {},
    ...
    async def _resolve_dependencies(
        ...
        for dep in file_desc.dependencies:
            ...
            schema = Schema(
                _schema_to_str(dep), references=dep_refs, schema_type="PROTOBUF"
            )
    ...
    async def __serialize(
        ...
        subject = (
            self._subject_name_func(ctx, message.DESCRIPTOR.full_name) if ctx else None
        )
        latest_schema = None
        ...
        if latest_schema is not None:
            self._schema_id = SchemaId(
                PROTOBUF_TYPE, latest_schema.schema_id, latest_schema.guid
            )
            ...
            self._schema = Schema(
                self._schema.schema_str, self._schema.schema_type, references
            )
            ...
            if self._auto_register:
                ...
                self._schema_id = SchemaId(
                    PROTOBUF_TYPE, registered_schema.schema_id, registered_schema.guid
                )
            else:
    ...
    async def _get_parsed_schema(
        self, schema: Schema
    ) -> Tuple[descriptor_pb2.FileDescriptorProto, DescriptorPool]:
...
class AsyncProtobufDeserializer(AsyncBaseDeserializer):
    ...
    def __call__(
        self, data: bytes, ctx: Optional[SerializationContext] = None
    ) -> Optional[bytes]:
    ...
    async def __serialize(
        ...
        if self._registry is not None:
            writer_schema_raw = await self._get_writer_schema(
                schema_id, subject, fmt="serialized"
            )
        ...
        if latest_schema is not None:
            migrations = await self._get_migrations(
                subject, writer_schema_raw, latest_schema, None
            )
        ...
        if reader_schema is not None:
            ...
            reader_desc = reader_schema.message_types_by_name.get(
                writer_desc.name, reader_desc
            )
        ...
        def field_transformer(rule_ctx, field_transform, message):
            return transform(  # noqa: E731
                rule_ctx, reader_desc, message, field_transform
            )
    ...
    def _get_message_desc(
        self, pool: DescriptorPool, fd: FileDescriptor, msg_index: List[int]
    ) -> Descriptor:
        file_desc_proto = descriptor_pb2.FileDescriptorProto()
        fd.CopyToProto(file_desc_proto)
        (full_name, desc_proto) = self._get_message_desc_proto(
            "", file_desc_proto, msg_index
        )
