...
async def _resolve_named_schema(
    ...
    if schema.references is not None:
        for ref in schema.references:
            ...
            ref_named_schemas = await _resolve_named_schema(
                referenced_schema.schema, schema_registry_client
            )
    ...
    async def __serialize(
        ...
        if latest_schema is not None:
            self._schema_id = SchemaId(
                AVRO_TYPE, latest_schema.schema_id, latest_schema.guid
            )
            ...
            if self._auto_register:
                ...
                self._schema_id = SchemaId(
                    AVRO_TYPE, registered_schema.schema_id, registered_schema.guid
                )
            else:
        ...
        if latest_schema is not None:
            ...
            def field_transformer(rule_ctx, field_transform, msg):
                return transform(  # noqa: E731
                    rule_ctx, parsed_schema, msg, field_transform
                )
...
class AsyncAvroDeserializer(AsyncBaseDeserializer):
    ...
    async def __deserialize(
        ...
        if subject is None:
            subject = (
                self._subject_name_func(ctx, writer_schema.get("name")) if ctx else None
            )
            if subject is not None:
                latest_schema = await self._get_reader_schema(subject)
        ...
        if migrations:
            obj_dict = schemaless_reader(
                payload, writer_schema, None, self._return_record_name
            )
            obj_dict = self._execute_migrations(ctx, subject, migrations, obj_dict)
        else:
            obj_dict = schemaless_reader(
                payload, writer_schema, reader_schema, self._return_record_name
            )

        def field_transformer(rule_ctx, field_transform, message):
            return transform(  # noqa: E731
                rule_ctx, reader_schema, message, field_transform
            )
