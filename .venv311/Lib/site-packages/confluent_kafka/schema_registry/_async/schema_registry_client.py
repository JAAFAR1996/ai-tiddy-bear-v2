        ...
        for i in range(self.max_retries + 1):
            try:
                self.token = await self.client.fetch_token(
                    url=self.token_endpoint, grant_type="client_credentials"
                )
                return
            except Exception as e:
                if i >= self.max_retries:
                    raise OAuthTokenError(
                        f"Failed to retrieve token after {self.max_retries} "
                        f"attempts due to error: {str(e)}"
                    )
                await asyncio.sleep(
                    full_jitter(self.retries_wait_ms, self.retries_max_wait_ms, i)
                    / 1000
                )
...
class _AsyncBaseRestClient(object):
    def __init__(self, conf: dict):
        ...
        if key is not None and client_cert is None:
            raise ValueError(
                "ssl.certificate.location required when" " configuring ssl.key.location"
            )
        ...
        if "basic.auth.user.info" in conf_copy:
            ...
            if len(userinfo) != 2:
                raise ValueError(
                    "basic.auth.user.info must be in the form"
        ...
        if cache_capacity is not None:
            if not isinstance(cache_capacity, (int, float)):
                raise TypeError(
                    "cache.capacity must be a number, not " + str(type(cache_capacity))
                )
        ...
        if cache_latest_ttl_sec is not None:
            if not isinstance(cache_latest_ttl_sec, (int, float)):
                raise TypeError(
                    "cache.latest.ttl.sec must be a number, not "
                    + str(type(cache_latest_ttl_sec))
                )
        ...
        if max_retries is not None:
            if not isinstance(max_retries, (int, float)):
                raise TypeError(
                    "max.retries must be a number, not " + str(type(max_retries))
                )
            self.max_retries = max_retries
        ...
        if retries_wait_ms is not None:
            if not isinstance(retries_wait_ms, (int, float)):
                raise TypeError(
                    "retries.wait.ms must be a number, not "
        ...
        if retries_max_wait_ms is not None:
            if not isinstance(retries_max_wait_ms, (int, float)):
                raise TypeError(
                    "retries.max.wait.ms must be a number, not "
        ...
        identity_pool = None
        self.bearer_auth_credentials_source = conf_copy.pop(
            "bearer.auth.credentials.source", None
        )
        if self.bearer_auth_credentials_source is not None:
            ...
            if self.bearer_auth_credentials_source in {"OAUTHBEARER", "STATIC_TOKEN"}:
                ...
                missing_headers = [
                    header for header in headers if header not in conf_copy
                ]
                if missing_headers:
                    raise ValueError(
                        "Missing required bearer configuration properties: {}".format(
                ...
                if not isinstance(logical_cluster, str):
                    raise TypeError(
                        "logical cluster must be a str, not "
                        + str(type(logical_cluster))
                    )

                identity_pool = conf_copy.pop("bearer.auth.identity.pool.id")
                if not isinstance(identity_pool, str):
                    raise TypeError(
                        "identity pool id must be a str, not "
                        + str(type(identity_pool))
                    )
            ...
            if self.bearer_auth_credentials_source == "OAUTHBEARER":
                ...
                missing_properties = [
                    prop for prop in properties_list if prop not in conf_copy
                ]
                if missing_properties:
                    raise ValueError(
                        "Missing required OAuth configuration properties: {}".format(
                ...
                if not isinstance(self.client_id, string_type):
                    raise TypeError(
                        "bearer.auth.client.id must be a str, not "
                ...
                missing_custom_properties = [
                    prop for prop in custom_bearer_properties if prop not in conf_copy
                ]
                ...
                if not callable(custom_function):
                    raise TypeError(
                        "bearer.auth.custom.provider.function must be a callable, not "
                ...
                if not isinstance(custom_config, dict):
                    raise TypeError(
                        "bearer.auth.custom.provider.config must be a dict, not "
                        + str(type(custom_config))
                    )

                self.bearer_field_provider = _AsyncCustomOAuthClient(
        ...
        if len(conf_copy) > 0:
            raise ValueError(
                "Unrecognized properties: {}".format(", ".join(conf_copy.keys()))
            )
...
class _AsyncRestClient(_AsyncBaseRestClient):
    ...
    async def handle_bearer_auth(self, headers: dict) -> None:
        ...
        headers["Authorization"] = "Bearer {}".format(
            bearer_fields["bearer.auth.token"]
        )
        headers["Confluent-Identity-Pool-Id"] = bearer_fields[
            "bearer.auth.identity.pool.id"
        ]
    ...
    async def send_request(
        ...
        for i, base_url in enumerate(self.base_urls):
            try:
                ...
                if (
                    not is_retriable(response.status_code)
                    or i == len(self.base_urls) - 1
                ):
                    break
            except Exception as e:
        ...
        # Schema Registry may return malformed output when it hits unexpected errors
            ...
            raise SchemaRegistryError(
                response.status_code,
                -1,
                "Unknown Schema Registry Error: " + str(response.content),
            )
    ...
    async def send_http_request(
        ...
        for i in range(self.max_retries + 1):
            ...
            await asyncio.sleep(
                full_jitter(self.retries_wait_ms, self.retries_max_wait_ms, i) / 1000
            )
        return response
...
class AsyncSchemaRegistryClient(object):
    ...
    async def register_schema(
        ...
        registered_schema = await self.register_schema_full_response(
            subject_name, schema, normalize_schemas
        )
    ...
    async def register_schema_full_response(
        ...
        s = (
            ...
            if registered_schema.schema.schema_str is not None
            else schema
        )
        self._cache.set_schema(
            subject_name, registered_schema.schema_id, registered_schema.guid, s
        )
    ...
    async def get_schema(
        ...
        response = await self._rest_client.get(
            "schemas/ids/{}".format(schema_id), query
        )

        registered_schema = RegisteredSchema.from_dict(response)

        self._cache.set_schema(
            subject_name, schema_id, registered_schema.guid, registered_schema.schema
        )
    ...
    async def lookup_schema(
        ...
        registered_schema = self._cache.get_registered_by_subject_schema(
            subject_name, schema
        )
    ...
    async def delete_subject(
        self, subject_name: str, permanent: bool = False
    ) -> List[int]:
        """
    ...
    async def get_version(
        ...
        registered_schema = self._cache.get_registered_by_subject_version(
    ...
    async def delete_version(
        self, subject_name: str, version: int, permanent: bool = False
    ) -> int:
        """
        ...
        if permanent:
            response = await self._rest_client.delete(
                "subjects/{}/versions/{}?permanent=true".format(
    ...
    async def set_compatibility(
        self, subject_name: Optional[str] = None, level: Optional[str] = None
    ) -> str:
        """
        ...
        return await self._rest_client.put(
            "config/{}".format(_urlencode(subject_name)),
    ...
    async def set_config(
        ...
        if subject_name is None:
            return await self._rest_client.put("config", body=config.to_dict())
