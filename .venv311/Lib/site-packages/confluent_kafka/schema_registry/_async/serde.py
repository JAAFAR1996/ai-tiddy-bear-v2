...
class AsyncBaseSerde(object):
    ...
    async def _get_reader_schema(
        self, subject: str, fmt: Optional[str] = None
        ...
        for index in range(len(rules)):
            ...
            try:
                ...
                self._run_action(
                    ...
                    self._get_on_failure(rule)
                    if message is None
                    else self._get_on_success(rule),
                    message,
                    None,
    ...
    def _get_rule_action(
        self, ctx: RuleContext, action_name: str
    ) -> Optional[RuleAction]:
...
class AsyncBaseDeserializer(AsyncBaseSerde, Deserializer):
    ...
    def _has_rules(self, rule_set: RuleSet, mode: RuleMode) -> bool:
        ...
        if mode in (RuleMode.UPGRADE, RuleMode.DOWNGRADE):
            return any(
                rule.mode == mode or rule.mode == RuleMode.UPDOWN
            ...
            return any(
                rule.mode == mode or rule.mode == RuleMode.WRITEREAD
    ...
    async def _get_migrations(
        ...
        for i in range(len(versions)):
            ...
            if version.schema.rule_set is not None and self._has_rules(
                version.schema.rule_set, migration_mode
            ):
