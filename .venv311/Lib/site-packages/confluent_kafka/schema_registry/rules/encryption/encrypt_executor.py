...
class FieldEncryptionExecutorTransform(object):
    ...
    def _get_or_create_kek(self, ctx: RuleContext) -> Kek:
        ...
        if kek is None:
            ...
            if not kms_type:
                raise RuleError(
                    f"no kms type found for {self._kek_name} during produce"
                )
            if not kms_key_id:
                raise RuleError(
                    f"no kms key id found for {self._kek_name} during produce"
                )
        ...
        if kms_type and kek.kms_type != kms_type:
            raise RuleError(
                f"found {self._kek_name} with kms type {kek.kms_type} "
    ...
    def _store_kek_to_registry(
        self, kek_id: KekId, kms_type: str, kms_key_id: str, shared: bool
    ) -> Optional[Kek]:
        try:
            return self._executor.client.register_kek(
                kek_id.name, kms_type, kms_key_id, shared
            )
        except Exception as e:
    ...
    def _get_or_create_dek(self, ctx: RuleContext, version: Optional[int]) -> Dek:
        ...
        if version is None or version == 0:
            version = 1
        dek_id = DekId(
            kek.name, ctx.subject, version, self._cryptor.dek_format, is_read
        )
    ...
    def _create_dek(
        self, dek_id: DekId, new_version: Optional[int], encrypted_dek: Optional[bytes]
    ) -> Dek:
        new_dek_id = DekId(
            dek_id.kek_name,
            dek_id.subject,
    ...
    def _retrieve_dek_from_registry(self, key: DekId) -> Optional[Dek]:
        try:
            ...
            if isinstance(e, SchemaRegistryError) and e.http_status_code == 404:
                return None
            raise RuleError(
                f"could not get dek for kek {key.kek_name}, subject {key.subject}"
            ) from e

    def _store_dek_to_registry(
        self, key: DekId, encrypted_dek: Optional[bytes]
    ) -> Optional[Dek]:
        try:
            encrypted_dek_str = (
            ...
            if isinstance(e, SchemaRegistryError) and e.http_status_code == 409:
                return None
            raise RuleError(
                f"could not register dek for kek {key.kek_name}, subject {key.subject}"
            ) from e
    ...
    def transform(
        self, ctx: RuleContext, field_ctx: FieldContext, field_value: Any
    ) -> Any:
        ...
        if ctx.rule_mode == RuleMode.WRITE:
            ...
            if plaintext is None:
                raise RuleError(
                    f"type {field_ctx.field_type} not supported for encryption"
                )
            version = None
            ...
            ciphertext = self._cryptor.encrypt(
                key_material_bytes, plaintext, Cryptor.EMPTY_AAD
            )
            if self._is_dek_rotated():
            ...
            dek = self._get_or_create_dek(ctx, version)
            key_material_bytes = dek.get_key_material_bytes()
            plaintext = self._cryptor.decrypt(
                key_material_bytes, ciphertext, Cryptor.EMPTY_AAD
            )
    ...
    def _prefix_version(self, version: int, ciphertext: bytes) -> bytes:
        return (
            bytes([_MAGIC_BYTE_V0]) + version.to_bytes(4, byteorder="big") + ciphertext
        )
    ...
    def _register_kms_client(
        self, kms_driver: KmsDriver, config: dict, kek_url: str
    ) -> KmsClient:
