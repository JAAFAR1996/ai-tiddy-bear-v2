...
class Dek:
    ...
    encrypted_key_material_bytes: Optional[bytes] = _attrs_field(
        init=False, eq=False, order=False, default=None
    )
    key_material: Optional[str] = _attrs_field(default=None)
    key_material_bytes: Optional[bytes] = _attrs_field(
        init=False, eq=False, order=False, default=None
    )
    ts: Optional[int] = _attrs_field(default=None)
    deleted: Optional[bool] = _attrs_field(default=None)
    _lock: threading.Lock = _attrs_field(
        factory=threading.Lock, init=False, eq=False, order=False
    )
    ...
    def get_encrypted_key_material_bytes(self) -> Optional[bytes]:
        ...
        if self.encrypted_key_material_bytes is None:
            with self._lock:
                if self.encrypted_key_material_bytes is None:
                    self.encrypted_key_material_bytes = base64.b64decode(
...
class DekRegistryClient(object):
    ...
    def get_kek(self, name: str, deleted: bool = False) -> Kek:
        ...
        response = self._rest_client.get(
            "/dek-registry/v1/keks/{}".format(urllib.parse.quote(name, safe="")), query
        )
        kek = Kek.from_dict(response)
    ...
    def register_dek(
        ...
        response = self._rest_client.post(
            "/dek-registry/v1/keks/{}/deks".format(urllib.parse.quote(kek_name)),
            request.to_dict(),
        )
