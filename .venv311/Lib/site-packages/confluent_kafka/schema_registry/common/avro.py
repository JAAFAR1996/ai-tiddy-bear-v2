...
def parse_schema_with_repo(
    schema_str: str, named_schemas: Dict[str, AvroSchema]
) -> AvroSchema:
...
def transform(
    ...
    if isinstance(schema, list):
        ...
        if schema_type == "array":
            return [
                transform(ctx, schema["items"], item, field_transform)
                for item in message
            ]
        elif schema_type == "map":
            return {
                key: transform(ctx, schema["values"], value, field_transform)
...
def _get_inline_tags_recursively(
    ns: str, name: str, schema: Optional[AvroSchema], tags: Dict[str, Set[str]]
):
    if schema is None:
        return
    if isinstance(schema, list):
        ...
        if schema_type == "array":
            ...
            for field in fields:
                ...
                if field_type is not None:
                    _get_inline_tags_recursively(
                        record_ns, record_name, field_type, tags
                    )
