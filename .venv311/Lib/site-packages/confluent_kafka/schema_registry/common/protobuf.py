...
def _proto_to_str(file_descriptor_proto: descriptor_pb2.FileDescriptorProto) -> str:
    ...
    return base64.standard_b64encode(file_descriptor_proto.SerializeToString()).decode(
        "ascii"
    )
...
def transform(
    ...
    if isinstance(message, dict):
        return {
            key: transform(ctx, descriptor, value, field_transform)
...
def _transform_field(
    ...
    try:
        ctx.enter_field(
            message, fd.full_name, fd.name, get_type(fd), get_inline_tags(fd)
        )
...
def _is_builtin(name: str) -> bool:
    return (
        name.startswith("confluent/")
        or name.startswith("google/protobuf/")
        or name.startswith("google/type/")
    )
