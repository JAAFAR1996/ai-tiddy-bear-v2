...
from ..util.typing import Protocol, Self, TypeGuard
...
if TYPE_CHECKING:
    from ..engine import Connection, CursorResult
    ...
    from . import coercions, elements, type_api
    from ._orm_types import DMLStrategyArgument, SynchronizeSessionArgument
    ...
    from .schema import Column, DefaultGenerator
    from .selectable import FromClause, _JoinTargetElement, _SelectIterable
...
class _DefaultDescriptionTuple(NamedTuple):
    ...
    def _from_column_default(
        ...
        return (
            ...
            if default
            and (default.has_arg or (not default.for_update and default.is_sentinel))
...
def _select_iterables(
    ...
    """
    return itertools.chain.from_iterable([c._select_iterable for c in elements])
...
def _exclusive_against(*names: str, **kw: Any) -> Callable[[_Fn], _Fn]:
    ...
    getters = [
        (name, operator.attrgetter(name), defaults.get(name, None)) for name in names
    ]
...
def _cloned_difference(a: Iterable[_CLE], b: Iterable[_CLE]) -> Set[_CLE]:
    all_overlap = set(_expand_cloned(a)).intersection(_expand_cloned(b))
    return {elem for elem in a if not all_overlap.intersection(elem._cloned_set)}
...
class _DialectArgView(MutableMapping[str, Any]):
    ...
    def __len__(self):
        return sum(
            len(args._non_defaults) for args in self.obj.dialect_options.values()
        )
        ...
        for k in kwargs:
            ...
            try:
                ...
                util.warn(
                    ...
                    "locate any SQLAlchemy dialect named %r" % (k, dialect_name)
                )
                ...
                if (
                    ...
                    raise exc.ArgumentError(
                        ...
                        "dialect %r on behalf of %r" % (k, dialect_name, self.__class__)
                    )
                else:
    ...
    def __init_subclass__(cls) -> None:
        ...
        cls._cache_attrs = tuple(
            sorted(
                d
                for d in dict_
                if not d.startswith("__") and d not in ("_cache_key_traversal",)
            )
        )
...
class Executable(roles.StatementRole):
    ...
    if TYPE_CHECKING:
        ...
        def _compile_w_cache(
            ...
            **kw: Any,
        ) -> Tuple[Compiled, Optional[Sequence[BindParameter[Any]]], CacheStats]:
            ...
    ...
    def options(self, *options: ExecutableOption) -> Self:
        ...
        self._with_options += tuple(
            coercions.expect(roles.ExecutableOptionRole, opt) for opt in options
        )
        return self
...
class _ColumnMetrics(Generic[_COL_co]):
    ...
    def embedded(
        self,
        target_set: Union[Set[ColumnElement[Any]], FrozenSet[ColumnElement[Any]]],
    ) -> bool:
...
class ColumnCollection(Generic[_COLKEY, _COL_co]):
        ...
        try:
            if isinstance(key, (tuple, slice)):
                if isinstance(key, slice):
                    cols = (
                        (sub_key, col) for (sub_key, col, _) in self._collection[key]
                    )
                else:
    ...
    def __setstate__(self, state: Dict[str, Any]) -> None:
        ...
        object.__setattr__(
            self,
            "_collection",
            [(k, c, _ColumnMetrics(self, c)) for (k, c) in state["_collection"]],
        )
                ...
                if len(current_intersection) > len(selected_intersection):
                    ...
                    selected_col_distance = sum(
                        [
                            ...
                            for sc in (selected_metrics.column._uncached_proxy_list())
                    ...
                    current_col_distance = sum(
                        [
                            sc._annotations.get("weight", 1)
                            for sc in (current_metrics.column._uncached_proxy_list())
                            if sc.shares_lineage(column)
                        ],
                    )
...
class DedupeColumnCollection(ColumnCollection[str, _NAMEDCOL]):
    ...
    def remove(self, column: _NAMEDCOL) -> None:
        if column not in self._colset:
            raise ValueError(
                "Can't remove column %r; column is not in this collection" % column
            )
        del self._index[column.key]
        self._colset.remove(column)
        self._collection[:] = [
            (k, c, metrics) for (k, c, metrics) in self._collection if c is not column
        ]
