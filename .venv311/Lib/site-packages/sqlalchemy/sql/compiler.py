...
from ..util.typing import Literal, Protocol, Self, TypedDict
from . import (
    base,
    coercions,
    crud,
...
if typing.TYPE_CHECKING:
    ...
    from .base import CompileState, Executable, _AmbiguousTableNameMap
    from .cache_key import CacheKey
    from .ddl import ExecutableDDLElement
    from .dml import Insert, Update, UpdateBase, UpdateDMLState, ValuesBase
    ...
    from .type_api import TypeDecorator, TypeEngine, UserDefinedType, _BindProcessorType
...
NO_LINTING, COLLECT_CARTESIAN_PRODUCTS, WARN_LINTING, FROM_LINTING = tuple(Linting)
...
class ilike_case_insensitive(
    ...
    def _with_binary_element_type(self, type_):
        return ilike_case_insensitive(self.element._with_binary_element_type(type_))
...
class SQLCompiler(Compiled):
    ...
    bindname_escape_characters: ClassVar[Mapping[str, str]] = util.immutabledict(
        {
            "%": "P",
            "(": "A",
            ")": "Z",
    ...
    def effective_returning(self) -> Optional[Sequence[ColumnElement[Any]]]:
        ...
        if self.implicit_returning:
            ...
            return [
                c for c in self.statement._all_selected_columns if is_column_element(c)
            ]
    ...
    def _process_positional(self):
        ...
        self.string = re.sub(self._positional_pattern, find_position, self.string)

        if self.escaped_bind_names:
            reverse_escape = {v: k for k, v in self.escaped_bind_names.items()}
            assert len(self.escaped_bind_names) == len(reverse_escape)
            self.positiontup = [reverse_escape.get(name, name) for name in positions]
        else:
        ...
        for bindparam in self.bind_names:
            ...
            if bindparam.type._is_tuple_type:
                inputsizes[bindparam] = [
                    lookup_type(typ) for typ in cast(TupleType, bindparam.type).types
                ]
            else:
        ...
        * literal_binds compiled with the literal_execute flag.  Used for
        ...
        if numeric_positiontup is not None:
            ...
            param_pos = {
                ...
                for num, key in enumerate(numeric_positiontup, self.next_numeric_pos)
            }
            # Can't use format here since % chars are not escaped.
        ...
        if _populate_self:
            ...
            self.positiontup = (
                list(expanded_state.positiontup or ()) if self.positional else None
            )
    ...
    def visit_typeclause(self, typeclause, **kw):
        ...
        return self.dialect.type_compiler_instance.process(typeclause.type, **kw)
    ...
    def visit_expression_clauselist(self, clauselist, **kw):
        ...
        try:
            ...
            return self._generate_delimited_list(clauselist.clauses, opstring, **kw)
    ...
    def _literal_execute_expanding_parameter_literal_binds(
        ...
        if not values:
            ...
            if typ_dialect_impl._has_bind_expression:
                raise NotImplementedError(
                    "bind_expression() on TupleType not supported with " "literal_binds"
                )
    ...
    def _literal_execute_expanding_parameter(self, name, parameter, values):
        ...
        if self.dialect._bind_typing_render_casts and typ_dialect_impl.render_bind_cast:
        ...
        if not values:
            ...
            replacement_expression = (
                ...
                % (
                    ", ".join(
                        _render_bindtemplate(to_update[i * len(tuple_element) + j][0])
        ...
        if from_linter and operators.is_comparison(binary.operator):
            if lateral_from_linter is not None:
                ...
                lateral_from_linter.edges.update(
                    itertools.product(
                        _de_clone(binary.left._from_objects + [enclosing_lateral]),
                        _de_clone(binary.right._from_objects + [enclosing_lateral]),
                    )
                )
            else:
    ...
    def visit_icontains_op_binary(self, binary, operator, **kw):
        ...
        binary.right = percent.concat(ilike_case_insensitive(binary.right)).concat(
            percent
        )
    ...
    def _truncated_identifier(self, ident_class: str, name: _truncated_label) -> str:
    ...
    def _dispatch_independent_ctes(self, stmt, kw):
        ...
        for cte, opt in zip(stmt._independent_ctes, stmt._independent_ctes_opts):
        ...
        if asfrom:
            ...
            if name:
                ...
                v = "%s(%s)%s (%s)" % (
                    ...
                    (
                        ", ".join(
                            c._compiler_dispatch(self, **kw) for c in element.columns
                        )
                    ),
                )
            else:
    ...
    def _add_to_result_map(
        ...
        self._result_columns.append(ResultColumnsEntry(keyname, name, objects, type_))
    ...
    **kw)`` and returns a select object.   this is used for structural changes
    ...
    def _setup_select_hints(self, select: Select[Any]) -> Tuple[str, _FromHintsType]:
        byfrom = {
    ...
    def fetch_clause(
        ...
        **kw,
        ...
        if select._offset_clause is not None:
            ...
            if use_literal_execute_for_simple_int and select._simple_int_clause(
                offset_clause
            ):
        ...
        **kwargs,
        ...
        if asfrom or ashint:
            ...
            if use_schema and effective_schema:
                ...
                if (
                    (enclosing_alias is None or enclosing_alias.element is not table)
    ...
    def _deliver_insertmanyvalues_batches(
        ...
        if not imv.sentinel_param_keys:
            ...
            _sentinel_from_params = operator.itemgetter(*imv.sentinel_param_keys)
        ...
        if max_params:
            ...
            num_params_outside_of_batch = total_num_of_params - num_params_per_batch
            batch_size = min(
                batch_size,
                ((max_params - num_params_outside_of_batch) // num_params_per_batch),
            )

        batches = cast("List[Sequence[Any]]", list(parameters))
        compiled_batches = cast("List[Sequence[Any]]", list(compiled_parameters))

        processed_setinputsizes: Optional[_GenericSetInputSizesType] = None
        batchnum = 1
        total_batches = lenparams // batch_size + (1 if lenparams % batch_size else 0)
        ...
        if rst:
            insert_crud_params = [
                (col, key, rst(expr), st) for col, key, expr, st in insert_crud_params
            ]
        ...
        if not self.positional:
            ...
            def apply_placeholders(keys, formatted):
                for key in keys:
                    ...
                    formatted = formatted.replace(
                        ...
                        self.bindtemplate % {"name": f"{key}__EXECMANY_INDEX__"},
                    )
                return formatted
            ...
            base_parameters = {
                key: parameters[0][key] for key in all_keys.difference(keys_to_replace)
            }
        ...
        while batches:
            ...
            if self.positional:
                ...
                if num_ins_params == len(batch[0]):
                    ...
                    batch_iterator = (
                        b[expand_pos_lower_index:expand_pos_upper_index] for b in batch
                    )
                ...
                replaced_parameters = (
                    extra_params_left + replaced_parameters + extra_params_right
                )
                ...
                for i, param in enumerate(batch):
                    fmv = formatted_values_clause.replace("EXECMANY_INDEX__", str(i))
                    if imv.embed_values_counter:
...
class StrSQLCompiler(SQLCompiler):
    ...
    def visit_sequence(self, sequence, **kw):
        return f"<next sequence value: {self.preparer.format_sequence(sequence)}>"
...
class DDLCompiler(Compiled):
    ...
    def _prepared_index_name(self, index: Index, include_schema: bool = False) -> str:
...
class GenericTypeCompiler(TypeCompiler):
    ...
    def visit_NVARCHAR(self, type_: sqltypes.NVARCHAR, **kw: Any) -> str:
        return self._render_string_type("NVARCHAR", type_.length, type_.collation)
    ...
    def visit_small_integer(self, type_: sqltypes.SmallInteger, **kw: Any) -> str:
    ...
    def visit_type_decorator(self, type_: TypeDecorator[Any], **kw: Any) -> str:
        return self.process(type_.type_engine(self.dialect), **kw)

    def visit_user_defined(self, type_: UserDefinedType[Any], **kw: Any) -> str:
