...
class Table(DialectKWArgs, HasSchemaAttr, TableClause, inspection.Inspectable["Table"]):
    ...
    def __init__(
        ...
        *args: SchemaItem,
        ...
        listeners: Optional[_typing_Sequence[Tuple[str, Callable[..., Any]]]] = None,
        ...
        **kw: Any,
        ...
        PrimaryKeyConstraint(_implicit_generated=True)._set_parent_with_dispatch(self)
        ...
        self._init_items(
            *args,
            allow_replacements=extend_existing or keep_existing or autoload_with,
            all_names={},
        )
    ...
    def foreign_key_constraints(self) -> Set[ForeignKeyConstraint]:
        ...
        """
        return {
            fkc.constraint for fkc in self.foreign_keys if fkc.constraint is not None
        }
            ...
            if isinstance(const, ForeignKeyConstraint):
                ...
                if referred_schema_fn:
                    ...
                    fk_constraint_schema = (
                        actual_schema if referred_schema == self.schema else None
                    )
                table.append_constraint(
                    const._copy(schema=fk_constraint_schema, target_table=table)
                )
...
class Column(DialectKWArgs, SchemaItem, ColumnClause[_T]):
    ...
    def __init__(
        ...
        __type_pos: Optional[Union[_TypeEngineArgument[_T], SchemaEventTarget]] = None,
        ...
        **dialect_kwargs: Any,
        ...
        if server_onupdate is not None:
            if isinstance(server_onupdate, FetchedValue):
                ...
                server_onupdate = DefaultClause(server_onupdate, for_update=True)
                l_args.append(server_onupdate)
    ...
    def _make_proxy(
        ...
        compound_select_cols: Optional[_typing_Sequence[ColumnElement[Any]]] = None,
        **kw: Any,
        ...
        fk = [
            ...
            for f, col in [
                (fk, fk._resolve_column(raiseerr=False)) for fk in self.foreign_keys
            ]
        ]
        ...
        try:
            c = self._constructor(
                ...
                _proxies=(
                    list(compound_select_cols) if compound_select_cols else [self]
                ),
                *fk,
            )
...
class Constraint(DialectKWArgs, HasConditionalDDL, SchemaItem):
    ...
    def _should_create_for_compiler(self, compiler: DDLCompiler, **kw: Any) -> bool:
...
class ColumnCollectionMixin:
    ...
    def __init__(
        ...
        *columns: _DDLColumnArgument,
        ...
        _gather_expressions: Optional[List[Union[str, ColumnElement[Any]]]] = None,
    ) -> None:
...
class PrimaryKeyConstraint(ColumnCollectionConstraint):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        ...
        if self._columns and table_pks and set(table_pks) != set(self._columns):
    ...
    def _autoincrement_column(self) -> Optional[Column[int]]:
        def _validate_autoinc(col: Column[Any], autoinc_true: bool) -> bool:
            if col.type._type_affinity is None or not issubclass(
                ...
                not isinstance(col.default, (type(None), Sequence)) and not autoinc_true
            ):
                return False
            elif (
...
class MetaData(HasSchemaAttr):
    ...
    def __init__(
        ...
        if schema is not None and not isinstance(schema, str):
            raise exc.ArgumentError(
                "expected schema argument to be a string, " f"got {type(schema)}."
            )
        self.tables = util.FacadeDict()
        self.schema = quoted_name.construct(schema, quote_schema)
        self.naming_convention = (
            naming_convention if naming_convention else DEFAULT_NAMING_CONVENTION
        )
        if info:
            self.info = info
    ...
    def _add_table(self, name: str, schema: Optional[str], table: Table) -> None:
        key = _get_table_key(name, schema)
    ...
    def reflect(
        ...
        schema: Optional[str] = ...,
        views: bool = ...,
        only: Union[_typing_Sequence[str], Callable[[str, MetaData], bool], None] = ...,
        ...
        with inspection.inspect(bind)._inspection_context() as insp:
            ...
            if only is None:
                ...
                load = [
                    name
                    for name, schname in zip(available, available_w_schema)
                    if (extend_existing or schname not in current) and only(name, self)
                ]
            else:
...
class Identity(IdentityOptions, FetchedValue, SchemaItem):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        ...
        if parent.autoincrement is False:
            raise exc.ArgumentError(
                "A column with an Identity object cannot specify " "autoincrement=False"
            )
        self.column = parent
