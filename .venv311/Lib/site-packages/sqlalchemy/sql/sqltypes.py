...
if TYPE_CHECKING:
    ...
    from ._typing import _ColumnExpressionArgument, _CreateDropBind, _TypeEngineArgument
...
class String(Concatenable, TypeEngine[str]):
    ...
    def bind_processor(self, dialect: Dialect) -> Optional[_BindProcessorType[str]]:
        return None
...
class SchemaType(SchemaEventTarget, TypeEngineMixin):
    ...
    def _on_table_create(self, target: Any, bind: _CreateDropBind, **kw: Any) -> None:
    ...
    def _on_table_drop(self, target: Any, bind: _CreateDropBind, **kw: Any) -> None:
        if not self._is_impl_for_variant(bind.dialect, kw):
            return
    ...
    def _on_metadata_drop(self, target: Any, bind: _CreateDropBind, **kw: Any) -> None:
        if not self._is_impl_for_variant(bind.dialect, kw):
            return
...
class ARRAY(SchemaEventTarget, Indexable, Concatenable, TypeEngine[Sequence[_T]]):
        ...
        def _setup_getitem(
            ...
            if isinstance(index, slice):
                ...
                if arr_type.dimensions is None or arr_type.dimensions == 1:
                    ...
                    return_type = arr_type.adapt(arr_type.__class__, **adapt_kw)
...
def _resolve_value_to_type(value: Any) -> TypeEngine[Any]:
    ...
    if _result_type is False:
        ...
        return _result_type._resolve_for_literal(value)  # type: ignore [union-attr]
