...
from typing import Any, Callable, Deque, Dict, Iterable, Optional, Set, Tuple, Type
...
class HasShallowCopy(HasTraverseInternals):
    ...
    def _generate_shallow_to_dict(
        ...
        code = ",\n".join(
            f"    '{attrname}': self.{attrname}" for attrname, _ in internal_dispatch
        )
...
class TraversalComparatorStrategy(HasTraversalDispatch, util.MemoizedSlots):
    ...
    def visit_string_multi_dict(
        ...
        for lk, rk in zip_longest(
            ...
            if lhc and rhc:
                if lv._gen_cache_key(self.anon_map[0], []) != rv._gen_cache_key(
                    self.anon_map[1], []
                ):
                    return COMPARE_FAILED
            elif lhc != rhc:
