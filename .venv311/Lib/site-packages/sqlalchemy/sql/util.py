...
from . import coercions, operators, roles, visitors
...
from .base import _expand_cloned, _from_objects
...
from .elements import _label_reference, _textual_label_reference
...
if typing.TYPE_CHECKING:
    ...
    from ._typing import _EquivalentColumnMap, _LimitOffsetType, _TypeEngineArgument
    from .elements import BinaryExpression, TextClause
    from .selectable import Selectable, _JoinTargetElement, _SelectIterable
...
def visit_binary_product(
    fn: Callable[[BinaryExpression[Any], ColumnElement[Any], ColumnElement[Any]], None],
...
def unwrap_order_by(clause: Any) -> Any:
    ...
    while stack:
        ...
        if isinstance(t, ColumnElement) and (
            ...
            if isinstance(t, Label) and not isinstance(t.element, ScalarSelect):
                t = t.element
...
class _repr_params(_repr_base):
    ...
    def _repr_param_dict(self, params: _CoreSingleExecuteParams) -> str:
        ...
        if items_second_batch:
            text = "{%s" % (
                ", ".join(
                    f"{key!r}: {trunc(value)}" for key, value in items_first_batch
                )
            )
            text += f" ... {trunclen} parameters truncated ... "
            text += "%s}" % (
                ", ".join(
                    f"{key!r}: {trunc(value)}" for key, value in items_second_batch
                )
            )
        else:
            text = "{%s}" % (
    ...
    if clauses:
        ...
        def visit_binary(binary):
            if binary.operator == operators.eq:
                cols = util.column_set(
                    chain(*[c.proxy_set for c in cset_no_text.difference(omit)])
                )
...
def criterion_as_pairs(
    ...
    def visit_binary(binary):
        ...
        if consider_as_foreign_keys:
            ...
            if isinstance(binary.left, Column) and isinstance(binary.right, Column):
...
class ClauseAdapter(visitors.ReplacingExternalTraversal):
    ...
    def replace(
        ...
        return self._corresponding_column(col, require_embedded=True)  # type: ignore
    ...
    * An adapt_required option, used by eager loading to indicate that
    ...
    class _IncludeExcludeMapping:
        ...
        def __getitem__(self, key):
            if (self.parent.include_fn and not self.parent.include_fn(key)) or (
...
def _make_slice(
    ...
    if start is not None and stop is not None:
        offset_clause = _offset_or_limit_clause_asint_if_possible(offset_clause)
