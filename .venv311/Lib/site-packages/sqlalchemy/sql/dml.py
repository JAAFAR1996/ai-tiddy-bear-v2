...
from ._typing import _TP, _unexpected_kw, is_column_element, is_named_from_clause
from .base import (
    ColumnCollection,
    ColumnSet,
    CompileState,
...
if TYPE_CHECKING:
    ...
    from .selectable import Select, Selectable, _ColumnsClauseElement, _SelectIterable
...
class DMLState(CompileState):
    ...
    def get_entity_description(cls, statement: UpdateBase) -> Dict[str, Any]:
        return {
            "name": (
                statement.table.name if is_named_from_clause(statement.table) else None
            ),
    ...
    def _get_multi_crud_kv_pairs(
        ...
        return [
            {coercions.expect(roles.DMLColumnRole, k): v for k, v in mapping.items()}
...
class UpdateBase(
        ...
        *cols: _DMLColumnArgument,
        ...
        if supplemental_cols:
            ...
            supplemental_col_tup = (
                coercions.expect(roles.ColumnsClauseRole, c) for c in supplemental_cols
            )
            ...
            if self._supplemental_returning is None:
                ...
                self._supplemental_returning = tuple(
                    util.unique_list(
                        self._supplemental_returning + tuple(supplemental_col_tup)
                    )
                )
    ...
    def exported_columns(
        ...
        return ColumnCollection(
            (c.key, c) for c in self._all_selected_columns if is_column_element(c)
        ).as_readonly()
...
class Insert(ValuesBase):
    ...
    def from_select(
        ...
        self._select_names = [
            coercions.expect(roles.DMLColumnRole, name, as_key=True) for name in names
        ]
        self._inline = True
