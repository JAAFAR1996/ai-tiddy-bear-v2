...
from . import cache_key, coercions, operators, roles, traversals, type_api, visitors
from ._typing import (
    _T,
    _TP,
...
from .visitors import InternalTraversal, _TraverseInternalsType, prefix_anon_map
...
if TYPE_CHECKING:
    ...
    from .base import ExecutableOption, ReadOnlyColumnCollection, _AmbiguousTableNameMap
...
class CTE(
    ...
    def union_all(self, *other: _SelectStatementForCompoundArgument[Any]) -> CTE:
...
class SelectBase(
    ...
    def _generate_fromclause_column_proxies(
        ...
        foreign_keys: Set[KeyedColumnElement[Any]],
        *,
        proxy_compound_columns: Optional[Iterable[Sequence[ColumnElement[Any]]]] = None,
    ) -> None:
        raise NotImplementedError()
        ...
        *clauses: _ColumnExpressionOrStrLabelArgument[Any],
        ...
        if not clauses and __first is None:
            ...
            self._order_by_clauses += tuple(
                coercions.expect(roles.OrderByRole, clause, apply_propagate_attrs=self)
        ...
        *clauses: _ColumnExpressionOrStrLabelArgument[Any],
        ...
        if not clauses and __first is None:
            ...
            self._group_by_clauses += tuple(
                coercions.expect(roles.GroupByRole, clause, apply_propagate_attrs=self)
                for clause in (__first,) + clauses
            )
        return self
        ...
        *,
        ...
        extra_col_iterator = zip(
            *[
                ...
                for dd, stmt in [
                    ({"weight": i + 1}, stmt) for i, stmt in enumerate(self.selects)
                ]
            ]
        )
        ...
        if froms:
            ...
            if ambiguous_table_name_map is not None:
                ambiguous_table_name_map.update(
                    (
                        fr.name,
                        _anonymous_label.safe_construct(hash(fr.name), fr.name),
                    )
                    for item in froms
...
class Select(
    ...
    def __init__(self, *entities: _ColumnsClauseArgument[Any], **dialect_kw: Any):
    ...
    def selected_columns(
        ...
        cc: ColumnCollection[str, ColumnElement[Any]] = ColumnCollection(
            [(conv(c), c) for c in self._all_selected_columns if is_column_element(c)]
        )
        ...
        *,
        ...
        if proxy_compound_columns:
            columns._populate_separate_keys(
                ...
                for c, extra_cols in zip(self.column_args, proxy_compound_columns)
            )
        else:
            columns._populate_separate_keys(
                c._make_proxy(
