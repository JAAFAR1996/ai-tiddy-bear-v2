...
class SQLCoreOperations(Generic[_T_co], ColumnOperators, TypingOnly):
    ...
    if typing.TYPE_CHECKING:
        ...
        def in_(
            self,
            other: Union[Iterable[Any], BindParameter[Any], roles.InElementRole],
        ) -> BinaryExpression[bool]:
            ...
...
class TypeClause(DQLDMLClauseElement):
    ...
    _traverse_internals: _TraverseInternalsType = [("type", InternalTraversal.dp_type)]
    type: TypeEngine[Any]
...
class True_(SingletonConstant, roles.ConstExprRole[bool], ColumnElement[bool]):
    ...
    @classmethod
    def _ifnone(cls, other: Optional[ColumnElement[Any]]) -> ColumnElement[Any]:
        if other is None:
    ...
    def _construct_for_op(
        ...
        *,
        ...
        if operators.is_associative(op):
            ...
            multi = False
            if getattr(left, "operator", None) is op and type_._compare_type_affinity(
                left.type
            ):
                multi = True
            ...
            if getattr(right, "operator", None) is op and type_._compare_type_affinity(
                right.type
            ):
                multi = True
        ...
        **kw: Any,
        ...
        lcc, convert_clauses = cls._process_clauses_for_boolean(
            ...
            [
                ...
                for clause in util.coerce_generator_arg((initial_clause,) + clauses)
            ],
        )
...
class Tuple(ClauseList, ColumnElement[typing_Tuple[Any, ...]]):
    ...
    _traverse_internals: _TraverseInternalsType = ClauseList._traverse_internals + []
...
class TypeCoerce(WrapsColumnExpression[_T]):
    ...
    def self_group(self, against: Optional[OperatorType] = None) -> TypeCoerce[_T]:
...
class Label(roles.LabeledColumnExprRole[_T], NamedColumn[_T]):
    ...
    def __init__(
        ...
        self.type = (
            type_api.to_instance(type_) if type_ is not None else self._element.type
        )
...
class _memoized_property_but_not_nulltype(util.memoized_property["TypeEngine[_T]"]):
