...
from ..sql import and_, cache_key, coercions, roles, traversals, visitors
from ..sql.base import _generative
from ..util.typing import Final, Literal, Self
from . import util as orm_util
from ._typing import (
    insp_is_aliased_class,
    insp_is_attribute,
    insp_is_mapper,
    insp_is_mapper_property,
)
...
from .util import AliasedInsp, _orm_full_deannotate
...
if typing.TYPE_CHECKING:
    ...
    from ..sql.cache_key import CacheKey, _CacheKeyTraversalType
    from ._typing import _EntityType, _InternalEntityType
    from .context import ORMCompileState, QueryContext, _MapperEntity
    from .interfaces import MapperProperty, ORMOption, _StrategyKey
...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def joinedload(
        ...
        loader = self._set_relationship_strategy(
            ...
            {"lazy": "joined"},
            opts=(
                {"innerjoin": innerjoin} if innerjoin is not None else util.EMPTY_DICT
            ),
        )
        return loader
    ...
    def _adjust_for_extra_criteria(self, context: QueryContext) -> Load:
        ...
        def process(
            ...
            cloned_opt._extra_criteria = tuple(
                replacement_cache_key._apply_params_to_element(orig_cache_key, crit)
...
class _TokenStrategyLoad(_LoadElement):
    ...
    def _prepare_for_compile_state(
        ...
        return [
            ("loader", natural_path)
            for natural_path in (
                cast(TokenRegistry, effective_path)._generate_natural_for_superclasses()
            )
        ]
