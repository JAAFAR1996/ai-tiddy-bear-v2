...
class MappedColumn(
    ...
    def columns_to_assign(self) -> List[Tuple[Column[Any], int]]:
        return [
            (
                self.column,
                (self._sort_order if self._sort_order is not _NoArg.NO_ARG else 0),
            )
        ]
    ...
    def _init_column_for_annotation(
        ...
        if is_fwd_ref(argument, check_generic=True, check_for_plain_string=True):
        ...
        if sqltype._isnull and not self.column.foreign_keys:
            ...
            for check_type in checks:
                ...
                if isinstance(our_type, TypeEngine) or (
                    isinstance(our_type, type) and issubclass(our_type, TypeEngine)
                ):
