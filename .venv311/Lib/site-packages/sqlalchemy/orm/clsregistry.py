...
if TYPE_CHECKING:
    from ..sql.schema import MetaData, Table
        ...
        try:
            ...
            if not isinstance(module, _ModuleMarker):
                raise exc.InvalidRequestError(
                    f'name "{classname}" matches both a ' "class name and a module name"
                ) from ae
            else:
                raise
    ...
    def add_class(self, name: str, cls: Type[Any]) -> None:
        if name in self.contents:
            ...
            try:
                ...
                if not isinstance(existing, _MultipleClassMarker):
                    raise exc.InvalidRequestError(
                        f'name "{name}" matches both a ' "class name and a module name"
                    ) from ae
                else:
                    raise
        else:
