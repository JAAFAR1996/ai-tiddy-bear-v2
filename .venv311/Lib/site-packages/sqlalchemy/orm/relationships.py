...
from ..sql._typing import _ColumnExpressionArgument, _HasClauseElement
from ..sql.annotation import _safe_annotate
from ..sql.elements import ColumnClause, ColumnElement
...
from ..util.typing import (
    ...
    resolve_name_to_real_class_name,
)
from . import attributes, strategy_options
from ._typing import insp_is_aliased_class, is_has_collection_adapter
from .base import (
    DynamicMapped,
...
if typing.TYPE_CHECKING:
    from ..sql._typing import _CoreAdapterProto, _EquivalentColumnMap, _InfoType
    from ..sql.annotation import SupportsAnnotations, _AnnotationDict
    from ..sql.elements import BinaryExpression, BindParameter, ClauseElement
    ...
    from .clsregistry import _class_resolver, _ModNS
    ...
    from .util import AliasedClass, AliasedInsp
...
class RelationshipProperty(
        ...
        def __eq__(self, other: Any) -> ColumnElement[bool]:  # type: ignore[override]  # noqa: E501
            ...
            * Comparisons against ``None`` given in a one-to-many
            ...
            if other is None or isinstance(other, expression.Null):
                ...
                return _orm_annotate(
                    self.property._optimized_compare(other, adapt_source=self.adapter)
                )
    ...
    def _optimized_compare(
        ...
        if self.secondary is not None and alias_secondary:
            criterion = ClauseAdapter(self.secondary._anonymous_fromclause()).traverse(
                criterion
            )
        ...
        for instance_state, c in tuples:
            ...
            if not instance_mapper.isa(self.mapper.class_manager.mapper):
                raise AssertionError(
                    ...
                    "of type '%s'" % (self.key, self.parent.class_, c.__class__)
                )
    ...
    def _add_reverse_property(self, key: str) -> None:
        ...
        if not other.mapper.common_parent(self.parent):
            raise sa_exc.ArgumentError(
                ...
                "does not reference mapper %s" % (key, self, other, self.parent)
            )
    ...
    def _process_dependent_arguments(self) -> None:
        ...
        for attr in "primaryjoin", "secondaryjoin":
            ...
            if val is not None:
                rel_arg.resolved = _orm_deannotate(
                    coercions.expect(roles.ColumnArgumentRole, val, argname=attr)
                )
        ...
        if (
            ...
            self.order_by = tuple(
                coercions.expect(roles.ColumnArgumentRole, x, argname="order_by")
        ...
        self._user_defined_foreign_keys = util.column_set(
            coercions.expect(roles.ColumnArgumentRole, x, argname="foreign_keys")
            for x in util.to_column_set(init_args.foreign_keys.resolved)
        )

        self.remote_side = util.column_set(
            coercions.expect(roles.ColumnArgumentRole, x, argname="remote_side")
        ...
        if (
            ...
            raise sa_exc.ArgumentError(
                ...
                % {
                    "rel": self,
                    "direction": (
                        "many-to-one" if self.direction is MANYTOONE else "many-to-many"
                    ),
    ...
    def _persists_for(self, mapper: Mapper[Any]) -> bool:
        ...
        return (
            self.key in mapper.relationships and mapper.relationships[self.key] is self
        )
                ...
                for m in check:
                    if m.has_property(backref_key) and not m.concrete:
                        raise sa_exc.ArgumentError(
                            ...
                            "name exists on mapper '%s'" % (backref_key, self, m)
                        )
    ...
    def _create_joins(
        ...
        aliased = aliased or (
            ...
            and (
                source_selectable is not self.parent._with_polymorphic_selectable
...
class JoinCondition:
    ...
    def _annotate_present_fks(self) -> None:
        ...
        def visit_binary(binary: BinaryExpression[Any]) -> None:
            if not isinstance(binary.left, sql.ColumnElement) or not isinstance(
    ...
    def _check_remote_side(self) -> None:
        if not self.local_remote_pairs:
            ...
            not_target = util.column_set(self.parent_persist_selectable.c).difference(
                self.child_persist_selectable.c
            )
            ...
            for _, rmt in self.local_remote_pairs:
                if rmt in not_target:
                    util.warn(
                        ...
                        "of the relationship refer to the same tables." % (rmt,)
                    )
    ...
    _track_overlapping_sync_targets: weakref.WeakKeyDictionary[
        ColumnElement[Any],
        weakref.WeakKeyDictionary[RelationshipProperty[Any], ColumnElement[Any]],
    ...
    def _warn_for_conflicting_sync_targets(self) -> None:
        ...
        for from_, to_ in [(from_, to_) for (from_, to_) in self.synchronize_pairs] + [
            ...
            if to_ not in self._track_overlapping_sync_targets:
                self._track_overlapping_sync_targets[to_] = weakref.WeakKeyDictionary(
                    {self.prop: from_}
                )
            else:
                other_props = []
    ...
    def _gather_join_annotations(self, annotation: str) -> Set[ColumnElement[Any]]:
        ...
        if self.secondaryjoin is not None:
            s.update(
                self._gather_columns_with_annotation(self.secondaryjoin, annotation)
            )
