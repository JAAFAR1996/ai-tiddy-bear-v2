...
from ..util import HasMemoized, HasMemoized_ro_memoized_attribute
...
if TYPE_CHECKING:
    from ..engine import Row, RowMapping
    from ..sql._typing import _ColumnExpressionArgument, _EquivalentColumnMap
...
class Mapper(
    ...
    def __init__(
        ...
        inherit_foreign_keys: Optional[Sequence[_ORMColumnExprArgument[Any]]] = None,
    ...
    def _reconcile_prop_with_incoming_columns(
        ...
        if incoming_prop and (
            self.concrete or not isinstance(existing_prop, properties.ColumnProperty)
        ):
        ...
        self._log(
            "inserting column to existing list " "in properties.ColumnProperty %s",
            key,
        )
    ...
    def _version_id_has_server_side_value(self) -> bool:
        ...
        if vid_col is None:
            ...
            return vid_col.server_default is not None or (
                ...
                and (not vid_col.default.is_scalar and not vid_col.default.is_callable)
            )
    ...
    def _insert_cols_evaluating_none(self):
        return {
            table: frozenset(col for col in columns if col.type.should_evaluate_none)
        ...
        return {
            table: frozenset(
                [
                    ...
                    or (col.default is not None and col.default.is_clause_element)
                ]
            )
            for table, columns in self._cols_by_table.items()
        }
        ...
        return {
            table: frozenset(
                [
                    ...
                    or (col.onupdate is not None and col.onupdate.is_clause_element)
                ]
            )
            for table, columns in self._cols_by_table.items()
        }
    ...
    def _pk_synonyms(self) -> Dict[str, str]:
        ...
        return {
            ...
            if isinstance(syn, descriptor_props.SynonymProperty) and syn.name in pk_keys
        }
    ...
    def _filter_properties(self, type_: Type[_MP]) -> util.ReadOnlyProperties[_MP]:
        self._check_configure()
    ...
    def _canload(self, state: InstanceState[Any], allow_subtypes: bool) -> bool:
    ...
    def primary_key_from_instance(self, instance: _O) -> Tuple[Any, ...]:
        ...
        """
        state = attributes.instance_state(instance)
        identity_key = self._identity_key_from_state(state, PassiveFlag.PASSIVE_OFF)
    ...
    ) -> Iterator[Tuple[object, Mapper[Any], InstanceState[Any], _InstanceDict]]:
