...
from ..util.langhelpers import Missing, MissingOr
from ..util.typing import Literal
from . import base
from .collections import collection, collection_adapter

if TYPE_CHECKING:
    from ..sql.elements import ColumnElement
    from . import AttributeEventToken, Mapper
...
class _PlainColumnGetter(Generic[_KT]):
    ...
    def __call__(self, value: _KT) -> MissingOr[Union[_KT, Tuple[_KT, ...]]]:
        ...
        key: List[_KT] = [
            m._get_state_attr_by_column(state, state.dict, col) for col in self._cols(m)
        ]
        if self.composite:
            return tuple(key)
        else:
