...
from ..sql import coercions, dml, expression, roles, select, sqltypes
...
from . import attributes, context, evaluator
from . import exc as orm_exc
from . import loading, persistence
...
if TYPE_CHECKING:
    from ..engine import Connection, cursor
    from ..engine.interfaces import _CoreAnyExecuteParams
    from ._typing import (
        DMLStrategyArgument,
        OrmExecuteOptionsParameter,
        SynchronizeSessionArgument,
    )
    from .mapper import Mapper
    from .session import ORMExecuteState, Session, SessionTransaction, _BindArguments
...
class BulkORMUpdate(BulkUDCompileState, UpdateDMLState):
    ...
    def _do_post_synchronize_fetch(cls, session, statement, result, update_options):
        ...
        if returned_defaults_rows:
            ...
            matched_rows = [
                tuple(row) + (update_options._identity_token,) for row in pk_rows
            ]
        else:
        ...
        if can_use_returning is not False:
            ...
            can_use_returning = (
                ...
                and self.can_use_returning(
                    ...
                    is_delete_using=compiler._annotations.get("is_delete_using", False),
                )
            )
