...
from ..sql._typing import _TP, is_dml, is_insert_update, is_select_base
...
from ..sql.elements import GroupedElement, TextClause
...
if TYPE_CHECKING:
    ...
    from ._typing import OrmExecuteOptionsParameter, _InternalEntityType
    from .loading import PostLoad
    from .mapper import Mapper
    from .query import Query
    from .session import Session, _BindArguments
...
_orm_load_exec_options = util.immutabledict({"_result_disable_adapt_to_context": True})
...
class ORMCompileState(AbstractORMCompileState):
    ...
    def orm_pre_session_exec(
        ...
        if (
            ...
            execution_options: util.immutabledict[str, Any] = execution_options.union(
                {
                    "compiled_cache": None,
                    "_cache_disable_reason": "excess depth for " "ORM loader options",
                }
            )
    ...
    def _create_with_polymorphic_adapter(self, ext_info, selectable):
        ...
        if (
            ...
            and ext_info.mapper.persist_selectable not in self._polymorphic_adapters
        ):
...
class _DMLReturningColFilter:
    ...
    def __init__(self, target_mapper, immediate_dml_mapper):
        if (
            ...
            and target_mapper.local_table is not immediate_dml_mapper.local_table
        ):
...
class ORMFromStatementCompileState(ORMCompileState):
    ...
    def setup_dml_returning_compile_state(self, dml_mapper):
        ...
        if self.statement.is_insert:
            adapter = _DMLBulkInsertReturningColFilter(target_mapper, dml_mapper)
        elif self.statement.is_update or self.statement.is_delete:
            adapter = _DMLUpdateDeleteReturningColFilter(target_mapper, dml_mapper)
        else:
            adapter = None
...
class ORMSelectCompileState(ORMCompileState, SelectState):
    ...
    def _create_orm_context(
        ...
        **kw: Any,
        ...
        if toplevel and (
            select_statement._with_options or select_statement._memoized_select_entities
        ):
            for memoized_entities in select_statement._memoized_select_entities:
                for opt in memoized_entities._with_options:
                    if opt._is_compile_state:
                        opt.process_compile_state_replaced_entities(
                            ...
                            [
                                ent
                                for ent in self._memoized_entities[memoized_entities]
        ...
        if query._where_criteria:
            ...
            if current_adapter:
                self._where_criteria = tuple(
                    current_adapter(crit, True) for crit in self._where_criteria
                )
        ...
        return cls._normalize_froms(
            itertools.chain.from_iterable(
                (
                    ...
                    else [element._annotations["parententity"].__clause_element__()]
                )
                for element in statement._raw_columns
            )
        )
    ...
    def _only_entity_zero(self, rationale=None):
        if len(self._entities) > 1:
            raise sa_exc.InvalidRequestError(
                rationale
                or "This operation requires a Query " "against a single mapper."
            )
    ...
    def _all_equivs(self):
        ...
        for memoized_entities in self._memoized_entities.values():
            for ent in [
                ent for ent in memoized_entities if isinstance(ent, _MapperEntity)
            ]:
    ...
    def _compound_eager_statement(self):
        ...
        if unwrapped_order_by:
            statement.order_by.non_generative(
                statement,
                *self.compound_eager_adapter.copy_and_process(unwrapped_order_by),
            )
    ...
    def _join_check_and_adapt_right_side(self, left, right, onclause, prop):
        ...
        if right_mapper and (
            ...
            for from_obj in self.from_clauses or [l_info.selectable]:
                ...
                ) and sql_util.selectables_overlap(from_obj, r_info.selectable):
                    overlap = True
                    break

        if overlap and l_info.selectable is r_info.selectable:
            raise sa_exc.InvalidRequestError(
                "Can't join table/selectable '%s' to itself" % l_info.selectable
            )
    ...
    def _select_args(self):
        return {
            ...
            "fetch_clause_options": (self.select_statement._fetch_clause_options),
            "independent_ctes": self.select_statement._independent_ctes,
            "independent_ctes_opts": (self.select_statement._independent_ctes_opts),
        }
...
def _column_descriptions(
    ...
    d = [
        ...
        for ent, insp_ent in [(_ent, _ent.entity_zero) for _ent in ctx._entities]
    ]
    return d
...
class _BundleEntity(_QueryEntity):
    ...
    def __init__(
        ...
        if isinstance(expr, (attributes.QueryableAttribute, interfaces.PropComparator)):
...
class _ORMColumnEntity(_ColumnEntity):
    ...
    def corresponds_to(self, entity):
        if _is_aliased_class(entity):
            ...
            return not _is_aliased_class(self.entity_zero) and entity.common_parent(
                self.entity_zero
            )
