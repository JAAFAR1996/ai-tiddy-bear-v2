...
class ORMExecuteState(util.MemoizedSlots):
    ...
    def user_defined_options(self) -> Sequence[UserDefinedOption]:
        ...
        """
        return [
            opt for opt in self.statement._with_options if is_user_defined_option(opt)
        ]
...
class SessionTransaction(_StateChange, TransactionalContext):
    ...
    def is_active(self) -> bool:
        return (
            self.session is not None and self._state is SessionTransactionState.ACTIVE
        )
...
class Session(_SessionClassMethods, EventTarget):
    ...
    def __init__(
        ...
        *,
        ...
        if autocommit:
            raise sa_exc.ArgumentError("autocommit=True is no longer supported")
        self.identity_map = identity.WeakInstanceDict()

        if not future:
            raise sa_exc.ArgumentError(
                "The 'future' parameter passed to " "Session() may only be set to True."
            )
        ...
        **kw: Any,
        ...
        if self.__binds:
            ...
            if clause is not None:
                plugin_subject = clause._propagate_attrs.get("plugin_subject", None)
    ...
    def _remove_newly_deleted(self, states: Iterable[InstanceState[Any]]) -> None:
    ...
    def get_one(
        ...
        *,
        ...
        if instance is None:
            raise sa_exc.NoResultFound("No row was found when one was required")
    ...
    def _get_impl(
        ...
        *,
        ...
        if (
            ...
            and type(primary_key_identity) in descriptor_props._composite_getters
        ):
            getter = descriptor_props._composite_getters[type(primary_key_identity)]
        ...
        if not is_dict:
            primary_key_identity = util.to_list(primary_key_identity, default=[None])
        ...
        if is_dict:
            ...
            if pk_synonyms:
                correct_keys = set(pk_synonyms).intersection(primary_key_identity)

                if correct_keys:
                    primary_key_identity = dict(primary_key_identity)
                    for k in correct_keys:
                        primary_key_identity[pk_synonyms[k]] = primary_key_identity[k]
        ...
        statement = sql.select(mapper).set_label_style(LABEL_STYLE_TABLENAME_PLUS_COL)
        if with_for_update is not None:
            statement._for_update_arg = ForUpdateArg._from_argument(with_for_update)
    ...
    def _merge(
        ...
        *,
        ...
        if key is None:
            ...
            key_is_persistent = LoaderCallableStatus.NEVER_SET not in key[1] and (
        ...
        if state is not merged_state:
            ...
            if mapper.version_id_col is not None:
                ...
                if (
                    existing_version is not LoaderCallableStatus.PASSIVE_NO_RESULT
                    and merged_version is not LoaderCallableStatus.PASSIVE_NO_RESULT
