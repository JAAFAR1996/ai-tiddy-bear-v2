...
from ..util.typing import get_args, is_fwd_ref, is_pep593
...
from .util import _none_set, de_stringify_annotation

if typing.TYPE_CHECKING:
    from ..engine.base import Connection
    from ..engine.row import Row
    from ..sql._typing import _DMLColumnArgument, _InfoType
    from ..sql.elements import ClauseList, ColumnElement
    ...
    from .attributes import History, InstrumentedAttribute, QueryableAttribute
    from .context import ORMCompileState
    from .decl_base import _ClassScanMapperConfig
    from .mapper import Mapper
    from .properties import ColumnProperty, MappedColumn
...
class CompositeProperty(
    ...
    def _populate_composite_bulk_save_mappings_fn(
        ...
        def populate(dest_dict: Dict[str, Any]) -> None:
            dest_dict.update(
                {
                    key: val
                    for key, val in zip(attrs, get_values(dest_dict.pop(self.key)))
                }
            )
    ...
    class Comparator(PropComparator[_PT]):
        ...
        def _compare(self, operator: OperatorType, other: Any) -> ColumnElement[bool]:
            ...
            comparisons = [
                operator(a, b) for a, b in zip(self.prop._comparable_elements, values)
            ]
            if self._adapt_to_entity:
