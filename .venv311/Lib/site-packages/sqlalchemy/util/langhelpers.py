        ...
        try:
            ...
            if default_len:
                ...
                kw_args.update(
                    [
                        (arg, default)
                        for arg, default in zip(spec.args[-default_len:], spec.defaults)
                    ]
                )
    output: List[str] = []
...
def class_hierarchy(cls):
    ...
    while process:
        ...
        for s in [
            _
            for _ in (
                c.__subclasses__() if not issubclass(c, type) else c.__subclasses__(c)
            )
            if _ not in hier
        ]:
            process.append(s)
...
def as_interface(obj, cls=None, methods=None, required=None):
    ...
    if not isinstance(obj, dict):
        ...
        raise TypeError(
            "%r does not implement %s: %s" % (obj, qualifier, ", ".join(interface))
        )
    ...
    raise TypeError(
        "dictionary does not contain required keys %s" % ", ".join(required - found)
    )
...
def has_compiled_ext(raise_=False):
    if HAS_CYEXTENSION:
        ...
        raise ImportError(
            "cython extensions were expected to be installed, " "but are not present"
        )
    else:
        return False
