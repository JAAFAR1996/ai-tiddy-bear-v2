...
from ... import Computed, exc
from ... import schema as sa_schema
from ... import sql, util
from ...engine import ObjectKind, ObjectScope, default, reflection
from ...engine.reflection import ReflectionDefaults
from ...sql import and_, bindparam, compiler, expression, func, null, or_, select
...
from .vector import VECTOR, VectorIndexConfig, VectorIndexType
    ...
    def fetch_clause(
        ...
        **kw,
        ...
        text = super().fetch_clause(
            ...
            use_literal_execute_for_simple_int=(use_literal_execute_for_simple_int),
            **kw,
        )
...
class OracleDDLCompiler(compiler.DDLCompiler):
    def _build_vector_index_config(self, vector_index_config: VectorIndexConfig) -> str:
        parts = []
        sql_param_name = {
...
class OracleDialect(default.DefaultDialect):
    ...
    def _get_synonyms(self, connection, schema, filter_names, dblink, **kw):
        ...
        if has_filter_names:
            query = query.where(
                dictionary.all_synonyms.c.synonym_name.in_(params["filter_names"])
            )
    ...
    def _all_objects_query(self, owner, scope, kind, has_filter_names, has_mat_views):
        ...
        if has_filter_names:
            query = query.where(
                dictionary.all_objects.c.object_name.in_(bindparam("filter_names"))
            )
        return query
    ...
    def get_table_names(self, connection, schema=None, dblink=None, **kw):
        ...
        if kw.get("oracle_resolve_synonyms", False):
            tables = (
                ...
                .union_all(
                    select(
                        dictionary.all_synonyms.c.synonym_name.label("table_name"),
        ...
        if self.exclude_tablespaces:
            query = query.where(
                func.coalesce(tables.c.tablespace_name, "no tablespace").not_in(
                    self.exclude_tablespaces
                )
            )
        query = query.where(
    ...
    def get_materialized_view_names(
        ...
        query = select(dictionary.all_mviews.c.mview_name).where(
            dictionary.all_mviews.c.owner == self.denormalize_schema_name(schema)
        )
        result = self._execute_reflection(
            connection, query, dblink, returns_long=False
        ).scalars()
    ...
    def get_view_names(self, connection, schema=None, dblink=None, **kw):
        ...
        query = select(dictionary.all_views.c.view_name).where(
            dictionary.all_views.c.owner == self.denormalize_schema_name(schema)
        )
        result = self._execute_reflection(
            connection, query, dblink, returns_long=False
        ).scalars()
    ...
    def _table_options_query(self, owner, scope, kind, has_filter_names, has_mat_views):
        ...
        if has_filter_names:
            query = query.where(
                dictionary.all_tables.c.table_name.in_(bindparam("filter_names"))
            )
        if scope is ObjectScope.DEFAULT:
        ...
        if (
            ...
            query = query.where(
                dictionary.all_tables.c.table_name.not_in(bindparam("mat_views"))
            )
        ...
        if self.server_version_info >= (12,):
            add_cols = (
                ...
                sql.case(
                    ...
                    else_=all_ids.c.generation_type + "," + all_ids.c.identity_options,
        ...
        **kw,
        ...
        return (
            (
                ...
                (
                    {"text": comment}
                    if comment is not None and not comment.startswith(ignore_mat_view)
                    else default()
                ),
            )
    ...
    def _constraint_query(self, owner):
        ...
        return (
            ...
            .where(
                ...
                dictionary.all_constraints.c.table_name.in_(bindparam("all_objects")),
                dictionary.all_constraints.c.constraint_type.in_(("R", "P", "U", "C")),
            )
            .order_by(dictionary.all_constraints.c.constraint_name, local.c.position)
        )
    ...
    def get_multi_unique_constraints(
        ...
        **kw,
        ...
        for row_dict in self._get_all_constraint_rows(
            ...
            if constraint_name not in table_uc:
                table_uc[constraint_name] = uc = {
                    ...
                    "column_names": [],
                    "duplicates_index": (
                        constraint_name if constraint_name_orig in index_names else None
                    ),
                }
            else:
        ...
        return (
            (
                key,
                (list(unique_cons[key].values()) if key in unique_cons else default()),
            )
            for key in (
                (schema, self.normalize_name(obj_name)) for obj_name in all_objects
            )
    ...
    def get_multi_check_constraints(
        ...
        **kw,
        ...
        return (
            (
                key,
                (check_constraints[key] if key in check_constraints else default()),
            )
            for key in (
                (schema, self.normalize_name(obj_name)) for obj_name in all_objects
            )
