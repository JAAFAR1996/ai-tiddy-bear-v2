...
from ... import exc, schema, select, sql, util
from ...engine import (
    URL,
    ObjectKind,
    ObjectScope,
...
from ...sql import bindparam, coercions, compiler, elements, expression, roles, sqltypes
...
from .ext import _regconfig_fn, aggregate_order_by
...
class PGCompiler(compiler.SQLCompiler):
    ...
    def for_update_clause(self, select, **kw):
        ...
        if select._for_update_arg.of:
            ...
            tmp += " OF " + ", ".join(self.process(table, **of_kw) for table in tables)
...
class PGDDLCompiler(compiler.DDLCompiler):
    ...
    def _define_include(self, obj):
        ...
        if not includeclause:
            return ""
        inclusions = [
            obj.table.c[col] if isinstance(col, str) else col for col in includeclause
        ]
    ...
    def _fk_ondelete_pattern(self):
        return re.compile(
            r"^(?:RESTRICT|CASCADE|SET (?:NULL|DEFAULT)(?:\s*\(.+\))?" r"|NO ACTION)$",
            re.I,
        )
...
class PGDialect(default.DefaultDialect):
    ...
    def set_isolation_level(self, dbapi_connection, level):
        cursor = dbapi_connection.cursor()
        cursor.execute(
            "SET SESSION CHARACTERISTICS AS TRANSACTION " f"ISOLATION LEVEL {level}"
        )
    ...
    def _has_table_query(self, schema):
        query = select(pg_catalog.pg_class.c.relname).where(
            ...
            self._pg_class_relkind_condition(pg_catalog.RELKINDS_ALL_TABLE_LIKE),
        )
    ...
    def _table_oids_query(self, schema, has_filter_names, scope, kind):
        relkinds = self._kind_to_relkinds(kind)
        oid_q = select(pg_catalog.pg_class.c.oid, pg_catalog.pg_class.c.relname).where(
            self._pg_class_relkind_condition(relkinds)
        )
    ...
    def _constraint_query(self):
        ...
        con_sq = (
            select(
                ...
                sql.func.generate_subscripts(pg_catalog.pg_index.c.indkey, 1).label(
                    "ord"
                ),
                indnkeyatts,
                indnullsnotdistinct,
            ...
            .join(
                pg_catalog.pg_index,
                pg_catalog.pg_constraint.c.conindid == pg_catalog.pg_index.c.indexrelid,
            )
            .outerjoin(
                pg_catalog.pg_description,
                pg_catalog.pg_description.c.objoid == pg_catalog.pg_constraint.c.oid,
            )
            .where(
            ...
            for oid, tablename in batch:
                ...
                if for_oid:
                    for row in for_oid:
                        ...
                        if indnkeyatts is not None and len(all_cols) > indnkeyatts:
    ...
    def _foreing_key_query(self, schema, has_filter_names, scope, kind):
        ...
        query = (
            ...
            .outerjoin(
                ...
                sql.and_(
                    pg_catalog.pg_class.c.oid == pg_catalog.pg_constraint.c.conrelid,
    ...
    def _enum_query(self, schema):
        ...
        query = (
            ...
            .outerjoin(lbl_agg_sq, pg_catalog.pg_type.c.oid == lbl_agg_sq.c.enumtypid)
            .where(pg_catalog.pg_type.c.typtype == "e")
            .order_by(pg_catalog.pg_namespace.c.nspname, pg_catalog.pg_type.c.typname)
        )
    ...
    def _domain_query(self, schema):
        ...
        query = (
            ...
            .outerjoin(
                ...
                pg_catalog.pg_type.c.typcollation == pg_catalog.pg_collation.c.oid,
            )
            .outerjoin(
                con_sq,
