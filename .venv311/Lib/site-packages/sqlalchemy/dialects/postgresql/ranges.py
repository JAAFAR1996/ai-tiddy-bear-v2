...
if TYPE_CHECKING:
    ...
    from ...sql.type_api import _TE, TypeEngineMixin
...
class Range(Generic[_T]):
    ...
    def _contains_value(self, value: _T) -> bool:
        ...
        if self.upper is None:
            return (  # type: ignore
                value > self.lower if self.bounds[0] == "(" else value >= self.lower
            )

        return (  # type: ignore
            value > self.lower if self.bounds[0] == "(" else value >= self.lower
        ) and (value < self.upper if self.bounds[1] == ")" else value <= self.upper)
    ...
    def union(self, other: Range[_T]) -> Range[_T]:
        ...
        if not self.overlaps(other) and not self.adjacent_to(other):
            raise ValueError(
                "Adding non-overlapping and non-adjacent" " ranges is not implemented"
            )
    ...
    def difference(self, other: Range[_T]) -> Range[_T]:
        ...
        if sl_vs_ol <= 0 and su_vs_ol >= 0 and su_vs_ou <= 0:
            ...
            if (
                slower_b != "["
                and rupper_b != "]"
                and self._compare_edges(slower, slower_b, olower, rupper_b) == 0
            ):
                return Range(None, None, empty=True)
            else:
                return Range(
        ...
        if sl_vs_ol >= 0 and su_vs_ou >= 0 and sl_vs_ou <= 0:
            ...
            if (
                rlower_b != "["
                and supper_b != "]"
                and self._compare_edges(oupper, rlower_b, supper, supper_b) == 0
            ):
                return Range(None, None, empty=True)
            else:
                return Range(
