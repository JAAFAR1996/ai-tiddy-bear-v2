...
from .. import event, exc, pool, util
from ..sql import compiler, dml, expression, type_api
from ..sql import util as sql_util
from ..sql._typing import is_tuple_type
from ..sql.base import _NoArg
from ..sql.compiler import DDLCompiler, InsertmanyvaluesSentinelOpts, SQLCompiler
from ..sql.elements import quoted_name
from ..util.typing import Final, Literal
from . import characteristics
from . import cursor as _cursor
from . import interfaces
from .base import Connection
from .interfaces import CacheStats, DBAPICursor, Dialect, ExecuteStyle, ExecutionContext
from .reflection import ObjectKind, ObjectScope
...
if typing.TYPE_CHECKING:
    ...
    from ..sql.compiler import Compiled, Linting, ResultColumnsEntry
    from ..sql.dml import DMLState, UpdateBase
    from ..sql.elements import BindParameter
    from ..sql.schema import Column
    from ..sql.type_api import TypeEngine, _BindProcessorType, _ResultProcessorType
...
class DefaultDialect(Dialect):
        ...
        if connection.in_transaction():
            ...
            if trans_objs:
                raise exc.InvalidRequestError(
                    ...
                    "is called first." % (", ".join(name for name, obj in trans_objs))
                )
                    ...
                    if not _sentinel_proc_initialized:
                        if composite_sentinel:
                            _composite_sentinel_proc = [
                                col.type._cached_result_processor(self, cursor_desc[1])
    ...
    def is_disconnect(
        self,
        e: Exception,
        connection: Union[pool.PoolProxiedConnection, interfaces.DBAPIConnection, None],
    ...
    def reset_isolation_level(self, dbapi_conn):
        if self._on_connect_isolation_level is not None:
            assert (
                ...
                or self._on_connect_isolation_level == self.default_isolation_level
            )
            self._assert_and_set_isolation_level(
                dbapi_conn, self._on_connect_isolation_level
            )
        ...
        for table in names:
            if not filter_names or table in filter_names:
                ...
                try:
                    yield (
                        key,
                        single_tbl_method(connection, table, schema=schema, **kw),
                    )
        ...
        if self.isinsert:
            if (
                ...
                strategy = _cursor.FullyBufferedCursorFetchStrategy(
                    ...
                    alternate_description=(strategy.alternate_cursor_description),
                )
