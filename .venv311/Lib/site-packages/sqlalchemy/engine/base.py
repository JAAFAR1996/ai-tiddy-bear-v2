...
from .util import TransactionalContext, _distill_params_20, _distill_raw_params

if typing.TYPE_CHECKING:
    from ..event import dispatcher
    from ..log import _EchoFlagType
    from ..pool import Pool, PoolProxiedConnection, _ConnectionFairy
    from ..sql import Executable
    from ..sql._typing import _InfoType
    from ..sql.compiler import Compiled
    from ..sql.ddl import ExecutableDDLElement, InvokeDDLBase
    from ..sql.functions import FunctionElement
    from ..sql.schema import DefaultGenerator, HasSchemaAttr, SchemaVisitable
    from ..sql.selectable import TypedReturnsRows
    from . import CursorResult, ScalarResult
    from .interfaces import (
        CompiledCacheType,
...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    ...
    def __init__(
        ...
        if connection is None:
            try:
                ...
                Connection._handle_dbapi_exception_noconnection(err, dialect, engine)
                raise
        else:
        ...
        self._has_events = _has_events or (_has_events is None and engine._has_events)
    ...
    def _still_open_and_dbapi_connection_is_valid(self) -> bool:
        ...
        return pool_proxied_connection is not None and pool_proxied_connection.is_valid
    ...
    def detach(self) -> None:
        ...
        if pool_proxied_connection is None:
            raise exc.InvalidRequestError("Can't detach an invalidated Connection")
    ...
    def _get_required_nested_transaction(self) -> NestedTransaction:
        ...
        if trans is None:
            raise exc.InvalidRequestError("connection is not in a nested transaction")
        return trans
    ...
    def close(self) -> None:
        ...
        if self._dbapi_connection is not None:
            ...
            if skip_reset:
                cast("_ConnectionFairy", conn)._close_special(transaction_reset=True)
            else:
                conn.close()
    ...
    def _execute_ddl(
        ...
        if self._has_events or self.engine._has_events:
            (
                ...
                event_multiparams,
                event_params,
            ) = self._invoke_before_exec_event(ddl, distilled_parameters, exec_opts)
        else:
            event_multiparams = event_params = None
    ...
    def _exec_insertmany_context(
        ...
        if self.dialect._has_events:
            do_execute_dispatch: Iterable[Any] = self.dialect.dispatch.do_execute
        else:
            do_execute_dispatch = ()
        ...
        preserve_rowcount = context.execution_options.get("preserve_rowcount", False)
        rowcount = 0
    ...
    def _handle_dbapi_exception(
        ...
        ismulti: bool = (
            not is_sub_exec and context.executemany if context is not None else False
        )
