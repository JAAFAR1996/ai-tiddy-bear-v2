...
from ..util import NONE_SET, HasMemoized_ro_memoized_attribute
...
class ResultMetaData:
    ...
    def _raise_for_ambiguous_column_name(self, rec: _KeyMapRecType) -> NoReturn:
        raise NotImplementedError(
            "ambiguous column name logic is implemented for " "CursorResultMetaData"
        )
    ...
    def _make_key_to_index(
        ...
        return {
            key: rec[index] for key, rec in keymap.items() if rec[index] is not None
        }
...
class ResultInternal(InPlaceGenerative, Generic[_R]):
    ...
    def _row_getter(self) -> Optional[Callable[..., _R]]:
        real_result: Result[Any] = (
            self._real_result if self._real_result else cast("Result[Any]", self)
        )
...
class ChunkedIteratorResult(IteratorResult[_TP]):
    ...
    def __init__(
        self,
        cursor_metadata: ResultMetaData,
        chunks: Callable[[Optional[int]], Iterator[Sequence[_InterimRowType[_R]]]],
...
class MergedResult(IteratorResult[_TP]):
    ...
    def __init__(self, cursor_metadata: ResultMetaData, results: Sequence[Result[_TP]]):
