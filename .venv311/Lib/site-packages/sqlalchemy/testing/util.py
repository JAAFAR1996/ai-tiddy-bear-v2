...
from ..util import decorator, defaultdict, has_refcount_gc, inspect_getfullargspec
        ...
        for table_key, fkcs in reversed(
            ...
            if table_key:
                if include_names is not None and table_key[1] not in include_names:
                    continue
                conn.execute(
                    DropTable(Table(table_key[1], MetaData(), schema=table_key[0]))
                )
            elif fkcs:
...
def skip_if_timeout(seconds: float, cleanup: Any = None):
    ...
    if sec > seconds:
        try:
            ...
            config.skip_test(f"test took too long ({sec:.4f} seconds > {seconds})")
