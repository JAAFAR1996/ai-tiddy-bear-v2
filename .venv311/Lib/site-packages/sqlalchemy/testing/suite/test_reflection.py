...
class HasIndexTest(fixtures.TablesTest):
    ...
    def test_has_index_schema(self, kind, connection):
        ...
        assert not meth("nonexistent_table", "my_idx_s", schema=config.test_schema)
        assert not meth("test_table", "nonexistent_idx_s", schema=config.test_schema)
...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    def test_reflect_table_temp_table(self, connection):
        ...
        reflected_user_tmp = Table(table_name, MetaData(), autoload_with=connection)
        self.assert_tables_equal(user_tmp, reflected_user_tmp, strict_constraints=False)
    ...
    def test_unreflectable(self, connection):
        ...
        with mock.patch.object(Inspector, "get_multi_columns", patched):
            with expect_raises_message(UnreflectableTableError, "err"):
                inspect(connection).reflect_table(Table("some_table", MetaData()), None)
...
class ComponentReflectionTestExtra(ComparesIndexes, fixtures.TestBase):
    ...
    def ck_eq(self, reflected, expected):
        ...
        def normalize(sqltext):
            return " ".join(re.findall(r"and|\d|=|a|b|c|or|<|>", sqltext.lower(), re.I))
    ...
    @testing.combinations("my_inline", "MyInline", None, argnames="constraint_name")
        ...
        with inspect_for_table("sa_cc") as (schema, inspector):
            Table(
                ...
                Column(
                    "a",
                    Integer(),
                    sa.CheckConstraint("a > 1 AND a < 5", name=constraint_name),
                ),
                Column("data", String(50)),
                schema=schema,
            )
    ...
    @testing.requires.check_constraint_reflection
    @testing.combinations("my_ck_const", "MyCkConst", None, argnames="constraint_name")
    def test_check_constraint_standalone(
        ...
        with inspect_for_table("sa_cc") as (schema, inspector):
            Table(
                ...
                metadata,
                Column("a", Integer()),
                sa.CheckConstraint("a = 1 OR (a > 2 AND a < 5)", name=constraint_name),
                schema=schema,
            )
    ...
    def test_reflect_covering_index(self, metadata, connection):
        ...
        eq_(
            get_indexes[0]["dialect_options"]["%s_include" % connection.engine.name],
            ["y"],
        )
