...
def _parse_time_to_freeze(
    ...
    if isinstance(time_to_freeze_str, datetime.datetime):
        ...
        time_to_freeze = (
            datetime.datetime.now(datetime.timezone.utc) + time_to_freeze_str
        )
    else:
...
class _freeze_time:
        ...
        if issubclass(klass, unittest.TestCase):
            ...
            for base_klass in klasses:
                for attr, attr_value in base_klass.__dict__.items():
                    ...
                    try:
                        if attr_value.__dict__.get(
                            "_pytestfixturefunction"
                        ) and hasattr(attr_value, "__pytest_wrapped__"):
                            ...
                            attr_value.__pytest_wrapped__.obj = self(
                                attr_value.__pytest_wrapped__.obj
                            )
                        elif attr_value.__dict__.get("_fixture_function"):
                            # PYTEST==8.4.x
                            # Same
                            attr_value._fixture_function = self(
                                attr_value._fixture_function
                            )
                        else:
    ...
    def start(
        ...
        if self.auto_tick_seconds:
            freeze_factory: Union[
                StepTickTimeFactory, TickingDateTimeFactory, FrozenDateTimeFactory
            ] = StepTickTimeFactory(self.time_to_freeze, self.auto_tick_seconds)
        elif self.tick:
