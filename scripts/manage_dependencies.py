...
def check_compatibility() -> None:
    ...
    if python_version.major != required_major or python_version.minor < required_minor:
        print(f"❌ Python {required_major}.{required_minor}+ required, got {python_version.major}.{python_version.minor}")
        sys.exit(1)
    else:
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="AI Teddy Bear Dependency Manager")
    parser.add_argument(
        "command",
        choices=["install", "security", "lock", "check", "clean", "validate", "all"],
        help="Command to run"
    )
    parser.add_argument(
        "--env",
