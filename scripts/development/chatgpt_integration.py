    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            response = await client.generate_child_safe_response(
                test_case["message"], test_case["age"], test_case["preferences"]
            )

            print(f"Response: {response['response']}")
            print(f"Emotion: {response['emotion']}")
            print(
                f"Safety: {'✅ Safe' if response['safety_analysis']['safe'] else '❌ Unsafe'}"
            )
...
def main():
    ...
    if client:
        ...
        print(
            'python -c "import asyncio; from chatgpt_integration import test_child_safe_response; asyncio.run(test_child_safe_response())"'
        )
