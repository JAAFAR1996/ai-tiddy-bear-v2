...
def main():
    ...
    try:
        ...
        print(
            f"✅ COPPA compliance status: {audit_result['compliance_status']}")

        # Step 5: Run security tests
        print("\n🔍 Step 5: Running Security Tests...")
        test_results = run_security_tests()
        print(
            f"✅ Security tests completed. Pass rate: {test_results['pass_rate']:.1f}%"
        )
        ...
        print(
            f"• Fixed vulnerabilities in {len(vulnerability_fixer.fixed_files)} files"
        )
        print(
            f"• Environment validation: {'✅ PASSED' if validation_result['valid'] else '❌ FAILED'}"
        )
        print(
            f"• COPPA compliance: ✅ {audit_result['compliance_status'].upper()}")
        print(
            f"• Security tests: {test_results['passed_tests']}/{test_results['total_tests']} passed"
        )
