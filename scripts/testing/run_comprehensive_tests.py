...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.StreamHandler(
            sys.stdout
        ),  # Removed encoding, relying on PYTHONIOENCODING or system default
    ],
)
...
class ComprehensiveTestRunner:
    ...
    def setup_environment(self):
        ...
        os.environ["PYTHONIOENCODING"] = (
            "UTF-8"  # Ensure consistent encoding for subprocesses
        )
    ...
    def install_dependencies(self):
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning(
                    f"Some production dependencies failed to install or upgrade: {result.stderr.decode('utf-8', errors='ignore')}"
                )
            ...
            if result.returncode != 0:
                logger.warning(
                    f"Some dev dependencies failed to install: {result.stderr.decode('utf-8', errors='ignore')}"
                )
    ...
    def run_unit_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, timeout=300, env=os.environ.copy()
            )
            ...
            if "passed" in test_report["stdout"]:
                ...
                passed_match = re.search(
                    r"(\d+) passed", test_report["stdout"])
                if passed_match:
                    test_report["tests_passed"] = int(passed_match.group(1))

                failed_match = re.search(
                    r"(\d+) failed", test_report["stdout"])
                if failed_match:
                    test_report["tests_failed"] = int(failed_match.group(1))

            test_report["tests_found"] = (
                test_report["tests_passed"] + test_report["tests_failed"]
            )
            ...
            try:
                if os.path.exists("coverage_unit.json"):
                    with open(
                        "coverage_unit.json", "r", encoding="utf-8"
                    ) as f:  # Added encoding
                        coverage_data = json.load(f)
                        test_report["coverage_percentage"] = coverage_data.get(
                            "totals", {}
            ...
            logger.info(
                f"Unit tests: {test_report['status']} - {test_report['tests_passed']} passed, {test_report['tests_failed']} failed"
            )
            return test_report
    ...
    def run_integration_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Integration tests: {test_report['status']} - {test_report['tests_passed']} passed, {test_report['tests_failed']} failed"
            )
            return test_report
    ...
    def run_security_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Security tests: {test_report['status']} - {test_report['security_issues_found']} issues found"
            )
            return test_report
    ...
    def run_code_quality_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_results["linting"] = {
                "status": "SKIPPED",
                "error": f"Flake8 not available: {e}",
            }
        ...
        try:
            ...
            if result.stdout:
                # Attempt to decode, then load JSON. Use default empty dict if
                # decoding fails.
                decoded_stdout = (
                    ...
                    errors="ignore")
                try:
                    bandit_results = json.loads(
                        decoded_stdout or "{}"
                    )  # Ensure it's not None or empty string before loading
                except json.JSONDecodeError:
                    logger.warning(
                        f"Failed to decode Bandit JSON: {decoded_stdout}")
                    bandit_results = {}
                ...
                quality_results["security_scanning"] = {
                    "status": "PASSED",
                    "issues": 0,
                    "details": (result.stderr or b"").decode("utf-8", errors="ignore")
                    or "No output from Bandit",
                }
        except (subprocess.CalledProcessError, FileNotFoundError, OSError) as e:
            quality_results["security_scanning"] = {
                "status": "SKIPPED",
                "error": f"Bandit not available: {e}",
            }
        except (
            Exception
        ) as e:  # Catch any other unexpected errors during Bandit processing
            logger.error(
                f"An unexpected error occurred during Bandit scan: {e}")
            quality_results["security_scanning"] = {
        ...
        logger.info(
            f"Code quality: {quality_results['overall_score']:.1f}% score")
    ...
    def run_performance_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if str(self.project_root /
                   "src") not in current_env.get("PYTHONPATH", ""):
            ...
            # This avoids potential conflicts with the main test runner's
            # environment
            performance_test_script = (
                Path(__file__).parent / "_run_single_performance_test.py"
            )
...
"""
            ...
            if result.returncode != 0:
                raise Exception(
                    f"Performance test script failed: {result.stderr.decode('utf-8', errors='ignore')}"
                )
            ...
            try:
                ...
                health_result = {
                    ...
                    "details": "Failed to parse health check output",
                }
                logger.warning(
                    f"Could not parse performance test health result: {health_output}"
                )
            ...
            return {
                "status": "TIMEOUT",
                "error": "Performance test script timed out after 30 seconds",
            }
        except Exception as e:
            logger.error(
                f"❌ Performance tests failed: {e}", exc_info=True
            )  # Log full traceback
            return {"status": "ERROR", "error": str(e)}
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        security_issues = (
            self.test_results["test_categories"]
            .get("security", {})
            .get("security_issues_found", 0)
        )
        if security_issues > 0:
            recommendations.append(
                f"🔒 Fix {security_issues} security issues identified in tests"
            )

        # Quality recommendations
        quality_score = self.test_results["quality_metrics"].get(
            "overall_score", 0)
        if quality_score < 90:
            recommendations.append(
                f"🏗️ Improve code quality from {quality_score:.1f}% to at least 90%"
            )
    ...
    def generate_final_report(self):
        ...
        failed_categories = [
            name
            for name, results in test_categories.items()
        ...
        start_time = datetime.fromisoformat(
            self.test_results["execution_start"])
        ...
        for category, results in test_categories.items():
            ...
            if "security_issues_found" in results:
                print(
                    f"   🔒 Security Issues: {results['security_issues_found']}")
        ...
        for i, recommendation in enumerate(
                self.test_results["recommendations"][:5], 1):
    ...
    def run_all_tests(self):
        ...
        for category_name, test_function in test_categories:
            ...
            try:
                ...
                logger.info(
                    f"✅ {category_name} tests completed: {results.get('status', 'UNKNOWN')}"
                )
            except Exception as e:
