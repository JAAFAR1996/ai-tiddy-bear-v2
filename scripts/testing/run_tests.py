...
def run_basic_tests():
    ...
    logger.info("=" * 60)
    logger.info(f"📈 النتيجة الإجمالية: {passed}/{total} ({passed/total*100:.1f}%)")
...
def run_security_scan():
    ...
    try:
        ...
        bandit_result = subprocess.run(bandit_cmd, capture_output=True, text=True, check=False)
            ...
            with open("bandit_report.json", "r", encoding="utf-8") as f:
                ...
                for result in report.get("results", []):
                    ...
                    if result['issue_severity'] in ['HIGH', 'CRITICAL']:
                        logger.error(f"🚨 Critical/High Security Issue (Bandit): {issue_details}")
                        return False
        else:
            logger.warning("Bandit report file not found, skipping Bandit results processing.")
    ...
    try:
        safety_cmd = [
            sys.executable, "-m", "safety", "check", "-r", "requirements.txt", "--full-report", "--json"
        ]
        logger.info(f"Running Safety: {' '.join(safety_cmd)}")
        safety_result = subprocess.run(safety_cmd, capture_output=True, text=True, check=False)
        ...
        logger.error(f"Error decoding Safety JSON output: {e}. Output was: {safety_result.stdout}")
    except Exception as e:
        logger.error(f"Error running Safety: {e}")

    if all_issues_found:
        logger.warning("⚠️ Potential security issues found (review logs for full details): ")
        for issue in all_issues_found[:10]:
            logger.warning(f"  - {issue}")
        if len(all_issues_found) > 10:
            logger.warning(f"  ... and {len(all_issues_found) - 10} more issues.")
        return False
    else:
...
def main():
    ...
    logger.info(f"🔒 الأمان: {'✅ نظيف' if security_clean else '⚠️ مشاكل محتملة'}")
