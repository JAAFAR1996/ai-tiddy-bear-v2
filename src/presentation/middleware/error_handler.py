...
def handle_exception(exc: Exception, error_id: str, path: str) -> ErrorResponse:
    ...
    if isinstance(exc, RequestValidationError):
        return ErrorResponse(
            ...
            details={"validation_errors": exc.errors()} if settings.application.DEBUG else None,
            path=path
        )
    ...
    # Default: Internal server error (500)
        ...
        message = "An unexpected error occurred" if not settings.application.DEBUG else str(exc)
        return ErrorResponse(
            error_id=error_id,
            message=message,
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            details={"exception": type(exc).__name__} if settings.application.DEBUG else None,
            path=path
        )
