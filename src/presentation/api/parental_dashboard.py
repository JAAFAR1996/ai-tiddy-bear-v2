try:
    ...
    try: 
        ...
        raise ImportError(f"Missing required dependency: pydantic. Install with 'pip install pydantic'") from e
  
    # Production-only imports - no fallbacks allowed for core dependencies
    try:
        ...
        raise ImportError(f"Missing required dependency: fastapi. Install with 'pip install fastapi'") from e
    ...
    class UpdateChildRequest(BaseModel):
        ...
        preferences: Optional[Dict[str, Any]] = Field(
            None,
            description="Updated child preferences (optional)"
        )  # ✅

    class ConsentRequest(BaseModel):
        """Request model for granting parental consent"""
        consent_types: list[str] = Field(
            ...,
            description="List of consent types to grant",
            examples=[["data_collection", "voice_recording", "usage_analytics"]]
        )
    ...
    @router.get(
        "/children/{child_id}", response_model=ChildData, status_code=status.HTTP_200_OK)
    ...
    @router.post("/children/{child_id}/story", response_model=StoryResponse, status_code=status.HTTP_200_OK)
    async def generate_child_story_endpoint(
        ...
        """Generates a dynamic story for a child based on their preferences and a given theme."""
        try:
    ...
    async def request_consent_verification(
        ...
        """Request parental consent verification for specified activities"""
        try:
    ...
    async def grant_parental_consent(
        ...
        try:
            ...
            raise error_handler.create_safe_error_response(
                ...
                "Failed to grant parental consent"
            )
    ...
    async def revoke_consent(
        ...
        """Revoke specific parental consent"""
        try:
            consent_manager = get_consent_manager()
            # Get parent_id from authenticated user
            parent_id = current_user.id
