...
async def get_child_stats(
    ...
    try:
        ...
        stats = {
            ...
            "emotion_analysis": child_stats.get("emotions", {
        ...
        from src.presentation.api.models.standard_responses import create_child_safety_response
...
async def get_devices_status(
    ...
    try:
        ...
        if current_user.get("role") == "admin":
            ...
            devices_data = await database.get_devices_status_for_children(user_child_ids)
        
        devices = []
        for device in devices_data:
            device_info = {
                "device_id": str(device.get("device_id", "")),
                "child_name": str(device.get("child_name", "Unknown"))[:50],  # Limit length
                "status": str(device.get("status", "unknown")),
                "last_seen": device.get("last_seen"),
                "battery_level": max(0, min(100, device.get("battery_level", 0))),  # 0-100 range
                "wifi_strength": max(-100, min(0, device.get("wifi_strength", -100))),  # dBm range
        ...
        from src.presentation.api.models.standard_responses import create_success_response
...
async def get_system_health(
    ...
    try:
        ...
        try:
            ...
            ai_health = await ai_service.health_check() if hasattr(ai_service, 'health_check') else "unknown"
        ...
        try:
            ...
            voice_health = await voice_service.health_check() if hasattr(voice_service, 'health_check') else "unknown"
        ...
        try:
            ...
            db_health = await database.health_check() if hasattr(database, 'health_check') else "unknown"
        ...
        try:
            # Check Redis health
            redis_health = await redis_cache.health_check() if hasattr(redis_cache, 'health_check') else "unknown"
