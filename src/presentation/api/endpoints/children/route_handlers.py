...
class ChildRouteHandlers:
    ...
    async def create_child_handler(
        ...
        try:
            ...
            consent_id = await self.coppa_integration.validate_child_creation(request, parent_id)
            logger.info(f"COPPA consent obtained: {consent_id}")
            
            # Create the child
            child = await self.manage_child_profile_use_case.create_child(request, parent_id)
    ...
    async def get_children_handler(
        ...
        try:
            parent_id = current_user.id
            children = await self.manage_child_profile_use_case.get_children_by_parent(parent_id)
    ...
    async def update_child_handler(
        ...
        try:
            ...
            can_modify = await self.coppa_integration.validate_data_access(
                child_id, parent_id, "preferences"
            )
            ...
            # Update the child
            updated_child = await self.manage_child_profile_use_case.update_child(child_id, request)
    ...
    async def delete_child_handler(
        ...
        try:
            ...
            deletion_result = await self.coppa_integration.handle_child_deletion(child_id, parent_id)
    ...
    async def get_child_safety_summary_handler(
        ...
        try:
            ...
            can_access = await self.coppa_integration.validate_data_access(
                child_id, parent_id, "safety_data"
            )
            ...
            safety_summary = await self.safety_monitor.get_child_safety_summary(child_id)
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to get safety summary: {str(e)}"
            )
    ...
    async def request_consent_handler(
        ...
        try:
            ...
            consent_result = await parental_consent_manager.request_consent(child_id, parent_id)
