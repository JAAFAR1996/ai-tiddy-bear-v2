...
class COPPAIntegration:
    ...
    async def validate_child_creation(self, request: ChildCreateRequest, parent_id: str) -> str:
        """
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Failed to validate COPPA compliance: {str(e)}"
            )

    async def validate_data_access(self, child_id: str, parent_id: str, data_type: str) -> bool:
        """
        ...
        try:
            return self.coppa.check_data_access_permission(child_id, parent_id, data_type)
        except Exception as e:
            logger.error(f"Error checking data access permission: {e}")
            return False

    async def validate_data_modification(self, child_id: str, parent_id: str, data_types: List[str]) -> bool:
        """التحقق من صلاحية تعديل بيانات الطفل"""
        try:
            # التحقق من كل نوع بيانات
            for data_type in data_types:
                if not self.coppa.check_data_modification_permission(child_id, parent_id, data_type):
                    return False
            return True
        except Exception as e:
    ...
    async def handle_child_deletion(self, child_id: str, parent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to delete child data: {str(e)}"
            )
...
class ParentalConsentManager:
    ...
    async def request_consent(self, parent_id: str, child_id: str, data_types: List[str]) -> str:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to request parental consent: {str(e)}"
            )
...
class DataRetentionManager:
    ...
    async def schedule_data_cleanup(self, child_id: str, cleanup_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to schedule data cleanup: {str(e)}"
            )
...
class ComplianceValidator:
    ...
    def validate_data_collection(self, data_types: List[str], child_age: int) -> Dict[str, Any]:
    ...
    def validate_data_sharing(self, data_types: List[str], child_age: int, third_party: str) -> Dict[str, Any]:
        """
...
async def validate_child_creation_compliance(
    ...
    parent_id: str,
    coppa_integration: COPPAIntegration = Depends(Provide[container.coppa_integration_service]),
) -> str:
    """
...
async def request_parental_consent(
    ...
    child_id: str,
    data_types: List[str],
    parental_consent_manager: ParentalConsentManager = Depends(Provide[container.parental_consent_manager_service]),
) -> str:
    """طلب موافقة أبوية"""
    return await parental_consent_manager.request_consent(parent_id, child_id, data_types)