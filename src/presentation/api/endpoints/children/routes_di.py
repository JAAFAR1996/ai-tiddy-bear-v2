...
async def create_child(
    ...
    coppa_service = Depends(
        Provide[Container.coppa_compliance_service]
    ),
    safety_monitor = Depends(
    ...
    try:
        ...
        consent_record = await coppa_service.create_consent_record(
            consent_data,
...
async def delete_child(
    ...
    try:
        ...
        await safety_monitor.record_safety_event(
            ...
            details=f"Child profile deleted with data retention policy",
            severity="info"
        )
...
async def get_child_safety_summary(
    ...
    try:
        ...
        # Get safety events
        safety_events = await database_service.get_safety_events(str(child_id), limit=50)
        ...
        high_severity_count = sum(1 for event in safety_events if event.get("severity") == "high")
        medium_severity_count = sum(1 for event in safety_events if event.get("severity") == "medium")
        ...
        return {
            ...
            "event_summary": {
                ...
                "low_severity": len(safety_events) - high_severity_count - medium_severity_count
            },
            "usage_summary": {
            ...
            "recommendations": [
                ...
                "Consider adjusting content filters" if high_severity_count > 2 else None
            ]
        }
    except HTTPException:
        raise
