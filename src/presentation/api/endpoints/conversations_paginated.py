...
from src.infrastructure.pagination import (
    PaginationService, PaginationRequest, PaginatedResponse,
    extract_pagination_params, SortOrder
)
...
class ConversationPaginationService:
    ...
    async def get_child_conversations(
        ...
        try:
            ...
            if parent_id and not await self._validate_parent_access(parent_id, child_id):
                raise HTTPException(
            ...
            safe_pagination = self.pagination_service.create_child_safe_pagination(pagination_request)
            ...
            # Apply pagination
            result = self.pagination_service.paginate_list(conversations, safe_pagination)
            
            logger.info(f"Retrieved {len(result.items)} conversations for child {child_id}")
            return result
    ...
    async def get_conversation_history(
        ...
        try:
            ...
            # Apply pagination
            safe_pagination = self.pagination_service.create_child_safe_pagination(pagination_request)
            result = self.pagination_service.paginate_list(recent_conversations, safe_pagination)
            
            logger.info(f"Retrieved {len(result.items)} recent conversations for child {child_id}")
            return result
        except Exception as e:
            logger.error(f"Error getting conversation history for child {child_id}: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def search_conversations(
        ...
        try:
            ...
            logger.info(f"Found {len(result.items)} conversations matching '{search_term}' for child {child_id}")
            return result
        except Exception as e:
...
if FASTAPI_AVAILABLE:
    ...
    async def get_child_conversations_paginated(
        ...
        parent_id: Optional[str] = Query(None, description="Parent ID for COPPA validation")
    ):
        """Get paginated conversations for a child."""
    ...
    async def search_conversations_paginated(
        child_id: str,
        search_term: str = Query(..., min_length=1, max_length=100, description="Search term"),
        page: int = Query(1, ge=1, description="Page number"),
        size: int = Query(20, ge=1, le=50, description="Page size"),
        sort_by: Optional[str] = Query("timestamp", description="Sort field"),
        sort_order: str = Query("desc", regex="^(asc|desc)$", description="Sort order")
