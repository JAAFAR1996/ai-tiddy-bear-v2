...
from .middleware.consent_verification import ConsentVerificationRoute, require_consent # ✅ Consent verification
...
async def get_current_user_esp32(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Dict[str, Any]:
...
class AudioRequest(BaseModel):
    ...
    audio_data: str = Field(..., min_length=10)  # Base64 encoded audio, min length for non-empty
...
@require_consent("data_collection", "voice_recording")  # ✅ Requires parental consent
@router.post("/process-audio", response_model=AudioResponse)
async def process_audio(
    request: AudioRequest,
    current_user: Dict[str, Any] = Depends(get_current_user_esp32),  # ✅ - Authentication added
    ai_orchestration_service: AIOrchestrationService = Depends(Provide[Container.ai_orchestration_service]),
