...
class APIErrorHandler:
    ...
    def handle_validation_error(
        ...
        logger.warning(
            f"Validation error [{error_id}] in {operation}: {error}"
        )
    ...
    def handle_authentication_error(
        ...
        if error:
            logger.warning(
                f"Authentication error [{error_id}] in {operation}: {error}"
            )
        return HTTPException(
    ...
    def handle_authorization_error(
        ...
        if error:
            logger.warning(
                f"Authorization error [{error_id}] in {operation}: {error}"
            )
        return HTTPException(
    ...
    def handle_not_found_error(
        ...
        if resource_id:
            logger.info(
                f"Resource not found [{error_id}] in {operation}: {resource} with ID {resource_id}"
            )
            user_message = f"{resource.title()} not found"
        else:
            logger.info(
                f"Resource not found [{error_id}] in {operation}: {resource}"
            )
            user_message = f"{resource.title()} not found"
        return HTTPException(
    ...
    def handle_child_safety_error(
        ...
        error_id = str(uuid4())[:8]
        logger.critical(
            f"CHILD SAFETY VIOLATION [{error_id}] in {operation}: {error}"
        )
        return HTTPException(
    ...
    def handle_rate_limit_error(
        operation: str, retry_after: Optional[int] = None
    ) -> HTTPException:
    ...
    def handle_service_unavailable_error(
        service: str, operation: str, error: Optional[Exception] = None
    ) -> HTTPException:
        """Handle service unavailability errors(503)"""
        error_id = str(uuid4())[:8]
        if error:
            logger.error(
                f"Service unavailable [{error_id}] in {operation}: {service} - {error}"
            )
        else:
            logger.error(
                f"Service unavailable [{error_id}] in {operation}: {service}"
            )
        return HTTPException(
