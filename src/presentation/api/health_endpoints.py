...
async def health_check() -> HealthResponse:
    ...
    try:
        ...
        is_healthy = system_health.status in [
...
async def liveness_check():
    ...
    try:
        ...
        uptime = (
            health_manager.start_time
            if hasattr(health_manager, "start_time")
            else 0
        )
        current_uptime = (
            datetime.utcnow().timestamp() - uptime if uptime else 0
        )
        ...
        return JSONResponse(
            content=response.dict(), status_code=status.HTTP_200_OK
        )
    except Exception as e:
...
async def health_metrics():
    ...
    try:
        ...
        for check in system_health.checks:
            ...
            metrics.append(
                f'ai_teddy_health_check_status{{check="{check.name}"}} {status_value}'
            )
            metrics.append(
                f'ai_teddy_health_check_duration_ms{{check="{check.name}"}} {check.duration_ms}'
            )

        # System metrics
        metrics.append(f"ai_teddy_uptime_seconds {system_health.uptime}")
        metrics.append(
            f"ai_teddy_health_checks_total {len(system_health.checks)}"
        )
        ...
        for check in system_health.checks:
            if check.name == "system_resources" and "details" in check.details:
                ...
                if "cpu_percent" in details:
                    metrics.append(
                        f'ai_teddy_cpu_usage_percent {details["cpu_percent"]}'
                    )
                if "memory_percent" in details:
                    metrics.append(
                        f'ai_teddy_memory_usage_percent {details["memory_percent"]}'
                    )
                if "disk_percent" in details:
                    metrics.append(
                        f'ai_teddy_disk_usage_percent {details["disk_percent"]}'
                    )
