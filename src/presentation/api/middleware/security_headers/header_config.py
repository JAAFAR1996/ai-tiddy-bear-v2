...
class SecurityHeadersConfig:
    ...
    custom_child_headers: Dict[str, str] = field(default_factory=lambda: {
        "X-Child-Safe": "1",
        "X-COPPA-Compliant": "1",
...
def validate_config(config: SecurityHeadersConfig) -> Dict[str, list]:
    ...
    if "'unsafe-inline'" in config.csp.script_src:
        issues["warnings"].append("CSP allows unsafe-inline scripts - consider using nonces")
    ...
    if not config.child_safety_mode:
        issues["recommendations"].append("Enable child safety mode for COPPA compliance")
