...
class CommentFormatter:
    ...
    def __init__(self) -> None:
        ...
        self.patterns = {
            ...
            # TODO/FIXME comments
            'todo': re.compile(r'^(\s*)#\s*(TODO|FIXME|HACK|NOTE|WARNING):\s*(.+)$', re.IGNORECASE),
            # Docstring patterns
    ...
    def format_single_comment(self, line: str) -> str:
        ...
        if formatted_content and formatted_content[0].islower() and not formatted_content.startswith(('http', 'ftp', 'www')):
            ...
            if (stripped.startswith('class ') or
                ...
                if not has_docstring:
                    if stripped.startswith('class '):
                        ...
                        formatted_lines.append(f'    """')
                        formatted_lines.append(f'    {func_name.replace("_", " ").title()} function.')
                        formatted_lines.append(f'    """')
            else:
                formatted_lines.append(line)
            i += 1
...
def format_comments_in_directory(directory: Path, add_missing_docstrings: bool = False) -> None:
    """
...
if __name__ == "__main__":
    ...
    for comment in sample_comments:
        logger.info(f"  {repr(comment)}")
    logger.info("\nFormatted comments:")
    for comment in sample_comments:
        if comment.strip().startswith('#'):
            formatted = formatter.format_single_comment(comment)
            logger.info(f"  {repr(formatted)}")
