...
class EmotionAnalyzer:
    ...
    def analyze_text(self, text: str) -> EmotionResult:
        ...
        elif any(word in text_lower for word in ["sad", "cry", "upset", "down"]):
        ...
        elif any(word in text_lower for word in ["curious", "wonder", "question"]):
            return EmotionResult(
    ...
    def analyze_voice(self, audio_features: Optional[Dict[str, Any]]) -> EmotionResult:
        ...
        if pitch > 0.7 and energy > 0.6:
            return EmotionResult(
                "excited", 0.8, {"excited": 0.8}, sentiment_score=0.7, arousal_score=0.9
            )
        elif pitch < 0.3 and energy < 0.4:
            return EmotionResult(
                "sad", 0.7, {"sad": 0.7}, sentiment_score=-0.5, arousal_score=0.3
            )
        elif energy > 0.8:
            return EmotionResult(
                "energetic",
    ...
    def requires_attention(self, emotion_result: EmotionResult) -> bool:
        ...
        if emotion_result.arousal_score > 0.8 and emotion_result.sentiment_score < -0.3:
            return True
        return False