...
class EmotionResult:
    ...
    def __post_init__(self):
        ...
        assert -1.0 <= self.sentiment_score <= 1.0, "Sentiment score must be between -1.0 and 1.0"
        assert 0.0 <= self.arousal_score <= 1.0, "Arousal score must be between 0.0 and 1.0"


class EmotionAnalyzer:
    def __init__(self):
        self.logger = logging.getLogger("emotion_analyzer")
        self._positive_emotions = {"happy", "excited", "calm", "curious", "proud", "energetic"}
    ...
    def analyze_text(self, text: str) -> EmotionResult:
        ...
        if any(word in text_lower for word in ["happy", "joy", "excited", "fun", "yay"]):
        ...
        if any(word in text_lower for word in ["angry", "mad", "frustrated"]):
            return EmotionResult("frustrated", 0.7, {"frustrated": 0.7}, sentiment_score=-0.4, arousal_score=0.8)
        if any(word in text_lower for word in ["scared", "afraid", "worried"]):
            return EmotionResult("worried", 0.75, {"worried": 0.75}, sentiment_score=-0.5, arousal_score=0.6)
        if any(word in text_lower for word in ["calm", "peaceful", "relaxed"]):
            return EmotionResult("calm", 0.8, {"calm": 0.8}, sentiment_score=0.3, arousal_score=0.2)
        if any(word in text_lower for word in ["curious", "wonder", "question"]):
            return EmotionResult("curious", 0.7, {"curious": 0.7}, sentiment_score=0.4, arousal_score=0.5)
        return EmotionResult("neutral", 0.5, {"neutral": 0.5}, sentiment_score=0.0, arousal_score=0.3)
    ...
    def analyze_voice(self, audio_features: dict[str, Any] | None) -> EmotionResult:
        if not audio_features or "audio_path" not in audio_features:
            self.logger.warning("No audio_path provided to analyze_voice; returning neutral emotion result.")
            return EmotionResult("neutral", 0.4, {"neutral": 0.4})
        if not LIBROSA_AVAILABLE:
            self.logger.error("librosa is not installed; cannot perform real voice emotion analysis.")
        ...
        try:
            ...
            if pitch > PITCH_EXCITED and energy > ENERGY_EXCITED:
                return EmotionResult("excited", 0.85, {"excited": 0.85}, sentiment_score=0.7, arousal_score=0.9)
            if pitch < PITCH_SAD and energy < ENERGY_SAD:
                return EmotionResult("sad", 0.8, {"sad": 0.8}, sentiment_score=-0.6, arousal_score=0.3)
            if energy > ENERGY_ENERGETIC:
                return EmotionResult("energetic", 0.8, {"energetic": 0.8}, sentiment_score=0.5, arousal_score=0.8)
            return EmotionResult("calm", 0.7, {"calm": 0.7}, sentiment_score=0.2, arousal_score=0.3)
    ...
    def requires_attention(self, emotion_result: EmotionResult) -> bool:
        if (emotion_result.primary_emotion in self._negative_emotions and emotion_result.confidence > 0.7):
            return True
