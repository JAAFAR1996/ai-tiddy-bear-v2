...
class ProcessESP32AudioUseCase:
    ...
    async def execute(self, request: ESP32Request) -> AIResponse:
        ...
        (
            ...
            request.audio_data, request.language_code
        )
        ...
        await self.conversation_service.start_new_conversation(
            request.child_id, transcription
        )
        
        conversation_id = UUID(ai_response.conversation_id) if isinstance(ai_response.conversation_id, str) else (ai_response.conversation_id if ai_response.conversation_id else uuid4())
        ...
        child_profile.preferences.vocabulary_size += len(transcription.split()) // 10 # Simple approximation
        child_profile.preferences.interaction_history_summary = (child_profile.preferences.interaction_history_summary or "") + " " + transcription + " " + ai_response.response_text
        
        # Update emotional tendencies based on AI response emotion
        current_emotion_score = child_profile.preferences.emotional_tendencies.get(ai_response.emotion, 0.0)
        child_profile.preferences.emotional_tendencies[ai_response.emotion] = (current_emotion_score + 1.0) / 2.0 # Simple averaging
