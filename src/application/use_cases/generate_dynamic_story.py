...
class GenerateDynamicStoryUseCase:
    ...
    async def execute(
        ...
        try:
            ...
            prompt = self._create_story_prompt(
                theme=story_theme, preferences=child_preferences, length=story_length
            )
            
            # Generate story using AI service
            story_response = await self.ai_service.generate_story(
                prompt=prompt, child_preferences=child_preferences
            )
            ...
            self.logger.error(f"Error generating story for child {child_id}: {str(e)}")
            return {"success": False, "error": str(e), "story": None}
            
    def _create_story_prompt(
        self, theme: Optional[str], preferences: Dict[str, Any], length: str
    ) -> str:
    ...
    async def _get_child_preferences(self, child_id: UUID) -> Optional[Dict[str, Any]]:
        """Get child preferences from profile service"""
        try:
            ...
            self.logger.error(f"Error retrieving child preferences: {str(e)}")
            return None
    ...
    def _get_age_appropriate_interests(self, age: int) -> List[str]:
        ...
        if age <= 3:
            ...
            return ["adventures", "friendship", "school", "sports", "science", "art"]
        else:
            return ["adventures", "friendship", "challenges", "discovery", "creativity"]