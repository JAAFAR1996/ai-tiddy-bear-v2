...
class ChildProfileEventHandlers:
    ...
    async def handle_child_registered(self, event: ChildRegistered) -> None:
        ...
        try:
            ...
            logger.debug(f"Child profile created for age {event.age}")  # PII-safe logging
    ...
    async def handle_child_profile_updated(self, event: ChildProfileUpdated) -> None:
        ...
        try:
            ...
            if not existing_model:
                logger.warning(f"Child profile not found for update")
                return
            ...
            if updates_made:
                ...
                logger.debug(f"Child profile updated with {len(event.preferences or {})} preference changes")
            else:
    ...
    async def _async_get_by_id(self, child_id: str) -> Optional[ChildProfileReadModel]:
        """
