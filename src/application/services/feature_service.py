...
class FeatureService:
    ...
    async def enable_feature(
        ...
        try:
            ...
            self.logger.warning(f"Invalid feature type provided for enable_feature: {feature_name}")
        ...
        if not self.default_features[feature_type]:  # If feature is off by default
    ...
    async def disable_feature(self, feature_name: str, child_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"Invalid feature type provided for disable_feature: {feature_name}")
            return {"success": False, "message": "Invalid feature type."}

        if not self.default_features.get(feature_type):
            return {"success": False, "message": f"Feature {feature_type.value} is already disabled or unknown."}

        self.logger.info(f"Feature '{feature_type.value}' disabled for child {child_id}")
    ...
    def get_feature_status(self, feature_name: str, child_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"Invalid feature type provided for get_feature_status: {feature_name}")
            return {"success": False, "message": "Invalid feature type."}

        is_enabled = self.default_features.get(feature_type, False)
        self.logger.info(f"Feature '{feature_type.value}' status for child {child_id}: {is_enabled}")
