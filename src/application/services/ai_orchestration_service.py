...
from src.application.exceptions import ServiceUnavailableError, TimeoutError, APIError, InvalidInputError
...
class AIOrchestrationService:
    ...
    async def get_ai_response(
        ...
        except (ServiceUnavailableError, TimeoutError, APIError) as e: # Assuming these specific exceptions from external_apis or a custom exception module
            logger.error(f"AI provider error: {e}", exc_info=True)
            return AIResponse.safe_fallback("I'm sorry, I'm having trouble understanding right now. Can we talk about something else?")
        except Exception as e:
            logger.critical(f"Unexpected error during AI response generation: {e}", exc_info=True)
            return AIResponse.safe_fallback("An unexpected error occurred. Please try again later.")
