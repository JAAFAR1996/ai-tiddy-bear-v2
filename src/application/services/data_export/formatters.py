...
class CSVFormatter(BaseFormatter):
    ...
    def format_data(self, data: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            output.write(f"# Child Data Export\n")
...
class XMLFormatter(BaseFormatter):
    ...
    def format_data(self, data: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            ET.SubElement(metadata_elem, "export_timestamp").text = self.metadata.export_timestamp.isoformat()
            ...
            ET.SubElement(metadata_elem, "data_version").text = self.metadata.data_version
            ET.SubElement(metadata_elem, "total_records").text = str(self.metadata.total_records)
            ET.SubElement(metadata_elem, "retention_policy").text = self.metadata.retention_policy
            ...
            for category, records in data.items():
                ...
                if isinstance(records, list):
                    for i, record in enumerate(records):
                        record_elem = ET.SubElement(category_elem, "record", {"index": str(i)})
    ...
    def _add_dict_to_element(self, parent: ET.Element, data: dict) -> None:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                ET.SubElement(parent, str(key)).text = str(value) if value is not None else ""
...
class ArchiveFormatter(BaseFormatter):
    ...
    def __init__(self, metadata: ExportMetadata, formatters: List[BaseFormatter]) -> None:
            ...
            with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                ...
                for formatter in self.formatters:
                    try:
                        ...
                        logger.warning(f"Failed to add {formatter.__class__.__name__} to archive: {e}")
...
class FormatterFactory:
    ...
    @staticmethod
    def create_formatter(format_type: ExportFormat, metadata: ExportMetadata) -> BaseFormatter:
