...
from src.application.interfaces.safety_monitor import SafetyMonitor, SafetyAnalysisResult
from src.domain.exceptions import ResourceNotFoundError, InvalidInputError
...
class InteractionService:
    ...
    async def process(self, child_id: str, message: str) -> Dict[str, Any]:
        ...
        if len(message) > self.config.max_message_length:
            self.logger.warning(f"Message from child {child_id} too long (length: {len(message)}).")
            raise InvalidInputError(f"Message too long. Maximum {self.config.max_message_length} characters")

        # Age verification before any processing
        try:
            if not await self._check_child_age(UUID(child_id)):
                self.logger.warning(f"Interaction blocked for child {child_id} due to age restriction.")
                return {
                    "success": False,
            ...
            return {
                "success": False,
                "error": f"Age verification failed: {str(e)}",
                "safe": False,
                "timestamp": datetime.now(timezone.utc).isoformat(),
            }
        ...
        if await self.sanitization_service.detect_pii(cleaned_message):
            self.logger.warning(f"PII detected in message for child {child_id}. Blocking interaction.")
            return {
                "success": False,
        ...
        if not safety_result.is_safe:
            self.logger.warning(f"Unsafe content detected for child {child_id}: {safety_result.overall_risk_level.value}")
            return {
                "success": False,
    ...
    async def _check_content_safety(self, message: str) -> SafetyAnalysisResult:
        ...
        self.logger.info(f"Content safety check complete. Is safe: {safety_result.is_safe}, Risk level: {safety_result.overall_risk_level.value}")
    ...
    async def _check_child_age(self, child_id: UUID) -> bool:
        ...
        if child_age is None:
            self.logger.warning(f"Child age not found for child {child_id}. Cannot perform age check.")
            raise ResourceNotFoundError(f"Child profile or age not found for child_id: {child_id}")
        
        if not isinstance(child_age, int) or child_age < 0:
            self.logger.error(f"Invalid child age retrieved for child {child_id}: {child_age}")
            raise InvalidInputError(f"Invalid age format for child {child_id}")

        is_age_valid = self.config.min_child_age <= child_age <= self.config.max_child_age
        if not is_age_valid:
            self.logger.warning(f"Child {child_id} (age {child_age}) is outside allowed age range ({self.config.min_child_age}-{self.config.max_child_age}).")
        else:
            self.logger.info(f"Child {child_id} (age {child_age}) is within allowed age range.")
        return is_age_valid
