...
class SafetyService:
    ...
    def __init__(self, config: SafetyConfig, ai_provider: AIProvider, performance_monitor: PerformanceMonitor) -> None:
        """
        ...
        # self.metrics dictionary removed as metrics are now handled by PerformanceMonitor.
    ...
    def _analyze_harmful_content(self, content: str, result: SafetyAnalysisResult):
        ...
        except (ValueError, TypeError) as e: # Catching specific input/type related errors
            logger.error(f"Validation or type error during harmful content analysis: {e}", exc_info=True)
            result.is_safe = False
            result.risk_level = RiskLevel.CRITICAL
            result.modifications.append(
                ContentModification(
                    type="error", description=f"Input/type validation error: {str(e)}"
                )
            )
        except Exception as e: # Catching any other unexpected errors
            logger.critical(f"Unexpected critical error during harmful content analysis: {e}", exc_info=True)
            result.is_safe = False
            result.risk_level = RiskLevel.CRITICAL
            result.modifications.append(
                ContentModification(
                    type="error", description=f"Unexpected analysis error: {str(e)}"
                )
            )
    ...
    async def _analyze_toxicity(self, content: str, result: SafetyAnalysisResult):
        ...
        try:
            ...
            if toxicity_score > self.config.toxicity_threshold:
                ...
                result.modifications.append(
                    ContentModification(
                        type="flag", description=f"Toxicity score: {toxicity_score}"
                    )
                )
            ...
            result.modifications.append(
                ContentModification(
                    type="error", description=f"Toxicity analysis error: {str(e)}"
                )
            )

    async def _analyze_emotional_impact(self, content: str, result: SafetyAnalysisResult):
        """
        ...
        try:
            ...
            # In a real scenario, this might return a more detailed object including scores.
            emotional_impact = EmotionalImpact(sentiment=sentiment_label, emotions={}) # Populate emotions based on real API response
            result.emotional_impact = emotional_impact
            self.logger.debug(f"Emotional impact analysis complete: {sentiment_label}")
        except Exception as e:
            self.logger.error(f"Error during emotional impact analysis: {e}", exc_info=True)
            result.emotional_impact = EmotionalImpact(sentiment="error") # Indicate error state
            result.risk_level = max(result.risk_level, RiskLevel.CRITICAL)
            result.modifications.append(
                ContentModification(
                    type="error", description=f"Emotional analysis error: {str(e)}"
                )
            )

    async def _analyze_educational_value(self, content: str, result: SafetyAnalysisResult):
        """
        ...
        """
        try:
            educational_value_data = await self.ai_provider.evaluate_educational_value(content)
            ...
            self.logger.debug(f"Educational value analysis complete: {educational_value}")
        except Exception as e:
            self.logger.error(f"Error during educational value assessment: {e}", exc_info=True)
            result.educational_value = EducationalValue(score=0.0) # Indicate error state
            result.risk_level = max(result.risk_level, RiskLevel.CRITICAL)
            result.modifications.append(
                ContentModification(
                    type="error", description=f"Educational value analysis error: {str(e)}"
                )
            )

    async def _analyze_context(self, context: dict[str, Any], result: SafetyAnalysisResult):
        """
        ...
        """
        try:
            # Assuming AIProvider.analyze_context returns a dictionary that can be used to construct ContextAnalysis
            ...
            result.context_analysis = ContextAnalysis(context_safe=False) # Indicate error state
            result.risk_level = max(result.risk_level, RiskLevel.CRITICAL)
            result.modifications.append(
                ContentModification(
                    type="error", description=f"Context analysis error: {str(e)}"
                )
            )
    ...
    async def analyze_content(
        ...
        """
        await self.performance_monitor.record_safety_check(result="total_checked") # Increment total safety checks

        result = SafetyAnalysisResult(original_content=content)

        # Note: _analyze_harmful_content is synchronous and does not use AIProvider, so it remains synchronous.
        ...
        if not result.is_safe:
            await self.performance_monitor.record_safety_check(result="blocked_content") # Increment blocked content checks
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        """
        self.logger.warning("SafetyService.get_metrics is deprecated. Use PerformanceMonitor directly for comprehensive metrics.")
        return {}
