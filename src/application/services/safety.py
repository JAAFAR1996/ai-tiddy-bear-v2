...
class SafetyService:
    ...
    async def _analyze_toxicity(self, content: str,
        ...
        try:
            ...
            self.logger.error(
                f"Error during toxicity analysis: {e}",
                exc_info=True)
            result.is_safe = False
            result.risk_level = RiskLevel.CRITICAL
            result.modifications.append(
    ...
    async def _analyze_emotional_impact(
        ...
        try:
            ...
            emotional_impact = EmotionalImpact(
                sentiment=sentiment_label, emotions={}
            ...
            self.logger.error(
                f"Error during emotional impact analysis: {e}", exc_info=True
            )
    ...
    async def _analyze_educational_value(
        ...
        try:
            ...
            self.logger.error(
                f"Error during educational value assessment: {e}", exc_info=True
            )
            ...
            result.modifications.append(
                ContentModification(
                    type="error",
                    description=f"Educational value analysis error: {str(e)}",
                )
            )

    async def _analyze_context(
        self, context: dict[str, Any], result: SafetyAnalysisResult
    ):
        """
