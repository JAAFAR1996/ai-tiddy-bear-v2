        ...
        try:
            if self.safety_monitor:
                ...
                if safety_result.risk_level in [SafetyLevel.UNSAFE, SafetyLevel.POTENTIALLY_UNSAFE]:
                    self.logger.warning(f"Notification blocked: Unsafe content detected for recipient {recipient}. Message: '{message[:50]}...' Reason: {safety_result.analysis_details}")
                    return {"id": None, "status": NotificationStatus.BLOCKED.value, "message": "Notification content blocked due to safety concerns."}

            notification_id = uuid4()
            max_attempts = self.config.urgent_max_attempts if urgent else self.config.default_max_attempts
            ...
            self.logger.info(f"Notification {notification_id} queued for {recipient} via {channel.value}.")

            # Send based on channel
            success = await self._send_by_channel(notification_record)
            
            notification_record.attempts += 1
            notification_record.last_attempt_at = datetime.now(timezone.utc)
            notification_record.status = NotificationStatus.SENT if success else NotificationStatus.FAILED
            notification_record.error_message = None if success else "Failed to send via channel"

            await self.repository.save_notification(notification_record)
            self.logger.info(f"Notification {notification_id} sent to {recipient} via {channel.value} with status {notification_record.status.value}.")
            return {"id": str(notification_id), "status": notification_record.status.value, "message": "Notification process initiated."}
        except (ServiceUnavailableError, ConnectionError, TimeoutError, ValueError) as e:
            self.logger.error(f"Specific error sending notification: {e}", exc_info=True)
            return {"id": str(notification_id), "status": NotificationStatus.FAILED.value, "message": f"Failed to send notification: {e}"}
        except Exception as e:
            self.logger.critical(f"Unexpected critical error sending notification: {e}", exc_info=True)
            return {"id": str(notification_id), "status": NotificationStatus.FAILED.value, "message": "An unexpected critical error occurred during notification sending."}
    ...
    async def _send_by_channel(self, notification: NotificationRecord) -> bool:
        ...
        try:
            if channel == NotificationChannel.EMAIL.value:
                ...
                return await self.email_client.send_email(recipient, f"AI Teddy Bear Notification: {notification.notification_type}", message)
                ...
                return await self.in_app_notifier.send_in_app_notification(recipient, message, {"notification_id": str(notification_id)})
                ...
                return await self.push_notifier.send_push_notification(recipient, f"AI Teddy Bear: {notification.notification_type.replace('_', ' ').title()}", message, {"notification_id": str(notification_id)})
            else:
                self.logger.warning(f"Notification {notification_id}: Unknown notification channel: {channel}")
                return False
        except Exception as e:
            self.logger.error(f"Notification {notification_id}: Failed to send via {channel} to {recipient}: {e}", exc_info=True)
            return False

    async def get_notification_history(self, recipient: str) -> list[NotificationRecord]:
        """
        ...
        self.logger.info(f"Retrieved {len(history)} notifications for recipient: {recipient}")
        return history
