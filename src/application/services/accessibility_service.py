...
class AccessibilityService:
    ...
    async def create_accessibility_profile(
        ...
        self.logger.info(f"Accessibility profile created and saved for child: {child_id}")
        return profile

    async def get_accessibility_profile(self, child_id: UUID) -> AccessibilityProfile | None:
        """
        ...
        """
        self.logger.debug(f"Attempting to retrieve accessibility profile for child: {child_id}")
    ...
    def _get_accessibility_settings(
        ...
        if any(need in [SpecialNeedType.MOTOR_IMPAIRMENT, SpecialNeedType.COGNITIVE_DELAY] for need in needs):
