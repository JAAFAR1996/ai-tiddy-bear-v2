...
class ConversationService:
    ...
    def __init__(self, conversation_repo: ConversationRepository, logger: logging.Logger = logger) -> None:
    ...
    async def start_new_conversation(
        ...
        conversation.update_summary(initial_text)  # Initial text can be part of summary
    ...
    async def add_interaction(
        ...
        # For simplicity, let's assume we always start a new one or get the latest.
        conversations = await self.conversation_repo.find_by_child_id(child_id)
        if not conversations:
            self.logger.info(f"No active conversation found for child {child_id}. Starting a new one.")
            conversation = await self.start_new_conversation(child_id, user_input)
        else:
            # Sort conversations to reliably get the latest one
            conversation = sorted(conversations, key=lambda c: c.created_at, reverse=True)[0]
            self.logger.info(f"Using latest conversation {conversation.id} for child {child_id}.")
