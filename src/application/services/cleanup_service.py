...
class CleanupService:
    ...
    def __init__(self, logger: logging.Logger = get_logger(__name__, component="cleanup_service")) -> None:
        ...
        self.retention_policies = {
            ...
            "audio_recordings": timedelta(days=30),  # Audio data shorter retention
            "session_logs": timedelta(days=365),  # System logs longer retention
            "temporary_files": timedelta(days=7),  # Temp files short retention
            "analytics_data": timedelta(days=180),  # Analytics medium retention
        }
        self.logger = logger
    ...
    def _is_safe_path(self, file_path: Path) -> bool:
        ...
        if ".." in str(file_path):
            ...
            return False
        # More robust checks (e.g., against a list of allowed base directories) would go here.
    ...
    async def _cleanup_data_type(self, data_type: str) -> Dict[str, Any]:
        ...
        if not policy:
            return {"items_cleaned": 0, "space_freed_mb": 0, "message": "No policy defined"}

        cutoff_date = datetime.now() - policy
        self.logger.info(f"Cleaning up {data_type} older than {cutoff_date}")

        # Simulate cleanup operation
        items_cleaned, space_freed_mb = await self._simulate_data_cleanup(data_type, cutoff_date)
    ...
    async def _simulate_data_cleanup(self, data_type: str, cutoff_date: datetime) -> tuple[int, float]:
        """
