...
class TranscriptionService:
    ...
    def _initialize_engines(self):
        ...
        if SPEECH_RECOGNITION_AVAILABLE:
            try:
                ...
                self.google_recognizer.energy_threshold = 4000  # Adjust for children's voices
            ...
            logger.warning("Speech recognition not available - transcription will use mock responses")

    async def transcribe_audio(self, audio_data: bytes, language: str = None,
                             child_id: str = None) -> Dict[str, Any]:
        """
        ...
        if len(audio_data) > self.max_audio_size:
            raise ValueError(f"Audio too large: {len(audio_data)} bytes (max: {self.max_audio_size})")
        # Language validation
        language = language or self.language_default
        if language not in self.supported_languages:
            logger.warning(f"Unsupported language {language}, using {self.language_default}")
        ...
        try:
            ...
            if child_id:
                logger.info(f"Audio transcribed for child {child_id}: "
    ...
    async def _validate_audio_file(self, audio_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['duration'] > self.max_audio_duration:
                result['valid'] = False
                result['error'] = f"Audio too long: {result['duration']:.1f}s (max: {self.max_audio_duration}s)"
            return result
        except Exception as e:
            return {
                "valid": False,
    ...
    async def _apply_safety_filters(self, transcription_result: Dict[str, Any],
                                  child_id: str = None) -> Dict[str, Any]:
