...
class ResponseGenerator:
    ...
    """
    def __init__(self, api_key: str = None, model: str = "gpt-4") -> None:
        self.model = model
    ...
    async def generate_response(self, text: str, child_context: Dict[str, Any] = None,
                              response_type: ResponseType = ResponseType.CONVERSATIONAL) -> Dict[str, Any]:
        """
        ...
        if len(text) > self.max_input_length:
            raise ValueError(f"Input too long: {len(text)} chars (max: {self.max_input_length})")
        ...
        language = child_context.get('language', 'ar')
        ...
        try:
            ...
            if not filtered_input['safe']:
                return await self._generate_safety_redirect_response(
                    filtered_input['warnings'], safety_level, child_name
                )
            ...
            logger.info(f"Response generated for child age {child_age}: "
    ...
    async def _filter_input(self, text: str, safety_level: SafetyLevel) -> Dict[str, Any]:
        ...
        for pattern in self.unsafe_patterns:
            ...
            if matches:
                ...
                filtered_text = re.sub(pattern, "[محذوف]", filtered_text, flags=re.IGNORECASE)
        # Length validation
    ...
    async def _generate_openai_response(self, text: str, child_context: Dict[str, Any],
    ...
    async def _generate_fallback_response(self, text: str, safety_level: SafetyLevel,
                                        child_name: str = "صديقي") -> Dict[str, Any]:
    ...
    async def _filter_response(self, response_text: str, safety_level: SafetyLevel) -> Dict[str, Any]: 
    ...
    async def _generate_safety_redirect_response(self, warnings: List[str],
    ...
    async def _generate_emergency_fallback(self, safety_level: SafetyLevel,
                                         child_name: str) -> Dict[str, Any]:
