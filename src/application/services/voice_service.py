...
class VoiceService:
    ...
    async def speech_to_text(
        ...
        try:
            ...
            safety_result = await self.safety_monitor.check_content_safety(transcript, child_age=child_age)

            if safety_result.risk_level in [SafetyLevel.UNSAFE, SafetyLevel.POTENTIALLY_UNSAFE]:
                self.logger.warning(f"STT content blocked: Unsafe content detected for child age {child_age}. Transcript: '{transcript[:50]}...' Reason: {safety_result.analysis_details}")
            ...
            self.logger.error(f"Error during speech-to-text processing: {e}", exc_info=True)
    ...
    async def text_to_speech(
        ...
        try:
            ...
            safety_result = await self.safety_monitor.check_content_safety(text, child_age=child_age)

            if safety_result.risk_level in [SafetyLevel.UNSAFE, SafetyLevel.POTENTIALLY_UNSAFE]:
                self.logger.warning(f"TTS content blocked: Unsafe content detected for child age {child_age}. Text: '{text[:50]}...' Reason: {safety_result.analysis_details}")
            ...
            self.logger.error(f"Error during text-to-speech processing: {e}", exc_info=True)
