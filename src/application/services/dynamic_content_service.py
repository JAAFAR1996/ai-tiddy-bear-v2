...
class DynamicContentService:
    ...
    def __init__(self, api_client: IExternalAPIClient = None, safety_monitor: SafetyMonitor = None) -> None:
        """
    ...
    async def generate_personalized_story(
        ...
        if self.safety_monitor:
            safety_result = await self.safety_monitor.check_content_safety(story, child_age=child_profile.age)
            if safety_result.risk_level in [SafetyLevel.UNSAFE, SafetyLevel.POTENTIALLY_UNSAFE]:
                self.logger.warning(f"Unsafe story generated for child {child_profile.id}: {story[:100]}... Returning fallback.")
    ...
    async def generate_educational_content(
        ...
        # Validate generated content
        if self.safety_monitor:
            safety_result = await self.safety_monitor.check_content_safety(educational_text, child_age=child_profile.age)
            if safety_result.risk_level in [SafetyLevel.UNSAFE, SafetyLevel.POTENTIALLY_UNSAFE]:
                self.logger.warning(f"Unsafe educational content generated for child {child_profile.id}: {educational_text[:100]}... Returning fallback.")
    ...
    async def get_interactive_activity(
        ...
        if self.safety_monitor and isinstance(activity_response, str):
            safety_result = await self.safety_monitor.check_content_safety(activity_response, child_age=child_profile.age)
            if safety_result.risk_level in [SafetyLevel.UNSAFE, SafetyLevel.POTENTIALLY_UNSAFE]:
                self.logger.warning(f"Unsafe activity generated for child {child_profile.id}: {activity_response[:100]}... Returning empty activity.")
