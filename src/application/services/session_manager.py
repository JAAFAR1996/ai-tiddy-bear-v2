...
class SessionManager:
    ...
    async def create_session(
        ...
        """
        session = SessionData(
            child_id=child_id, client_ip=client_ip, user_agent=user_agent
        )
        await self.session_repository.save(
            session, self.session_config.session_timeout_minutes
        )
        self.logger.info(
    ...
    async def end_session(self, session_id: str) -> bool:
        ...
        if success:
            ...
            self.logger.warning(
                f"Attempted to end session {session_id}, but it was not found."
            )
        return success
    ...
    async def cleanup_expired_sessions(self) -> None:
        ...
        await self.session_repository.delete_expired(
