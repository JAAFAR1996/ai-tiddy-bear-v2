...
class ContentFilterService:
    ...
    def __init__(self, safety_monitor: SafetyMonitor, logger: logging.Logger = logger) -> None:
        """
    ...
    async def filter_content(self, text: str, age: int = 0) -> str:
        ...
        safety_result = await self.safety_monitor.check_content_safety(text, child_age=age)

        if safety_result.risk_level == SafetyLevel.UNSAFE:
            self.logger.warning(f"Content deemed UNSAFE for age {age}. Original: '{text[:50]}...' Reason: {safety_result.analysis_details}")
            return "[CONTENT BLOCKED]"
        elif safety_result.risk_level == SafetyLevel.POTENTIALLY_UNSAFE:
            self.logger.warning(f"Content deemed POTENTIALLY UNSAFE for age {age}. Original: '{text[:50]}...' Reason: {safety_result.analysis_details}")
