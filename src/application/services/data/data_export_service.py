...
class DataExportService:
        ...
        try:
            ...
            try:
                ...
                result.expires_at = datetime.utcnow() + timedelta(
                    days=EXPORT_LIMITS["retention_days"]
                )
    ...
    def _execute_export(self, request: ExportRequest) -> List[str]:
        ...
        metadata = ExportMetadata(
            ...
            date_range={
                "start": (
                    request.date_range_start.isoformat()
                    if request.date_range_start
                    else None
                ),
                "end": (
                    request.date_range_end.isoformat()
        ...
        for export_format in request.formats:
            ...
            # Save to file
            filename = (
                f"{request.child_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    def _gather_export_data(self, request: ExportRequest) -> Dict[str, Any]:
        ...
        for category in request.categories:
            if category == DataCategory.PROFILE:
                export_data["profile"] = self._get_child_profile(
                    request.child_id, child_service
                )
        ...
        try:
            if conversation_service:
                conversations = conversation_service.get_conversations(
                    child_id=child_id, start_date=start_date, end_date=end_date
                )
                if conversations:
                    return [
