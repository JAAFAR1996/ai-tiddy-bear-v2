...
class EmotionAnalyzer:
    ...
    async def analyze_text(self, text: str) -> EmotionResult:
        ...
        if not text or not text.strip():
            self.logger.warning("Received empty text for emotion analysis. Returning neutral.")
            return EmotionResult("neutral", 0.0, {}, 0.0, 0.0)
        if len(text) > 1000:
            self.logger.warning(f"Received very long text ({len(text)} chars) for emotion analysis. Truncating to 1000 chars.")
            text = text[:1000]
    ...
    async def analyze_multimodal(
        ...
        if not text and not audio_data:
            self.logger.warning("Received empty text and audio data for multimodal analysis. Returning neutral.")
            return {"primary": "neutral", "confidence": 0.0}
        if len(text) > 1000:
            self.logger.warning(f"Received very long text ({len(text)} chars) for multimodal analysis. Truncating to 1000 chars.")
            text = text[:1000]
        if len(audio_data) > (5 * 1024 * 1024):
            self.logger.warning(f"Received very large audio data ({len(audio_data)} bytes) for multimodal analysis. Skipping processing.")
            return {"primary": "neutral", "confidence": 0.0}

        try:
            # Placeholder for actual multimodal emotion analysis logic
            self.logger.debug(f"Analyzing multimodal input for emotions. Text: {text[:50]}..., Audio size: {len(audio_data)} bytes.")
            ...
            self.logger.error(f"Error during multimodal emotion analysis: {e}", exc_info=True)
