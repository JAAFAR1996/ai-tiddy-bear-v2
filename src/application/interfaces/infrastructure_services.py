...
class IAuditLogger(ABC):
    @abstractmethod
    async def log_child_access(self, parent_id: str, child_id: str, action: str, ip_address: str, success: bool) -> None:
        """Log child data access"""

    @abstractmethod
    async def log_consent_change(self, parent_id: str, child_id: str, consent_type: str, action: str, metadata: dict[str, Any]) -> None:
        """Log consent changes"""
...
class IContentFilterService(ABC):
    @abstractmethod
    async def filter_content(self, content: str, child_age: int, context: str = "general") -> dict[str, Any]:
