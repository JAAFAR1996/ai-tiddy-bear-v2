...
This module defines the data structures used to represent AI - generated responses
throughout the application layer. These DTOs ensure type safety and data consistency
when transferring AI response data between different layers of the hexagonal architecture.
...
class AIResponse:
    ...
    Represents a complete AI - generated response including text content, audio data, emotional analysis, and safety validation results.
    ...
    emotion: str = field(metadata={"description": "Detected emotion (happy, sad, excited, etc.)"})
    sentiment: float = field(metadata={"description": "Sentiment score from -1.0 (negative) to 1.0 (positive)"})
    safe: bool = field(metadata={"description": "Content safety validation result"})
    conversation_id: Optional[str] = field(default=None, metadata={"description": "Conversation tracking ID"})
    safety_score: float = field(default=1.0, metadata={"description": "Detailed safety score (0.0-1.0)"})
    moderation_flags: List[str] = field(default_factory=list, metadata={"description": "Content moderation warnings"})
    age_appropriate: bool = field(default=True, metadata={"description": "COPPA age appropriateness"})
    ...
    def __post_init__(self) -> None:
        ...
        """
        if not isinstance(self.sentiment, (int, float)) or not -1.0 <= self.sentiment <= 1.0:
            raise ValueError(f"Sentiment must be between -1.0 and 1.0, got {self.sentiment}")
            
        if not isinstance(self.safety_score, (int, float)) or not 0.0 <= self.safety_score <= 1.0:
