...
from typing import Any # Consolidated imports; Any covers the flexibility of Optional and Dict if types are not strictly constrained elsewhere.
...
from src.infrastructure.health.health_manager import HealthCheckManager, get_health_manager, HealthStatus # HealthCheckManager defines detailed health checks within src/infrastructure/health, ensuring transparency of monitoring.
from src.domain.exceptions.base import StartupValidationException # Moved to central exceptions module
...
async def lifespan(app: FastAPI):
    try:
        ...
        logger.critical(f"Failed to initialize FastAPILimiter or connect to Redis: {e}", exc_info=True)
        raise RuntimeError("Failed to initialize FastAPILimiter due to Redis connection issue") from e
...
def _mount_static_files(fast_app: FastAPI, project_root: Path) -> None:
    ...
    if environment == "production":
        logger.info("Skipping static file serving in production environment. Use Nginx or a CDN.")
        return
    ...
    if not static_dir.is_dir():
        ...
        try:
            ...
            raise ValueError("Invalid static files directory configuration: Path traversal detected.")
...
def create_app() -> FastAPI:
    ...
    fast_app = FastAPI(
        title=container.settings().application.APP_NAME, # Configured via settings for flexibility.
        description=container.settings().application.APP_NAME, # Configured via settings for flexibility.
        version=container.settings().application.APP_VERSION, # Configured via settings for flexibility.
        docs_url=container.settings().application.DOCS_URL, # Configured via settings for flexibility.
        redoc_url=container.settings().application.REDOC_URL, # Configured via settings for flexibility.
    ...
    project_root = container.settings().application.PROJECT_ROOT # Now directly a Path object
