...
class EmergencyContactValidator:
    ...
    def __init__(self) -> None:
        ...
        self.valid_relationships = {
            'parent', 'mother', 'father', 'guardian', 'grandparent',
            'grandmother', 'grandfather', 'aunt', 'uncle',
            'family_friend', 'neighbor', 'caregiver', 'babysitter'
        }
        ...
        self.compiled_patterns = [re.compile(pattern) for pattern in self.phone_patterns]
        
        logger.info("Emergency contact validator initialized")

    def validate_emergency_contact(self, contact_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        relationship_result = self._validate_relationship(contact_data.get('relationship', ''))
    ...
    def validate_emergency_contacts_list(self, contacts: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for i, contact in enumerate(contacts):
            ...
            if not result['valid']:
                global_errors.extend([f"Contact {i+1}: {error}" for error in result['errors']])
                continue
        ...
        if len(relationships) == 1 and len(validated_contacts) > 1:
            global_errors.append("Recommended: Include contacts with different relationships for redundancy")
    ...
    def _validate_relationship(self, relationship: str) -> Dict[str, Any]:
        ...
        if not relationship or not isinstance(relationship, str):
            return {
                'valid': False,
                'reason': 'Relationship is required'
            }
        ...
        for pattern in self.compiled_patterns:
            ...
            if match:
                ...
                if phone.startswith('+'):
                    ...
                    country_code = cleaned_phone[1:2] if len(cleaned_phone) > 10 else None
                else:
    ...
    def _validate_address(self, address: str) -> Dict[str, Any]:
        ...
        return {
            'valid': True,
            'sanitized_address': sanitized_address
        }
    ...
    def get_contact_accessibility_score(self, contacts: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        relationships = set(c.get('relationship', '') for c in contacts if c.get('relationship', ''))
        if len(relationships) > 1:
            score += 15.0
        elif len(contacts) > 1:
            recommendations.append('Include contacts with different relationships for redundancy')
        ...
        if len(contacts_with_address) > 1:
            ...
            recommendations.append('Consider contacts in different locations for emergencies')
