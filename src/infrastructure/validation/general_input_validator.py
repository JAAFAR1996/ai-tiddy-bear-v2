...
class GeneralInputValidator:
        ...
        try:
            ...
            if parsed_url.scheme not in allowed_schemes:
                return ValidationResult(
                    ...
                    metadata={"code": "INVALID_URL_SCHEME", "allowed_schemes": allowed_schemes}
                )
    ...
    def validate_file_upload(self, filename: str, file_size: int,
                           file_content: bytes = None) -> ValidationResult:
        """
        ...
        if file_size > max_size:
            return ValidationResult(
                valid=False,
                errors=[f"File size {file_size} bytes exceeds maximum {max_size} bytes"],
                metadata={"code": "FILE_TOO_LARGE", "size": file_size, "max_size": max_size}
            )
    ...
    def validate_text_input(self, text: str, max_length: int = 1000) -> ValidationResult:
        """
        ...
        if len(text) > max_length:
            return ValidationResult(
                valid=False,
                errors=[f"Text length {len(text)} exceeds maximum {max_length} characters"],
                metadata={"code": "TEXT_TOO_LONG", "length": len(text), "max_length": max_length}
            )
