...
class SafetyMonitorService:
    ...
    def _load_blocked_words(self) -> List[str]:
        ...
        default_blocked_words = [
            "violence", "kill", "death", "weapon", "gun", "knife", "blood",
            "hate", "stupid", "idiot", "dumb", "moron", "loser",
            "scary", "nightmare", "monster", "demon", "evil",
        ...
        if self.settings.content_moderation.ADDITIONAL_BLOCKED_WORDS:
            default_blocked_words.extend(self.settings.content_moderation.ADDITIONAL_BLOCKED_WORDS)
        
        self.logger.info(f"Loaded {len(default_blocked_words)} blocked words from configuration")
    ...
    async def check_content(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            is_safe = safety_score >= self.settings.content_moderation.SAFETY_THRESHOLD and len(flags) == 0
            ...
            self.logger.info(f"Content safety check: {is_safe}, score: {safety_score:.2f}")
            return result
        except Exception as e:
            self.logger.error(f"Error checking content safety: {str(e)}")
            return {
                "safe": False,
                "confidence": 0.0,
    ...
    async def check_audio(self, audio_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Audio safety check: {is_safe}, score: {safety_score:.2f}, duration: {estimated_duration:.1f}s")
            return result
        except Exception as e:
            self.logger.error(f"Error checking audio safety: {str(e)}")
            return {
                "safe": False,
                "confidence": 0.0,
