...
class DatabaseInputValidator:
    ...
    def validate_input_data(self, data: Dict[str, Any], table_name: str) -> Dict[str, Any]:
        ...
        for field, value in data.items():
            ...
            if isinstance(value, str):
                sanitization_result = self.sql_prevention.sanitize_input(value, input_type)
                if not sanitization_result.safe:
                    threats_detected.extend(sanitization_result.threats_found)
                    logger.warning(f"Threats detected in field '{field}': {sanitization_result.threats_found}")
                ...
                if sanitization_result.modifications:
                    logger.info(f"Input sanitized for field '{field}': {sanitization_result.modifications}")
            
            elif isinstance(value, (int, float)):
                # Validate numeric ranges
                validated_data[field] = self._validate_numeric_input(field, value, table_name)
                ...
                logger.debug(f"Non-string field '{field}' passed through: {type(value)}")
    ...
    def _validate_numeric_input(self, field_name: str, value: Union[int, float], table_name: str) -> Union[int, float]:
        ...
        if field_name in numeric_ranges:
            ...
            if not (min_val <= value <= max_val):
                raise ValueError(f"Value {value} for field '{field_name}' is outside valid range [{min_val}, {max_val}]")
        
        # Additional validation for specific cases
        if field_name == 'age' and table_name == 'children':
            if value > 13:
    ...
    def validate_query_execution(self, query: str, params: Optional[Dict[str, Any]] = None) -> QueryValidationResult:
        ...
        if not validation_result.safe:
            ...
            raise SecurityError(f"Dangerous SQL query blocked: {validation_result.errors}")
        ...
        async def wrapper(*args, **kwargs):
            ...
            for param_name in ['data', 'user_data', 'child_data', 'update_data']:
                if param_name in kwargs:
                    validated_data = validator.validate_input_data(kwargs[param_name], table_name)
            ...
            if 'query' in kwargs and 'params' in kwargs:
                validation_result = validator.validate_query_execution(kwargs['query'], kwargs['params'])
                if not validation_result.safe:
...
def validate_database_operation(operation: str, table_name: str, data: Dict[str, Any] = None,
                               where_conditions: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    ...
    if where_conditions:
        validated_operation["where_conditions"] = validator.validate_input_data(where_conditions, table_name)
...
class SafeDatabaseSession:
    ...
    async def safe_execute(self, query: str, params: Optional[Dict[str, Any]] = None) -> Any:
    ...
    async def safe_insert(self, table_name: str, data: Dict[str, Any]) -> Any:
        ...
        query, params = self.validator.query_builder.build_insert(table_name, validated_data)
        return await self.safe_execute(query, params)
    
    async def safe_update(self, table_name: str, data: Dict[str, Any], where_conditions: Dict[str, Any]) -> Any:
        """تحديث آمن للبيانات"""
        validated_data = self.validator.validate_input_data(data, table_name)
        validated_where = self.validator.validate_input_data(where_conditions, table_name)
        query, params = self.validator.query_builder.build_update(table_name, validated_data, validated_where)
        return await self.safe_execute(query, params)
    
    async def safe_select(self, table_name: str, columns: List[str], where_conditions: Dict[str, Any] = None, limit: int = None):
        """استعلام آمن للبيانات"""
        validated_where = None
        if where_conditions:
            validated_where = self.validator.validate_input_data(where_conditions, table_name)
        
        query, params = self.validator.query_builder.build_select(table_name, columns, validated_where, limit)
        return await self.safe_execute(query, params)
