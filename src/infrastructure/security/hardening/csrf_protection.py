    ...
    def invalidate_session_tokens(self, session_id: str) -> int:
        ...
        try:
            ...
            for token in tokens_to_remove:
                del self.token_cache[token]
            logger.info(f"Invalidated {len(tokens_to_remove)} CSRF tokens for session {session_id}")
    ...
    def validate_request(self, request: Request) -> bool:
        ...
        try:
            ...
            if not csrf_token:
                ...
                return False
            # Validate token
            is_valid = self.token_manager.validate_token(csrf_token, session_id, user_id)
            if not is_valid:
                logger.warning(
...
def get_csrf_protection() -> CSRFProtection:
    ...
    if _csrf_protection is None:
        ...
        logger.warning("Using default CSRF configuration - configure properly for production")
