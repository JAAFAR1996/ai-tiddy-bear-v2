...
class RedisRateLimiter:
    ...
    def _check_local_limit(self, identifier: str) -> RateLimitResult:
        ...
        if len(timestamps) >= self.config.requests_per_minute:
            return RateLimitResult(allowed=False, remaining=0, reset_time=now + 60, retry_after=60)

        timestamps.append(now)
        return RateLimitResult(allowed=True, remaining=self.config.requests_per_minute - len(timestamps), reset_time=now + 60)