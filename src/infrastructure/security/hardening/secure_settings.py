...
try:
    ...
    logger.critical("Install required dependencies: pip install pydantic pydantic-settings")
    raise ImportError(f"Missing required dependency: pydantic. This application cannot run without it.") from e
...
class SecureAppSettings(BaseSettings):
    ...
    # Security Settings
    secret_key: SecretStr = Field(..., min_length=32, description="Application secret key")
    jwt_secret: SecretStr = Field(..., min_length=32, description="JWT secret key")
    encryption_key: SecretStr = Field(..., min_length=32, description="Data encryption key")
    ...
    database_max_overflow: int = Field(default=20, ge=0, le=100, description="Database max overflow")

    # AI Service Settings
    openai_api_key: SecretStr = Field(..., description="OpenAI API key")
    anthropic_api_key: Optional[SecretStr] = Field(default=None, description="Anthropic API key")
    azure_speech_key: Optional[SecretStr] = Field(default=None, description="Azure Speech API key")
