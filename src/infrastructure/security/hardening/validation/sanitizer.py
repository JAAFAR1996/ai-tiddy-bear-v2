...
class InputSanitizer:
    ...
    def sanitize_string(
        ...
        try:
            # Check length limits
            max_length = (
                self.config.child_max_string_length if is_child_input
            ...
            if is_child_input:
                if not re.match(self.config.child_safe_pattern, value):
                    ...
                    result["sanitized"] = re.sub(
                        r'[^a-zA-Z0-9\\s\\.\\,\\!\\?\\-\\\'\\\"]', '', result["sanitized"]
                    )
                    result["is_safe"] = False
            ...
            for pattern, rule in self.compiled_patterns:
                if pattern.search(result["sanitized"]):
                    ...
                    if rule.action == "block":
                        ...
                        result["sanitized"] = pattern.sub("[REDACTED]", result["sanitized"])
    ...
    def sanitize_json(self, data: Any, is_child_input: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            result["violations"].append({
                ...
                "severity": "critical",
                "message": f"Failed to process JSON safely: {str(e)}"
            })
            return result
        ...
        if isinstance(data, dict):
            ...
            if len(data) > self.config.max_array_length:
                result["violations"].append({
                    ...
                    "severity": "medium",
                    "message": f"Object size exceeds limit"
                })
                result["is_safe"] = False
            sanitized = {}
            for key, value in data.items():
                # Sanitize key
                key_result = self.sanitize_string(str(key), is_child_input, "object_key")
                ...
                sanitized[key_result["sanitized"]] = self._sanitize_json_recursive(
                    value, is_child_input, depth + 1, result
                )
            return sanitized
        elif isinstance(data, list):
            # Check array size
            max_length = (
                self.config.child_max_array_length if is_child_input
            ...
            if len(data) > max_length:
                result["violations"].append({
                    ...
                    "severity": "medium",
                    "message": f"Array size exceeds limit"
                })
                result["is_safe"] = False
