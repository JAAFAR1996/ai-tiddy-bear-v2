...
import re
import secrets
from src.infrastructure.security.registration_models import RegistrationRequest, PasswordRequirements
...
class SecureUserRegistration:
    ...
    def __init__(self) -> None:
        ...
        self.password_requirements.forbidden_patterns = [
            ...
            "teddy", "bear", "child", "parent", "family"
        ]
    ...
    def validate_parent_eligibility(self, registration_data: RegistrationRequest) -> Dict[str, Any]:
        ...
        if registration_data.date_of_birth:
            try:
                birth_date = datetime.strptime(registration_data.date_of_birth, "%Y-%m-%d")
                age = (datetime.now() - birth_date).days // 365
                
                if age < self.min_parent_age:
                    validation["eligible"] = False
                    validation["errors"].append(f"Must be at least {self.min_parent_age} years old to register")
                elif age > 100:
                    validation["warnings"].append("Please verify the date of birth is correct")
            except ValueError:
    ...
    def register_parent_user(self, registration_data: RegistrationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            password_validation = validate_password_strength(
                ...
                {"full_name": registration_data.full_name, "email": registration_data.email}
            )
            ...
            # 5. توليد رمز التحقق
            verification_token = self.generate_verification_token(registration_data.email)
            ...
            return {
                ...
                "verification_expires": (datetime.utcnow() + self.verification_token_expiry).isoformat(),
