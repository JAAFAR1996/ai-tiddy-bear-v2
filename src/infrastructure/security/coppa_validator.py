...
class COPPAValidator:
    ...
    def validate_age_compliance(
        ...
        if not coppa_enabled:
            ...
            logger.debug(
                f"COPPA compliance disabled - relaxed age validation for age {age}",
            )
        ...
        if age < self.MIN_CHILD_AGE:
            raise ValueError(
                f"Age {age} is below minimum allowed age of {self.MIN_CHILD_AGE}",
            )

        if strict_validation and age > self.MAX_CHILD_AGE:
            raise ValueError(
                f"Age {age} exceeds maximum child age of {self.MAX_CHILD_AGE}",
            )
        ...
        logger.info(
            f"COPPA validation completed: age_group={compliance_level.value}, "
    ...
    def is_coppa_subject(self, age: int) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Invalid age for COPPA check: {age}, assuming COPPA subject",
            )
            return True
    ...
    def get_data_retention_period(self, age: int) -> int:
        ...
        try:
            ...
            logger.warning(
                f"Invalid age for retention check: {age}, using COPPA retention",
            )
