...
class InputValidationMiddleware(BaseHTTPMiddleware):
    ...
    """
    def __init__(self, app, enable_child_safety: bool = True, strict_mode: bool = False):
        super().__init__(app)
        ...
        try:
            ...
            if request.query_params:
                for key, value in request.query_params.items():
                    result = await self.validator.validate_input(value, f"query.{key}", context)
                    validation_results.append((f"query.{key}", result))
            
            # Validate path parameters
            if hasattr(request, "path_params") and request.path_params:
                for key, value in request.path_params.items():
                    result = await self.validator.validate_input(value, f"path.{key}", context)
                    validation_results.append((f"path.{key}", result))
            
            # Validate headers (selective)
            suspicious_headers = ["x-forwarded-for", "user-agent", "referer", "authorization"]
            for header in suspicious_headers:
                value = request.headers.get(header)
                if value:
                    result = await self.validator.validate_input(value, f"header.{header}", context)
            ...
            if request.method in ["POST", "PUT", "PATCH"]:
                try:
                    ...
                    if body:
                        result = await self.validator.validate_input(body, "body", context)
            ...
            should_block = await self._should_block_request(
                all_threats, all_child_safety_violations, context
            )
            
            if should_block:
                # Log security incident
                await self._log_security_incident(request, all_threats, all_child_safety_violations)
            ...
            if all_threats or all_child_safety_violations:
                await self._log_security_warning(request, all_threats, all_child_safety_violations)
    ...
    async def _extract_request_context(self, request: Request) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_child_endpoint": any(child_ep in str(request.url.path) for child_ep in self.child_endpoints),
            "is_auth_endpoint": any(auth_ep in str(request.url.path) for auth_ep in self.auth_endpoints)
        }
    ...
    async def _get_request_body(self, request: Request) -> Optional[Any]:
        ...
        try:
            ...
            if "application/json" in content_type:
                ...
                form_data = await request.form()
                return {key: value for key, value in form_data.items() if not hasattr(value, 'read')}
            return None
        except Exception as e:
