...
class ComprehensiveInputValidator(ThreatDetectors):
    ...
    async def validate_input(
        ...
        try:
            ...
            child_safety_violations.extend(await self.detect_inappropriate_content(text_data, field_name))
            child_safety_violations.extend(await self.detect_pii(text_data, field_name))
            
            # Validate input size
            if len(text_data) > 100000:  # 100KB limit
                threats.append(SecurityThreat(
                    "oversized_input", "high", field_name, text_data[:100],
            ...
            is_valid = (
                len(critical_threats) == 0 and
                len(high_threats) == 0 and
            ...
            return InputValidationResult(False, threats, errors, child_safety_violations)
