...
class ChildData(BaseModel):
    ...
    date_of_birth: Optional[datetime] = Field(None, description="Date of birth for precise age calculation")
    ...
    consent_date: Optional[datetime] = Field(None, description="Date consent was given")
    consent_type: str = Field(default="explicit", description="Type of consent obtained")
    
    # Data collection permissions
    data_collection_consent: bool = Field(default=False, description="Consent for data collection")
    voice_recording_consent: bool = Field(default=False, description="Consent for voice recording")
    usage_analytics_consent: bool = Field(default=False, description="Consent for usage analytics")
    ...
    location_tracking: bool = Field(default=False, description="Location tracking permission")
    third_party_sharing: bool = Field(default=False, description="Third-party data sharing permission")
    ...
    @classmethod
    def validate_consent_date(cls, v: Optional[datetime], info) -> Optional[datetime]:
...
class ParentConsent(BaseModel):
    ...
    consent_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique consent ID")
    parent_id: str = Field(..., description="Parent identifier")
    child_id: str = Field(..., description="Child identifier")
    
    # Consent details
    consent_type: str = Field(..., description="Type of consent (data_collection, voice_recording, etc.)")
    ...
    verification_method: str = Field(..., description="How consent was verified (email, sms, etc.)")
...
class DataRetentionPolicy(BaseModel):
    ...
    deletion_scheduled_date: Optional[datetime] = Field(None)
...
class AuditLogEntry(BaseModel):
    ...
    # Event details
    event_type: str = Field(..., description="Type of event (data_access, consent_change, etc.)")
    event_description: str = Field(..., description="Detailed description of the event")
    
    # Actor information
    actor_type: str = Field(..., description="Who performed the action (user, system, admin)")
    actor_id: Optional[str] = Field(None, description="ID of the actor")
    
    # Target information
    target_type: str = Field(..., description="What was affected (child_data, consent, etc.)")
    ...
    # Compliance flags
    coppa_relevant: bool = Field(default=True, description="Whether this event is relevant for COPPA compliance")
    gdpr_relevant: bool = Field(default=False, description="Whether this event is relevant for GDPR compliance")
...
class DataDeletionRequest(BaseModel):
    ...
    # Request details
    deletion_type: str = Field(..., description="Type of deletion (full, partial, specific)")
    data_types: List[str] = Field(default_factory=list, description="Specific data types to delete")
    reason: str = Field(..., description="Reason for deletion request")
    
    # Timing
    requested_at: datetime = Field(default_factory=datetime.utcnow)
    scheduled_for: Optional[datetime] = Field(None, description="When deletion should occur")
    ...
    verification_required: bool = Field(default=True, description="Whether parent verification is required")
    verified_at: Optional[datetime] = Field(None, description="When parent verification was completed")
    
    # Audit
    processed_by: Optional[str] = Field(None, description="System component that processed the request")
    processing_notes: Optional[str] = Field(None, description="Notes about the processing")
