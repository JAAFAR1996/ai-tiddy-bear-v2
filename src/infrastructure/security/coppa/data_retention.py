...
from ...config.coppa_config import is_coppa_enabled, get_data_retention_days
from .data_models import ChildData, DataRetentionPolicy, DataDeletionRequest
...
class DataRetentionManager:
    ...
    def __init__(self) -> None:
        ...
        if is_coppa_enabled():
            ...
            logger.info("Data retention initialized with COPPA compliance disabled - extended retention")
    
    async def create_retention_policy(self, child_data: ChildData) -> DataRetentionPolicy:
        """
        ...
        logger.info(f"Created retention policy for child {child_data.child_id}: {retention_days} days")
        return policy
    ...
    def calculate_retention_period(self, child_data: ChildData) -> int:
        ...
        return get_data_retention_days(child_data.age)
        
        # Adjust based on consent types
        if child_data.voice_recording_consent:
            # Voice data has shorter retention
            return min(base_retention, 30)
        if child_data.usage_analytics_consent:
    ...
    async def schedule_automatic_deletion(self, child_id: str) -> bool:
        ...
        if not is_coppa_enabled():
            logger.info(f"COPPA compliance disabled - skipping automatic deletion scheduling for child {child_id}")
        ...
        try:
            ...
            deletion_request = DataDeletionRequest(
                ...
                scheduled_for=datetime.utcnow() + timedelta(days=self.default_retention_days)
            )
            ...
            for child_data in expired_children:
                try:
                    ...
                    if self._is_grace_period_expired(child_data):
                        deletion_success = await self._perform_graduated_deletion(child_data["child_id"])
                        if deletion_success:
                            results["deleted"] += 1
                            logger.info(f"Successfully deleted data for child {child_data['child_id']}")
                        else:
                            results["errors"] += 1
                except Exception as e:
                    logger.error(f"Error processing child {child_data.get('child_id', 'unknown')}: {e}")
                    results["errors"] += 1
    ...
    async def _find_expired_children(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Found {len(mock_expired)} children with expired retention periods")
            return mock_expired
        except Exception as e:
    ...
    def _is_grace_period_expired(self, child_data: Dict[str, Any]) -> bool:
        ...
        grace_period_end = notification_date + timedelta(days=self.warning_days_before_deletion)
    ...
    async def _delete_voice_data(self, child_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Securely deleted voice data for child {sanitized_id} (proof: {deletion_proof[:8]})")
            return True
        except Exception as e:
            sanitized_id = hashlib.sha256(f"child_{child_id}".encode()).hexdigest()[:8]
    ...
    async def _log_compliance_summary(self, results: Dict[str, int]) -> None:
        ...
        try:
            summary = {
                ...
                "results": results,
                "compliance_status": "completed" if results["errors"] == 0 else "partial_errors"
            }
    ...
    async def notify_parent_before_deletion(self, child_id: str, deletion_date: datetime) -> bool:
        """Notify parent before scheduled deletion"""
        try:
            # This would send email/SMS to parent
            logger.info(f"Would notify parent about upcoming deletion for child {child_id} on {deletion_date}")
            return True
        except Exception as e:
            logger.error(f"Failed to notify parent for child {child_id}: {e}")
            return False
    
    async def extend_retention_period(self, child_id: str, additional_days: int) -> bool:
