...
Digital Signature Required Below"""
    ...
    async def complete_verification(
        ...
        try:
            ...
            is_verified = await self._validate_completion(
                method,
                session,
                completion_data,
            )
    ...
    async def _validate_completion(
        ...
        if method == VerificationMethod.DIGITAL_SIGNATURE:
    ...
    async def validate_consent(
        ...
        if consent_record["status"] != "active":
            return {
                "valid": False,
                "error": f"Consent status: {consent_record['status']}",
            }
        ...
        if required_permission not in consent_record["permissions"]:
            return {
                "valid": False,
                "error": f"Permission not granted: {required_permission}",
            }
