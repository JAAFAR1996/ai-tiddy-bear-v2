...
from src.infrastructure.security.key_rotation_service import (
    KeyType, RotationTrigger, RotationResult, KeyStorageInterface,
)
        ...
        with self._lock:
            try:
                ...
                if not current_key_data:
                    return RotationResult(
                        success=False, old_key_id=key_id, error_message="Key not found"
                    )
                ...
                new_key_id = self.lifecycle_manager.create_key(
                    current_metadata.key_type, current_metadata.algorithm
                )
                ...
                return RotationResult(
                    success=False, old_key_id=key_id, error_message=str(e)
                )
            finally:
