...
from src.infrastructure.security.key_rotation_service import (
    KeyType, RotationTrigger, RotationResult, KeyStorageInterface, FileKeyStorage,
)
...
class KeyRotationOrchestrator:
    ...
    def __init__(self, storage: Optional[KeyStorageInterface]=None) -> None:
        ...
        self.executor = RotationExecutor(
            self.storage, self.generator, self.lifecycle_manager, self.policy_manager
        )
    ...
    def rotate_key(
        self, key_id: str, trigger: RotationTrigger = RotationTrigger.MANUAL
    ) -> RotationResult:
    ...
    def rotate_all_keys(
        self, key_type: Optional[KeyType] = None
    ) -> List[RotationResult]:
        ...
        for key_metadata in keys:
            if key_metadata.is_active:
                result = self.executor.rotate_key(
                    key_metadata.key_id, RotationTrigger.MANUAL
                )
    ...
    def emergency_rotation(
        self, trigger: RotationTrigger = RotationTrigger.SECURITY_INCIDENT
    ) -> Dict[str, Any]:
