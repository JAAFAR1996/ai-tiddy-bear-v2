...
from ..config.coppa_config import is_coppa_enabled, requires_coppa_audit_logging
...
def log_child_activity(child_id: str, activity: str, details: Optional[Dict] = None) -> None:
...
def log_parent_action(parent_id: str, action: str, child_id: Optional[str] = None) -> None:
    ...
    if child_id:
        safe_child_id = logger._sanitize_child_id(child_id)
        logger.info(f"Parent action: {safe_parent_id} - {action} - child: {safe_child_id}")
    else:
        logger.info(f"Parent action: {safe_parent_id} - {action}")

def log_safety_event(child_id: str, event_type: str, severity: str, details: Optional[str] = None) -> None:
    """Log safety event with automatic sanitization"""
    logger = get_secure_logger("child_safety")
    safe_child_id = logger._sanitize_child_id(child_id)
    if details:
        logger.warning(f"Safety event: {safe_child_id} - {event_type} ({severity}) - {details}")
    else:
...
def log_coppa_event(child_id: str, event_type: str, consent_status: str) -> None:
    ...
    safe_child_id = logger._sanitize_child_id(child_id)
    logger.info(f"COPPA event: {safe_child_id} - {event_type} - consent: {consent_status}")
