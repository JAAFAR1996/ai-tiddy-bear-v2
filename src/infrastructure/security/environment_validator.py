...
class EnvironmentSecurityValidator:
    ...
    RECOMMENDED_SECRETS = {
        'VAULT_TOKEN': {'min_length': 20, 'description': 'HashiCorp Vault token'},
        'SENTRY_DSN': {'min_length': 20, 'description': 'Sentry error tracking'},
        'PARENT_VERIFICATION_API_KEY': {'min_length': 32, 'description': 'Parent verification service'}
    }
    ...
    def validate_all(cls) -> Tuple[bool, List[str]]:
        ...
        for var_name, rules in cls.REQUIRED_SECRETS.items():
            ...
            if not value:
                errors.append(f"Missing required secret: {var_name} ({rules['description']})")
                continue
    ...
    def validate_and_exit_on_error(cls) -> None:
        ...
        if not is_valid:
            ...
            logger.error("  python -c \"from src.infrastructure.security.environment_validator import EnvironmentSecurityValidator; EnvironmentSecurityValidator.generate_secure_secret()\"")
            sys.exit(1)
    ...
    def get_secure_config(cls) -> Dict[str, str]:
        ...
        if is_development:
            for var_name in cls.REQUIRED_SECRETS:
                value = os.getenv(var_name)
                if not value or any(d in value for d in cls.REQUIRED_SECRETS[var_name].get('no_defaults', [])):
                    if var_name == 'DATABASE_URL':
                        config[var_name] = 'postgresql://dev:dev@localhost:5432/ai_teddy_dev'
                    elif var_name == 'OPENAI_API_KEY':
                        config[var_name] = 'sk-development-mock-key-' + secrets.token_hex(20)
                    else:
