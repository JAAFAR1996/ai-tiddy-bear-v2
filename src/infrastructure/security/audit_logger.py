            ...
            try:
                ...
                async with self.buffer_lock:
                    if len(self.audit_entries) >= self.config.batch_size:
                        ...
                        self.audit_entries = self.audit_entries[self.config.batch_size:]
...
async def log_audit_event(
    ...
    **kwargs) -> str:
    ...
    return await audit_logger.log_event(event_type, severity, category, description, **kwargs)
...
async def log_child_safety_incident(
    ...
    **kwargs) -> str:
    ...
    audit_logger = get_audit_logger()
    return await audit_logger.log_safety_incident(child_id, incident_type, description, **kwargs)