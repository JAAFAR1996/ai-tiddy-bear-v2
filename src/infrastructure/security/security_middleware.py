...
class RateLimiter:
    ...
    def __init__(self, redis_client: redis.Redis, requests_per_minute: int = 60, burst_limit: int = 10) -> None:
    ...
    async def record_suspicious_activity(self, ip: str, activity: str) -> None:
        ...
        await self.redis.expire(key, timedelta(hours=24).total_seconds())
        
        # Check if IP should be blocked
        activity_count = await self.redis.zcard(key)
        if activity_count >= 5:
            block_key = f"blocked_ip:{ip}"
            await self.redis.setex(block_key, timedelta(hours=1).total_seconds(), "blocked")
    ...
    def scan_request(self, request_data: str) -> List[str]:
        ...
        for i, pattern in enumerate(self.compiled_patterns):
            if pattern.search(request_data):
                threat_type = [
                    'XSS', 'JavaScript Injection', 'VBScript Injection',
                    'Event Handler Injection', 'Error Handler Injection',
                    'JavaScript Eval', 'Code Execution', 'System Calls',
                    'SQL Injection', 'SQL Injection', 'SQL Injection',
                    'SQL Injection', 'SQL Injection', 'Path Traversal',
                    'System File Access', 'System File Access'
                ][i]
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, redis_client: redis.Redis, rate_limiter: RateLimiter = None, validator: SecurityValidator = None) -> None:
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        try:
            ...
            if not await self.rate_limiter.is_allowed(client_ip):
                await self.rate_limiter.record_suspicious_activity(client_ip, "Rate limit exceeded")
            ...
            if security_issues:
                self.record_suspicious_activity(client_ip, f"Security threats: {security_issues}")
                logger.warning(f"Security threats detected from {client_ip}: {security_issues}")
    ...
    def is_ip_blocked(self, ip: str) -> bool:
        ...
        if ip in self.blocked_ips:
            ...
            if datetime.now() - block_time < timedelta(hours=1):  # حظر لساعة واحدة
                return True
            else:
                del self.blocked_ips[ip]  # إزالة الحظر المنتهي
        return False

    def record_suspicious_activity(self, ip: str, activity: str) -> None:
        """تسجيل النشاط المشبوه"""
        now = datetime.now()
        self.suspicious_activity[ip].append(now)
        
        # تنظيف النشاطات القديمة (أكثر من 24 ساعة)
        self.suspicious_activity[ip] = [
            activity_time for activity_time in self.suspicious_activity[ip]
            if now - activity_time < timedelta(hours=24)
        ]
        
        # حظر IP إذا تجاوز عدد النشاطات المشبوهة
    ...
    async def scan_request_security(self, request: Request) -> List[str]:
        ...
        for header_name, header_value in request.headers.items():
            header_threats = self.validator.scan_request(f"{header_name}: {header_value}")
            ...
            try:
                ...
                if body:
                    ...
                    try:
                        ...
                        logger.debug(f"Body is not valid JSON, skipping COPPA validation: {e}")
    ...
    def log_request(self, request: Request, response: Response, duration: float, client_ip: str) -> None:
        """تسجيل الطلب"""
        log_data = {
