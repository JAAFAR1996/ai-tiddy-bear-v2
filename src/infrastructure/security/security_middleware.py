...
try:
    ...
    logger.error(
        "Install required dependencies: pip install fastapi starlette"
    )
...
class RateLimiter:
    ...
    async def record_suspicious_activity(self, ip: str, activity: str) -> None:
        ...
        if activity_count >= 5:
            ...
            logger.warning(
                f"IP {ip} blocked due to repeated suspicious activity"
            )
            ...
            try:
                ...
                if body:
                    ...
                    try:
                        ...
                        coppa_violations = self.validator.validate_child_data(
                            json_data
                        )
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if self.is_child_request(request):
            ...
            if not await self.child_rate_limiter.is_allowed(client_ip):
                ...
                raise HTTPException(
                    status_code=429,
                    detail="تم تجاوز حد الطلبات المسموح للأطفال"
                )
                ...
                if body:
                    ...
                    for word in inappropriate_words:
                        if word.lower() in body_str.lower():
                            ...
                            raise HTTPException(
                                status_code=400,
                                detail="المحتوى غير مناسب للأطفال"
                            )
