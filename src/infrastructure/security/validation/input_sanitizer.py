...
class InputSanitizer:
    ...
    def sanitize_child_input(self, input_value: str, context: str = "general") -> InputSanitizationResult:
        ...
        for pattern in self.child_unsafe_patterns:
            ...
            if matches:
                threats_found.extend([f"Child unsafe pattern: {match}" for match in matches])
                sanitized = re.sub(pattern, '[FILTERED]', sanitized, flags=re.IGNORECASE)
                modifications.append(f"Filtered child-unsafe content: {pattern}")
        ...
        if len(sanitized) > max_len:
            ...
            modifications.append(f"Truncated to {max_len} characters for child safety")
    ...
    def validate_pattern(self, input_value: str, pattern_type: str) -> bool:
        ...
        if pattern_type in self.safe_patterns:
            return bool(re.match(self.safe_patterns[pattern_type], input_value))
        return False
