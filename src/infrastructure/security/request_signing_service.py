...
class RequestSigningService:
    ...
    Features: - HMAC - based request signatures - Replay attack prevention - Timestamp validation - Nonce tracking - Child - specific security levels - Key rotation support
    """
    def __init__(self, default_level: RequestSecurityLevel = RequestSecurityLevel.CHILD_INTERACTION) -> None:
        ...
        logger.info(f"Request signing service initialized with {default_level.value} level")

    def _initialize_configurations(self) -> Dict[RequestSecurityLevel, SignatureConfiguration]:
    ...
    def sign_request(self,
                    ...
                    custom_claims: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
        """
        ...
        # Add custom claims
        if custom_claims:
            claims_json = json.dumps(custom_claims, sort_keys=True, separators=(',', ':'))
        ...
        logger.debug(f"Generated signature for {method} {url} with level {level.value}")
    ...
    def validate_request_signature(self,
                                 ...
                                 body: Optional[bytes] = None,
                                 level: Optional[RequestSecurityLevel] = None) -> SignatureValidationResult:
        """
        ...
        Returns: SignatureValidationResult with validation details
        """
        level = level or self.default_level
        config = self.configurations[level]
        # Check for required signature headers
        required_headers = ["X-Signature", "X-Signature-Timestamp", "X-Signature-KeyID"]
        ...
        try:
            ...
            if time_diff > config.timestamp_tolerance_seconds:
                return SignatureValidationResult(
                    valid=False,
                    reason=f"Request timestamp outside tolerance ({time_diff}s > {config.timestamp_tolerance_seconds}s)",
        ...
        if provided_algorithm != expected_algorithm:
            return SignatureValidationResult(
                valid=False,
                reason=f"Algorithm mismatch: expected {expected_algorithm}, got {provided_algorithm}",
        ...
        signature_hash = hashlib.sha256(provided_signature.encode()).hexdigest()
    ...
    def _generate_signature(self, payload: str, config: SignatureConfiguration) -> str:
        """Generate HMAC signature for payload."""
        if config.algorithm == SignatureAlgorithm.HMAC_SHA256:
            digest = hmac.new(config.secret_key, payload.encode('utf-8'), hashlib.sha256)
        elif config.algorithm == SignatureAlgorithm.HMAC_SHA512:
            digest = hmac.new(config.secret_key, payload.encode('utf-8'), hashlib.sha512)
        else:
    ...
    def _cleanup_nonce_cache(self, max_age_seconds: int) -> None:
        ...
        expired_nonces = [nonce for nonce, created in self.nonce_cache.items()
                         if created < cutoff]
    ...
    def _cleanup_caches(self) -> None:
        ...
        signature_cutoff = now - timedelta(hours=24)  # Keep signatures for 24 hours
        expired_signatures = [sig for sig, created in self.signature_cache.items()
    ...
    def rotate_signing_key(self, level: RequestSecurityLevel, new_secret_key: bytes) -> bool:
        """
        Rotate signing key for a security level.
        Args: level: Security level to update
        ...
        try:
            ...
            logger.info(f"Rotated signing key for {level.value}: {old_key_id} -> {new_key_id}")
            return True
        except Exception as e:
            logger.error(f"Failed to rotate signing key for {level.value}: {e}")
            return False

    def create_child_safe_headers(self, child_id: str, age: int) -> Dict[str, str]:
        """
        Create additional headers for child safety.
        Args: child_id: Child identifier
            age: Child's age
        Returns: Dict of child safety headers
        """
        headers = {
            "X-Child-ID": child_id,
