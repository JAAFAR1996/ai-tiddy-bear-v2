...
class RobustEncryptionService:
    ...
    - Key rotation with secure key management
    - Multiple encryption algorithms support
    - Integrity verification for all encrypted data
    - COPPA compliance with enhanced child data protection
    ...
    async def encrypt(
        ...
        Returns:
            EncryptionResult with detailed status
        """
        ...
        try:
            ...
            if not self._crypto_available:
                ...
                if policy == EncryptionPolicy.OPTIONAL:
                    await self._log_encryption_event(
                        "encryption_skipped", operation_id, error_msg, context
                    )
            ...
            if self.config.algorithm == "AES-256-GCM":
                encrypted_data = await self._encrypt_aes_gcm(
                    plaintext, encryption_key, operation_id
                )
            elif self.config.algorithm == "ChaCha20-Poly1305":
                encrypted_data = await self._encrypt_chacha20(
                    plaintext, encryption_key, operation_id
                )
            else:
                # Fallback to Fernet
                encrypted_data = await self._encrypt_fernet(
                    plaintext, encryption_key, operation_id
                )
            # Verify encryption
    ...
    async def decrypt(
        ...
        Args:
            ...
            context: Additional context for auditing
        Returns:
            EncryptionResult with decrypted data or error
        """
        operation_id = secrets.token_hex(8)
        start_time = datetime.utcnow()
        try:
            ...
            if not self._crypto_available:
                ...
                if policy == EncryptionPolicy.OPTIONAL:
                    await self._log_encryption_event(
                        "decryption_skipped", operation_id, error_msg, context
                    )
                    return EncryptionResult(
            ...
            try:
                if self.config.algorithm == "AES-256-GCM":
                    plaintext = await self._decrypt_aes_gcm(
                        ciphertext, encryption_key, operation_id
                    )
                elif self.config.algorithm == "ChaCha20-Poly1305":
                    plaintext = await self._decrypt_chacha20(
                        ciphertext, encryption_key, operation_id
                    )
                else:
                    # Fallback to Fernet
                    plaintext = await self._decrypt_fernet(
                        ciphertext, encryption_key, operation_id
                    )
            except Exception:
                # Try different algorithms as fallback
                plaintext = await self._try_fallback_decryption(
                    ciphertext, encryption_key, operation_id
                )
    ...
    async def _decrypt_aes_gcm(
        self, encrypted_data: str, key: bytes, operation_id: str
    ) -> str:
    ...
    async def _try_fallback_decryption(
        self, ciphertext: str, key: bytes, operation_id: str
    ) -> str:
        ...
        for alg_name, decrypt_func in algorithms:
            try:
                logger.info(
                    f"Trying fallback decryption with {alg_name} "
                    f"(operation_id: {operation_id})"
                )
...
async def encrypt_sensitive_data(
    data: str, policy: EncryptionPolicy = EncryptionPolicy.REQUIRED
) -> EncryptionResult:
    """Encrypt sensitive data (child information, medical notes, etc.)."""
    service = get_encryption_service()
    return await service.encrypt(
        data, policy=policy, context={"data_type": "sensitive"}
    )


async def decrypt_sensitive_data(
    encrypted_data: str, policy: EncryptionPolicy = EncryptionPolicy.REQUIRED
) -> EncryptionResult:
    """Decrypt sensitive data with strict policy enforcement."""
    service = get_encryption_service()
    return await service.decrypt(
        encrypted_data, policy=policy, context={"data_type": "sensitive"}
    )
