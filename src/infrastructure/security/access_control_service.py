...
class AccessControlService:
    ...
    async def register_parent_child_relationship(
        ...
        return {
            "relationship_id": relationship_id,
            "access_level": access_level.value,
            "permissions": [action.value for action in self.access_permissions[access_level]],
    ...
    async def verify_access(
        ...
        try:
            ...
            if not relationship:
                await self._log_failed_access(parent_id, child_id, action, "no_relationship")
                return {
            ...
            if relationship["status"] != "active":
                await self._log_failed_access(parent_id, child_id, action, "inactive_relationship")
                return {
                    "access_granted": False,
            ...
            if relationship["expires_at"]:
                ...
                if datetime.utcnow() > expiry:
                    await self._log_failed_access(parent_id, child_id, action, "expired_relationship")
                    return {
                        "access_granted": False,
            ...
            if action not in allowed_actions:
                await self._log_failed_access(parent_id, child_id, action, "insufficient_permissions")
                return {
                    "access_granted": False,
            ...
            access_token = await self._generate_access_token(
                parent_id, child_id, action, relationship
            )
            ...
            await self._log_failed_access(parent_id, child_id, action, f"system_error: {e}")
            return {
                "access_granted": False,
    ...
    async def _find_relationship(self, parent_id: str, child_id: str) -> Optional[Dict[str, Any]]:
        """Find active parent - child relationship."""
        if parent_id not in self.parent_child_relationships:
            return None
        
        for relationship in self.parent_child_relationships[parent_id]:
            if relationship["child_id"] == child_id and relationship["status"] == "active":
                return relationship
    ...
    async def validate_access_token(
        ...
        if (token["parent_id"] != parent_id or
            token["child_id"] != child_id or
            token["action"] != action.value):
    ...
    async def _log_failed_access(
        ...
        recent_failures = [
            attempt for attempt in self.failed_access_attempts[parent_id]
            if datetime.utcnow() - datetime.fromisoformat(attempt["timestamp"]) < timedelta(hours=1)
        ]
        
        if len(recent_failures) >= 5:
            logger.warning(f"Suspicious access activity detected for parent {parent_id}")
