...
try:
    ...
    raise ImportError(
        f"Missing required dependency for encryption: cryptography. Install with 'pip install cryptography'"
    ) from e

from src.infrastructure.security.models import EncryptionMetadata, COPPAComplianceRecord
...
class ChildDataEncryption:
    ...
    def validate_coppa_compliance(
        self, child_data: Dict[str, Any], consent_record: COPPAComplianceRecord
    ) -> Dict[str, Any]:
        ...
        pii_fields_present = [
            ...
            if field in child_data and child_data[field]
        ]
    ...
    def create_consent_record(
        self, child_id: str, consent_method: str, ip_address: Optional[str] = None
