...
class SecureErrorHandler:
    ...
    def _compile_sensitive_patterns(self) -> None:
        ...
        self.sensitive_patterns = [
            ...
            re.compile(r'\b(database|sql|query|table|column|constraint)\b', re.IGNORECASE),
            re.compile(r'\b(postgresql|mysql|sqlite|mongodb)\b', re.IGNORECASE),
            re.compile(r'\b(connection|host|port|username|password)\b', re.IGNORECASE),
            # File system information
            re.compile(r'[/\\][a-zA-Z0-9_\\-/\\\\.]+', re.IGNORECASE),  # File paths
            re.compile(r'\b(directory|folder|file|disk|volume)\b', re.IGNORECASE),
            # Network information
            ...
            re.compile(r'\b(traceback|stack|frame|line \\d+)\b', re.IGNORECASE),
            re.compile(r'\b(module|function|class|method)\b', re.IGNORECASE),
            re.compile(r'src[/\\][a-zA-Z0-9_\\-/\\\\.]+', re.IGNORECASE),
            # Cryptographic information
            re.compile(r'\b(key|token|secret|hash|cipher|encrypt)\b', re.IGNORECASE),
            re.compile(r'\b[A-Za-z0-9+/]{20,}={0,2}\b'),  # Base64 patterns
            # System information
            re.compile(r'\b(version|python|fastapi|server|application)\b', re.IGNORECASE),
        ...
        self.child_sensitive_patterns = [
            ...
            re.compile(r'\b(medical|health|condition|medication)\b', re.IGNORECASE),
        ]
    def _setup_error_mappings(self) -> None:
        ...
        self.child_error_mappings = {
            ...
            "child_safety_violation": "That message is not appropriate. Please try something else.",
    ...
    async def handle_error(
        ...
        try:
            ...
            safe_message = self._create_safe_message(error_info, context, error_category)
            # Log detailed error internally
            await self._log_detailed_error(error, context, error_info, severity, support_reference)
            ...
            response = SecureErrorResponse(
                ...
                support_reference=support_reference if severity in [ErrorSeverity.HIGH, ErrorSeverity.CRITICAL] else None
            )
            return response
    ...
    def _create_safe_message(
        ...
        is_child_context = (
            context.child_id is not None or
            "/children" in context.endpoint or
            "/interact" in context.endpoint or
        ...
        return category_messages.get(category, "An error occurred. Please try again later.")
    ...
    async def _log_detailed_error(
        ...
        try:
            ...
            if severity == ErrorSeverity.CRITICAL:
                logger.critical(f"Critical error (ref: {support_reference}): {error_info['original_message']}")
            elif severity == ErrorSeverity.HIGH:
                logger.error(f"High severity error (ref: {support_reference}): {error_info['original_message']}")
            elif severity == ErrorSeverity.MEDIUM:
                logger.warning(f"Medium severity error (ref: {support_reference}): {error_info['original_message']}")
            else:
                logger.info(f"Low severity error (ref: {support_reference}): {error_info['original_message']}")
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            context = ErrorContext(
                ...
                user_agent=next((h[1].decode() for h in scope.get("headers", []) if h[0] == b"user-agent"), None),
                timestamp=datetime.utcnow()
            )
            # Handle error securely
            secure_response = await self.error_handler.handle_error(error, context)
...
# Global error handler instance_error_handler: Optional[SecureErrorHandler] = Nonedef get_error_handler() -> SecureErrorHandler:
...
async def handle_validation_error(error: RequestValidationError, context: ErrorContext) -> SecureErrorResponse:
    """Handle validation errors securely."""
    handler = SecureErrorHandler()
    return await handler.handle_error(error, context, ErrorCategory.VALIDATION)

async def handle_security_error(error: Exception, context: ErrorContext) -> SecureErrorResponse:
    """Handle security errors with maximum protection."""
    handler = SecureErrorHandler()
    return await handler.handle_error(error, context, ErrorCategory.SECURITY)

async def handle_child_safety_error(error: Exception, context: ErrorContext) -> SecureErrorResponse:
    """Handle child safety errors with special care."""
    handler = SecureErrorHandler()
    return await handler.handle_error(error, context, ErrorCategory.CHILD_SAFETY)
