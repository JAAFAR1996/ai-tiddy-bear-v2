...
class PluginValidator:
    ...
    def _contains_forbidden_content(self, source_code: str) -> bool:
        ...
        for forbidden in self.FORBIDDEN_IMPORTS:
            if (
                f"import {forbidden}" in source_lower
                or f"from {forbidden}" in source_lower
            ):
                return True
...
class PluginManager:
        ...
        try:
            ...
            if not self.validator.validate_plugin(plugin_path):
                raise PluginSecurityError(
                    f"Plugin {plugin_name} failed security validation"
                )
    ...
    def _load_manifest(self, manifest_path: Path) -> PluginManifest | None:
