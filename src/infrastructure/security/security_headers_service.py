...
class SecurityLevel(Enum):
    ...
    PARENT_MODERATE = "parent_moderate"  # Balanced security for parent interfaces
...
class SecurityHeadersService:
    ...
    """
    def __init__(self, default_level: SecurityLevel = SecurityLevel.CHILD_STRICT) -> None:
        ...
        logger.info(f"Security headers service initialized with {default_level.value} level")

    def _initialize_configurations(self) -> Dict[SecurityLevel, SecurityHeadersConfiguration]:
        ...
        return {
            SecurityLevel.CHILD_STRICT: SecurityHeadersConfiguration(
                ...
                csp_style_src={"'self'", "'unsafe-inline'"},  # Needed for some child-friendly styling
            ...
            SecurityLevel.PARENT_MODERATE: SecurityHeadersConfiguration(
                ...
                csp_default_src={"'self'"},
                csp_script_src={"'self'", "'unsafe-inline'", "https://cdn.ai-teddy.com"},
                csp_style_src={"'self'", "'unsafe-inline'", "https://cdn.ai-teddy.com"},
            ...
            SecurityLevel.DEVELOPMENT: SecurityHeadersConfiguration(
                ...
                csp_script_src={"'self'", "'unsafe-inline'", "'unsafe-eval'", "http:", "https:"},
    ...
    def get_security_headers(self,
        ...
        logger.debug(f"Generated {len(headers)} security headers for level {level.value}")
        return headers

    def get_child_safety_headers(self, child_age: Optional[int] = None,
                               parental_consent: bool = False) -> Dict[str, str]:
        """
        ...
        headers = {
            ...
            "X-Parental-Consent": "verified" if parental_consent else "required"
        }
    ...
    def validate_security_headers(self, headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        if headers.get("X-Frame-Options") not in ["DENY", "SAMEORIGIN"]:
            weak_configurations.append("X-Frame-Options should be DENY or SAMEORIGIN")
        ...
        return {
            "secure": len(missing_headers) == 0 and len(weak_configurations) == 0,
    ...
    def _build_csp_header(self, config: SecurityHeadersConfiguration,
        ...
        if config.csp_default_src:
            directives.append(f"default-src {format_sources(config.csp_default_src)}")
        if config.csp_script_src:
            directives.append(f"script-src {format_sources(config.csp_script_src)}")
        if config.csp_style_src:
            directives.append(f"style-src {format_sources(config.csp_style_src)}")
        if config.csp_img_src:
            directives.append(f"img-src {format_sources(config.csp_img_src)}")
        if config.csp_connect_src:
            directives.append(f"connect-src {format_sources(config.csp_connect_src)}")
        if config.csp_font_src:
            directives.append(f"font-src {format_sources(config.csp_font_src)}")
        if config.csp_frame_src:
            directives.append(f"frame-src {format_sources(config.csp_frame_src)}")
        if config.csp_media_src:
            directives.append(f"media-src {format_sources(config.csp_media_src)}")
    ...
    def _validate_csp(self, csp_header: str) -> List[str]:
        ...
        if "'unsafe - eval'" in csp_header:
            issues.append("CSP contains 'unsafe - eval' which may allow code injection")
        if "'unsafe - inline'" in csp_header and "script-src" in csp_header:
            issues.append("CSP allows 'unsafe - inline' scripts which reduces XSS protection")
        if "data: " in csp_header and "script - src" in csp_header:
            issues.append("CSP allows data: URLs in script - src which may be risky")
