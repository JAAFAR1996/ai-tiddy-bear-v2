...
class EncryptionService:
    ...
    def _initialize_encryption(self) -> None:
        ...
        try:
            ...
            raise EncryptionKeyError(f"Encryption initialization failed: {str(e)}")
    ...
    def _derive_key(self, master_key: str) -> bytes:
        ...
        if os.getenv("PBKDF2_SALT") is None:
            logger.warning("PBKDF2_SALT not found in environment, using randomly generated salt.")
        kdf = PBKDF2HMAC(
    ...
    def decrypt(self, encrypted_data: Union[str, bytes]) -> Union[str, bytes, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if metadata.get("version") != self._key_version:
                logger.info(f"Decrypting data from older key version: {metadata.get('version')}")
            # Decode actual data
            ...
            raise EncryptionKeyError("Decryption failed - data may be corrupted or key mismatch")
        except Exception as e:
    ...
    def rotate_key(self, new_key: str) -> None:
        ...
        try:
            ...
            logger.info(f"Key rotated successfully from {old_version} to {self._key_version}")
        except Exception as e:
            # Rollback on failure
