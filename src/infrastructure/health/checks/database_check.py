...
class DatabaseHealthCheck:
    ...
    def __init__(self, database_session: Optional[AsyncSession] = None) -> None:
    ...
    async def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            async with self.database_session() as session:
                ...
                pool_stats = {
                    "size": session.bind.pool.size() if hasattr(session.bind, 'pool') else 0,
                    "checked_in": session.bind.pool.checkedin() if hasattr(session.bind, 'pool') else 0,
                    "overflow": session.bind.pool.overflow() if hasattr(session.bind, 'pool') else 0,
                }
