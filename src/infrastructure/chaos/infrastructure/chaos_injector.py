...
if TYPE_CHECKING:
    ...
    from .chaos_orchestrator import FailureType, ExperimentMetrics
...
class ChaosInjector:
    ...
    async def inject_failure(
        self, target: str, failure_type: FailureType, intensity: float, metrics: ExperimentMetrics,
    ):
    ...
    async def _inject_network_latency(self, target: str, intensity: float):
        ...
        await self.orchestrator._execute_chaos_command(f"tc qdisc add dev eth0 root netem delay {latency_ms}ms", target)
    ...
    async def _inject_service_crash(self, target: str, intensity: float):
        ...
        if secrets.randbelow(10000) / 10000 < crash_probability:
            await self.orchestrator._execute_chaos_command(f"kubectl delete pod -l app={target} --grace-period=0 --force", target)
    ...
    async def _inject_memory_pressure(self, target: str, intensity: float):
        ...
        await self.orchestrator._execute_chaos_command(f"stress --vm 1 --vm-bytes {memory_mb}M --timeout 60s", target)
    
    async def _inject_cpu_spike(self, target: str, intensity: float):
        """Inject CPU spikes"""
        cpu_load = int(100 * intensity)
        await self.orchestrator._execute_chaos_command(f"stress --cpu {cpu_load} --timeout 60s", target)
