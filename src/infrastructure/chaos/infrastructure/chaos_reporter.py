...
class ChaosReporter:
    ...
    def get_experiment_report(self, experiment_id: str) -> dict[str, any]:
        ...
        metrics = next(
            (m for m in self.orchestrator.experiment_history if m.experiment_id == experiment_id),
            None,
        )
        ...
        duration = (
            (metrics.end_time - metrics.start_time).total_seconds() if metrics.end_time else 0
        )
        ...
        return {
            ...
            "overall_status": (
                "PASS" if metrics.safety_violations == 0 and metrics.success_rate > 0.8 else "FAIL"
            ),
        }
    ...
    def get_system_resilience_score(self) -> dict[str, any]:
        ...
        successful_experiments = sum(
            1 for m in self.orchestrator.experiment_history 
            if m.safety_violations == 0 and m.success_rate > 0.8
        )
        
        avg_recovery_time = (
            sum(m.recovery_time_seconds for m in self.orchestrator.experiment_history) / total_experiments
        )
        
        avg_success_rate = (
            sum(m.success_rate for m in self.orchestrator.experiment_history) / total_experiments
        )
        
        total_safety_violations = sum(m.safety_violations for m in self.orchestrator.experiment_history)
