...
class SpeechDisorderDetector:
    ...
    async def analyze_speech_for_disorders(self, audio_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_result = await self.disorder_analyzer.analyze_for_disorders(features)
            
            # Add metadata
            analysis_result['audio_quality'] = validation_result
            analysis_result['features_extracted'] = True
            
            logger.info(f"Speech disorder analysis completed. Found {len(analysis_result.get('disorders_detected', []))} disorders")
