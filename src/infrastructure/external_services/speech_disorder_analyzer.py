...
class DisorderAnalyzer:
        ...
        try:
            ...
            for analysis in all_analyses:
                if analysis['detected']:
                    analysis_result['disorders_detected'].append(analysis['disorder_type'])
                    analysis_result['confidence_scores'][analysis['disorder_type']] = analysis['confidence']
            ...
            analysis_result['professional_referral_needed'] = self._needs_referral(all_analyses)
            
            return analysis_result
        except Exception as e:
            logger.error(f"Disorder analysis error: {e}")
            return {
                'error': f'Disorder analysis failed: {str(e)}',
    ...
    async def _analyze_stuttering(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        detected = confidence > self.config.disorder_patterns['stuttering']['confidence_threshold']
    ...
    async def _analyze_lisping(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        spectral_centroid = features.get('spectral_features', {}).get('spectral_centroid', 2500)
    ...
    async def _analyze_articulation(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        zero_crossing_rate = features.get('spectral_features', {}).get('zero_crossing_rate', 0.15)
        
        confidence = 0.0
        detected = False
        
        if zero_crossing_rate > 0.2:  # High zero crossing rate might indicate articulation issues
            confidence = 0.6
            detected = True
    ...
    async def _analyze_voice_disorder(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        fundamental_frequency = features.get('prosodic_features', {}).get('fundamental_frequency', 180)
        ...
        detected = confidence > self.config.disorder_patterns['voice_disorder']['confidence_threshold']
