...
class Container(containers.DeclarativeContainer):
    ...
    event_store = providers.Selector(
        lambda settings_instance: (
            EventStoreType.KAFKA
            if settings_instance.kafka.KAFKA_ENABLED
            else EventStoreType.POSTGRES
        ),
    ...
    # to ensure isolated transactions and prevent state leakage across
    # requests.
    child_repository = providers.Factory(
        EventSourcedChildRepository,
        event_store=event_store,
    )
    conversation_repository: providers.Factory[IConversationRepository] = (
        providers.Factory(
    ...
    # with other Factory-provided dependencies, thus requiring a new instance
    # per request.
    ...
    ai_orchestration_service: providers.Factory[AIOrchestrationService] = (
        providers.Factory(
            AIOrchestrationService,
    ...
    dynamic_content_service: providers.Factory[DynamicContentService] = (
        providers.Factory(
            DynamicContentService,
    ...
    # or Factory if it processes data per-request/per-batch. Assuming global
    # aggregation.
