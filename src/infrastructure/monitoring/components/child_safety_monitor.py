...
class ChildSafetyMonitor:
    ...
    def _create_pattern_alert(self, child_id: str, pattern_type: str, count: int) -> None:
        ...
        logger.error(f"Pattern alert created: {pattern_type} for child {child_id} (count: {count})")
    ...
    def get_child_safety_status(self, child_id: str) -> Dict[str, Any]:
        ...
        recent_events = [
            event for event in self.safety_events
            if (event["child_id"] == child_id and
                datetime.fromisoformat(event["timestamp"]) > twenty_four_hours_ago)
        ]
        # Get active alerts for this child
        active_alerts = [
            alert for alert in self.safety_alerts.values()
