...
class FallbackResponseGenerator:
    ...
    def __init__(self) -> None:
        ...
        self.safety_redirect_responses = {
            "violence": [
                "Instead of that, let's talk about friendship and helping others! What makes a good friend?",
                "How about we explore peaceful solutions? What activities make you feel happy and safe?",
                "Let's focus on positive things! Tell me about your favorite peaceful activities!"
            ],
            "scary": [
                "That might be too scary for us! Let's talk about something fun instead! What's your favorite adventure?",
                "How about we focus on exciting but safe adventures? What would you like to explore?",
                "Let's talk about something that makes you feel happy and brave! What's your favorite story?"
            ],
            "inappropriate": [
                "That's a topic for grown-ups! Let's talk about something fun for kids! What's your favorite game?",
                "How about we explore something more suitable for young explorers like you? What interests you?",
    ...
    async def generate_fallback_response(self, message: str, child_age: int, preferences: Dict[str, Any] = None) -> Dict[str, Any]:
        """توليد استجابة احتياطية آمنة"""
        preferences = preferences or {}
        # اختيار استجابة مناسبة للعمر
        age_responses = self.age_appropriate_responses.get(child_age, self.age_appropriate_responses[6])
    ...
    async def generate_safety_redirect_response(self, message: str, child_age: int) -> Dict[str, Any]:
        ...
        elif any(word in message_lower for word in ["scary", "monster", "nightmare", "ghost"]):
            redirect_type = "scary"
        # اختيار استجابة إعادة توجيه
        redirect_responses = self.safety_redirect_responses.get(redirect_type, self.safety_redirect_responses["inappropriate"])
    ...
    def generate_learning_suggestion(self, topic: str, child_age: int) -> str:
        ...
        return suggestions.get(topic, f"Let's explore something new together! What interests you most?")

    def get_conversation_starter(self, child_age: int, interests: List[str] = None) -> str:
