...
try:
    ...
    logger.critical(f"CRITICAL ERROR: Rate limiting dependencies required: {e}")
    logger.critical("Install required dependencies: pip install fastapi redis")
    raise ImportError(f"Missing required dependencies for rate limiting: {e}") from e
...
class ProductionRateLimiter:
    ...
    async def check_rate_limit(self, client_id: str, endpoint: str = "") -> RateLimitResult:
        ...
        try:
            ...
            burst_result = await self._check_burst_limit(client_id, current_time)
            ...
            minute_result = await self._check_time_window(
                client_id, current_time, 60, self.config.requests_per_minute, "minute"
            )
            ...
            hour_result = await self._check_time_window(
                client_id, current_time, 3600, self.config.requests_per_hour, "hour"
            )
            ...
            day_result = await self._check_time_window(
                client_id, current_time, 86400, self.config.requests_per_day, "day"
            )
            ...
            return RateLimitResult(
                allowed=True,
                remaining_requests=self.config.requests_per_minute - minute_result.remaining_requests
            )
    ...
    async def _check_burst_limit(self, client_id: str, current_time: float) -> RateLimitResult:
    ...
    async def _record_request(self, client_id: str, timestamp: float, endpoint: str) -> None:
    ...
    async def _record_violation(self, client_id: str, violation_type: str) -> None:
        ...
        try:
            ...
            await self.redis.expire(violation_key, 3600)  # Reset violations every hour
            
            # Log violation
            logger.warning(f"Rate limit violation: {client_id} - {violation_type} (total: {violations})")
            ...
            if violations >= 5:
                ...
                logger.critical(f"IP blocked due to repeated violations: {client_id}")
    ...
    async def _send_rate_limit_alert(self, client_id: str, violation_count: int) -> None:
        ...
        try:
            ...
            await self.redis.ltrim("security:alerts", 0, 1000)  # Keep last 1000 alerts
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if not self.rate_limiter:
                ...
                self.rate_limiter = ProductionRateLimiter(redis_client, self.config)
            ...
            if not result.allowed:
                # Rate limit exceeded
                headers = {
                    "X-RateLimit-Limit": str(self._get_limit_for_type(result.limit_type)),
                    "X-RateLimit-Remaining": str(result.remaining_requests),
                    "X-RateLimit-Reset": str(int(result.reset_time.timestamp())) if result.reset_time else "",
    ...
    def _get_client_identifier(self, request: Request) -> str:
        ...
        if forwarded_for:
            ...
            client_ip = request.headers.get("X-Real-IP", getattr(request.client, 'host', '127.0.0.1'))
        ...
        if auth_header:
            ...
            user_hash = hashlib.sha256(f"{client_ip}:{auth_header}".encode()).hexdigest()[:16]
...
def create_rate_limit_middleware(redis_url: str, **kwargs) -> RateLimitMiddleware:
