...
class ServerSettings(BaseModel):
    ...
    SSL_KEY_PATH: Optional[str] = Field(
        None, description="Path to SSL key file")
...
class DatabaseSettings(BaseModel):
    ...
    DATABASE_SSL_MODE: str = Field(
        "require", description="SSL mode for database connection"
    )
...
class SecuritySettings(BaseModel):
    ...
    RATE_LIMIT_PER_MINUTE: int = Field(
        100, description="Max requests per minute per IP"
    )
...
class LoggingSettings(BaseModel):
    ...
    LOG_ROTATION: str = Field(
        "10 MB", description="Log file rotation size or frequency"
    )
...
class FeatureFlagsSettings(BaseModel):
    ...
    NEW_FEATURE_ENABLED: bool = Field(
        False, description="Enable new experimental feature"
    )
