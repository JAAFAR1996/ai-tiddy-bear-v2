...
from src.infrastructure.security.main_security_service import MainSecurityService # Import the new security service
from src.infrastructure.di.container import container
from src.domain.exceptions.base import StartupValidationException # Import custom exception
...
def enforce_production_safety() -> None:
    ...
    if environment == "production":
        ...
        security_service = container.security_service() # Assuming security_service is wired
        errors = security_service.validate_production_environment_security()
        
        if errors:
            error_messages = "; ".join(errors)
            logger.critical(f"Production environment validation failed: {error_messages}")
            raise StartupValidationException(f"Critical production environment security checks failed: {error_messages}")
