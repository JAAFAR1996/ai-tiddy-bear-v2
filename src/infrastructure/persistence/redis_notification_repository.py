...
class RedisNotificationRepository(INotificationRepository):
    ...
    async def save_notification(self, notification: NotificationRecord) -> None:
        ...
        try:
            notification_data_dict = {
                ...
                "last_attempt_at": notification.last_attempt_at.isoformat() if notification.last_attempt_at else None,
            ...
            # For simplicity, using a list of IDs for now, or could use hset for a smaller set of recent items
            await self.redis_client.lpush(history_key, str(notification.id))
            await self.redis_client.ltrim(history_key, 0, 99) # Keep last 100 notifications per recipient
            self.logger.debug(f"Saved notification {notification.id} for recipient {notification.recipient} to Redis.")
        except Exception as e:
            self.logger.error(f"Error saving notification {notification.id} to Redis: {e}", exc_info=True)
        ...
        try:
            ...
            if notification_data_json:
                ...
                notification = NotificationRecord(
                    ...
                    created_at=datetime.fromisoformat(notification_data_dict["created_at"]),
                    ...
                    last_attempt_at=datetime.fromisoformat(notification_data_dict["last_attempt_at"]) if notification_data_dict.get("last_attempt_at") else None,
                    error_message=notification_data_dict.get("error_message"),
                    metadata=notification_data_dict.get("metadata", {}),
                )
                self.logger.debug(f"Retrieved notification {notification_id} from Redis.")
                return notification
            self.logger.debug(f"Notification {notification_id} not found in Redis.")
            return None
        except Exception as e:
            self.logger.error(f"Error getting notification {notification_id} from Redis: {e}", exc_info=True)
            return None
    ...
    async def get_notifications_for_recipient(self, recipient: str) -> List[NotificationRecord]:
        ...
        try:
            ...
            self.logger.debug(f"Retrieved {len(notifications)} notifications for recipient {recipient}.")
            return notifications
        except Exception as e:
            self.logger.error(f"Error getting notifications for recipient {recipient} from Redis: {e}", exc_info=True)
            return []
    ...
    async def delete_notification(self, notification_id: UUID) -> bool:
        ...
        try:
            ...
            if result > 0:
                ...
                return True
            self.logger.debug(f"Notification {notification_id} not found for deletion in Redis.")
            return False
        except Exception as e:
            self.logger.error(f"Error deleting notification {notification_id} from Redis: {e}", exc_info=True)
            return False 