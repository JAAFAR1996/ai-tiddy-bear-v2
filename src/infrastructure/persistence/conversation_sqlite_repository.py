...
class ConversationSQLiteRepository:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_child_timestamp ON conversations(child_id, timestamp)")
    ...
    async def save_conversation(self, conversation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Conversation saved successfully for child {child_id}: ID {conversation_id}")
            return {
                "success": True,
    ...
    async def get_conversations(self, child_id: str, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    async def delete_child_conversations(self, child_id: str) -> Dict[str, Any]:
        ...
        def _delete_from_db():
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute("DELETE FROM conversations WHERE child_id = ?", (child_id,))
    ...
    async def _validate_conversation_safety(self, conversation_data: Dict[str, Any]):
        ...
        # Basic safety checks
        unsafe_patterns = [
            'personal information', 'address', 'phone number', 'meet me',
            'where do you live', 'send photo', 'password', 'secret'
        ]
        for pattern in unsafe_patterns:
            if pattern.lower() in message.lower() or pattern.lower() in response.lower():
    ...
    async def _cleanup_old_conversations(self, child_id: str, keep_latest: int = 500):
        ...
        logger.info(f"Cleaned up {deleted_count} old conversations for child {child_id}")