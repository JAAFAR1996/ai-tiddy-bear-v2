...
class RedisSessionRepository(ISessionRepository):
    ...
    async def get(self, session_id: str) -> Optional[SessionData]:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting session {session_id} from Redis: {e}", exc_info=True
            )
            return None

    async def save(self, session_data: SessionData,
        ...
        try:
            ...
            await self.redis_client.setex(
                key, timedelta(
                    minutes=timeout_minutes), json.dumps(session_data_dict)
            )
            self.logger.debug(
    ...
    async def delete(self, session_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error deleting session {session_id} from Redis: {e}", exc_info=True
            )
            return False
