...
class RedisSessionRepository(ISessionRepository):
        ...
        try:
            ...
            if session_data_json:
                ...
                session_data = SessionData(
                    ...
                    last_activity=datetime.fromisoformat(session_data_dict["last_activity"]),
            ...
            self.logger.error(f"Error getting session {session_id} from Redis: {e}", exc_info=True)
            return None
    ...
    async def save(self, session_data: SessionData, timeout_minutes: int) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Saved session {session_data.session_id} to Redis with timeout {timeout_minutes} min.")
        except Exception as e:
            self.logger.error(f"Error saving session {session_data.session_id} to Redis: {e}", exc_info=True)
    ...
    async def delete(self, session_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error deleting session {session_id} from Redis: {e}", exc_info=True)
            return False
    ...
    async def delete_expired(self, timeout_minutes: int) -> None:
        ...
        self.logger.info("Redis handles session expiration automatically. Explicit delete_expired not required.") 