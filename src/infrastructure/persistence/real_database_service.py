...
from src.infrastructure.security.sql_injection_prevention import get_sql_injection_prevention
from src.infrastructure.security.coppa import get_consent_manager  # ✅ COPPA consent verification
...
class DatabaseService:
    ...
    async def create_user(self, email: str, hashed_password: str, role: str) -> str:
        ...
        try:
            ...
            # Validate operation
            validated_operation = validate_database_operation("INSERT", "users", user_data)
            ...
            async with self.database.get_session() as session:
                ...
                logger.info(f"Created user: {validated_data['email']} with role: {validated_data['role']}")
                return user_id
            ...
            raise ValueError(f"User creation failed: database constraint violation")
    ...
    async def get_user_by_email(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not email_sanitization.safe:
                logger.warning(f"Unsafe email input detected: {email_sanitization.threats_found}")
                return None
            ...
            async with self.database.get_session() as session:
                ...
                users = await safe_session.safe_select(
                    "users",
                    ["id", "email", "password_hash", "role", "is_active", "email_verified", "created_at", "last_login"],
    ...
    async def create_child(self, name: str, age: int, preferences: Dict[str, Any], parent_id: str) -> str:
        """Create a new child profile with COPPA compliance verification."""
        # ✅ Verify COPPA compliance before creating child profile
        if age <= 13:
            # For children 13 and under, require explicit data collection consent
            ...
            if not has_consent:
                raise ValueError("Parental consent required for data collection (COPPA compliance)")
        async with self.database.get_session() as session:
            child_id = str(uuid4())  # Generate UUID string for SQLite compatibility
            child = ChildModel(
                id=child_id,
                name=name,
                age=age,
            ...
            session.add(child)
            await session.commit()
            logger.info(f"Created child: {name} (ID: {child_id}) for parent: {parent_id}")
            return child_id
    ...
    async def create_conversation(self, child_id: str, parent_id: str, transcript: List[Dict[str, str]]) -> str:
    ...
    async def get_conversations_by_child_id(self, child_id: str) -> List[Dict[str, Any]]:
        """Retrieve conversation history for a child."""
        async with self.database.get_session() as session:
            result = await session.execute(
                select(ConversationModel).filter(ConversationModel.child_id == child_id).order_by(ConversationModel.created_at)
            )
