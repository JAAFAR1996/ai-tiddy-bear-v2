...
class COPPADataRetentionService:
    ...
    async def register_child_data(
        ...
        retention_record = {
            ...
            "deletion_scheduled_at": (
                datetime.utcnow() + timedelta(days=self.default_retention_days)
            ).isoformat(),
        ...
        logger.info(
            f"Child data registered for retention: {retention_id}, child: {child_id}",
        )
    ...
    async def schedule_daily_retention_check(self) -> None:
        ...
        notification_cutoff = current_time + timedelta(
            days=self.notification_days_before,
        )

        pending_notifications = []
        pending_deletions = []

        for _retention_id, record in self.retention_records.items():
...
Thank you for using AI Teddy Bear responsibly."""
    ...
    async def _send_deletion_confirmation(self, record: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to send deletion confirmation: {record['retention_id']}, {e}",
            )
...
Thank you for using AI Teddy Bear responsibly."""
    ...
    async def request_immediate_deletion(
        ...
        if record["status"] not in [
            ...
            RetentionStatus.EXTENDED.value,
        ]:
            return {
                "success": False,
                "error": f"Cannot delete: status is {record['status']}",
            }
